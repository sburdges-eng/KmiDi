...
class SpectocloudDataset(Dataset):
            ...
            samples.append(
                {
                    ...
                    "emotion": np.array(
                        [valence, arousal, intensity] + [0.0] * 61, dtype=np.float32
                    ),
    ...
    def __getitem__(self, idx):
        ...
        if self.use_real_data:
            ...
            if "target_pointcloud_path" in sample and os.path.exists(
                sample["target_pointcloud_path"]
            ):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if use_amp and scaler is not None:
            with torch.cuda.amp.autocast(
                dtype=torch.float16
                if config.get("amp_dtype", "float16") == "float16"
                else torch.bfloat16
            ):
...
def main(config_path: str):
    ...
    total_steps = sched_cfg.get(
        "max_steps", len(train_loader) * config.get("training", {}).get("epochs", 20)
    )
    ...
    for epoch in range(epochs):
        ...
        if is_best:
            ...
            print(
                f"Saved best model (val_loss: {best_val_loss:.4f}, improvement: {improvement:.4f})"
            )
        else:
            patience_counter += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="spectocloud_training_config.yaml", help="Path to config file"
    )
