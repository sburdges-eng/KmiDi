...
class MIDITokenizer:
    ...
    def __init__(self, vocab_size: int = 388, special_tokens: Optional[Dict[str, int]] = None):
        ...
        default_pad = vocab_size - 4
        default_bos = vocab_size - 3
        default_eos = vocab_size - 2
        default_bar = vocab_size - 1

        self.pad_token = special_tokens.get("pad", default_pad)
        self.bos_token = special_tokens.get("bos", default_bos)
        self.eos_token = special_tokens.get("eos", default_eos)
        self.bar_token = special_tokens.get("bar", default_bar)

        min_special = min(self.pad_token, self.bos_token, self.eos_token, self.bar_token)
        if min_special < self.timing_range[1]:
            print("Warning: special tokens overlap MIDI token ranges; using end-of-vocab defaults.")
            self.pad_token = default_pad
            self.bos_token = default_bos
            self.eos_token = default_eos
            self.bar_token = default_bar
...
def main(config_path: str):
    ...
    for epoch in range(epochs):
        ...
        val_metrics = evaluate(model, val_loader, device, pad_token=tokenizer.pad_token)
