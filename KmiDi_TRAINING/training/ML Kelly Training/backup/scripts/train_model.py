    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int]:
        ...
        if TORCHAUDIO_AVAILABLE and audio_path.exists():
            ...
            if not self.allow_synthetic_data:
                raise RuntimeError(
                    f"Audio missing or torchaudio unavailable for {audio_path}. "
                    "Set allow_synthetic_data=True to use synthetic data."
                )
            logger.warning("Using synthetic mel spectrogram for missing audio data")
...
def create_dataloaders(
    ...
    if data_path and data_path.exists():
        ...
        dataset = AudioDataset(
            ...
            augmentor=augmentor,
            allow_synthetic_data=config.allow_synthetic_data,
        )
    else:
        if not config.allow_synthetic_data:
            raise RuntimeError(
                "No data path specified. Set --synthetic or allow_synthetic_data=True to proceed."
            )
