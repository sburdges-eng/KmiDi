...
class MusicMetrics(BaseMetrics):
    ...
    def compute(self) -> Dict[str, MetricResult]:
        ...
        if not self._predictions or not self._targets:
            logger.warning("No targets available; skipping metric computation")
            return {}
...
def evaluate_model(
    ...
    if not has_targets:
        logger.warning("No targets provided by dataloader; returning empty metrics")
        return {}
...
def cross_validate(
    ...
    for fold in range(n_folds):
        ...
        is_regression = getattr(metrics, "task", "classification") != "classification"
