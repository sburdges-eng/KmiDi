"""GenerationTransaction context manager (Goal: Generation scope isolation,
...
def test_scope_returns_snapshot_of_state_on_enter():
    ...
    with GenerationTransaction(parent) as scope:
...
def test_nested_scopes_independent_rollback():
    parent = {"chord": "Am"}
    with GenerationTransaction(parent) as outer:
        outer.state["chord"] = "Dm"
        with GenerationTransaction(outer.state) as inner:
            inner.state["chord"] = "G"
...
def test_commit_outside_scope_raises():
    ...
    parent = {"chord": "Am"}
    scope = GenerationTransaction(parent)
    with scope:
        pass
