...
def audio_sandbox_root(tmp_path_factory):
    """A directory under which classify endpoints allow audio paths
    (module-scoped so app import sees it)."""
...
def test_classify_audio_accepts_path_inside_sandbox(client, audio_sandbox_root):
    ...
    # 200 if classifier works, 503 if model not available, 404 if file not found -
    # but not 400 path rejected
