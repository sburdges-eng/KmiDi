...
def test_spec_rejects_non_positive_sample_rate() -> None:
    with pytest.raises(ValueError):
        WavSpec(sample_rate=0, num_channels=1,
                bits_per_sample=16, total_samples_per_channel=0)


def test_spec_rejects_non_positive_channels() -> None:
    with pytest.raises(ValueError):
        WavSpec(sample_rate=44100, num_channels=0,
                bits_per_sample=16, total_samples_per_channel=0)


def test_spec_only_supports_16_bit() -> None:
    with pytest.raises(ValueError, match="16-bit"):
        WavSpec(sample_rate=44100, num_channels=1,
                bits_per_sample=24, total_samples_per_channel=0)
...
def test_in_order_chunks_emit_immediately() -> None:
    spec = WavSpec(sample_rate=44100, num_channels=1,
                   bits_per_sample=16, total_samples_per_channel=8)
...
def test_out_of_order_chunks_are_buffered_then_drained() -> None:
    spec = WavSpec(sample_rate=44100, num_channels=1,
                   bits_per_sample=16, total_samples_per_channel=12)
    writer = StreamingWavWriter(spec)
...
def test_replaying_old_index_raises() -> None:
    spec = WavSpec(sample_rate=44100, num_channels=1,
                   bits_per_sample=16, total_samples_per_channel=4)
    writer = StreamingWavWriter(spec)
...
def test_misaligned_chunk_size_raises() -> None:
    spec = WavSpec(sample_rate=44100, num_channels=2,
                   bits_per_sample=16, total_samples_per_channel=4)
    writer = StreamingWavWriter(spec)
...
def test_finalised_writer_refuses_more_chunks() -> None:
    spec = WavSpec(sample_rate=44100, num_channels=1,
                   bits_per_sample=16, total_samples_per_channel=2)
    writer = StreamingWavWriter(spec)
...
def test_deterministic_byte_output_regardless_of_submit_order() -> None:
    ...
    spec = WavSpec(sample_rate=44100, num_channels=1,
                   bits_per_sample=16, total_samples_per_channel=6)
    chunks = {
        0: b"\xaa\x00\xab\x00",
        1: b"\xbb\x00\xbc\x00",
        2: b"\xcc\x00\xcd\x00",
