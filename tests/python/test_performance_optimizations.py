...
def test_enumerate_vs_range_len():
    ...
    assert new_time <= old_time * 1.5, f"enumerate slower: old={old_time:.4f}s, new={new_time:.4f}s"
...
def test_zip_interval_calculation():
    ...
    # New should be faster
    assert new_time <= old_time * 1.3, f"zip not faster: old={old_time:.4f}s, new={new_time:.4f}s"
...
def test_polyrhythm_detection_performance():
    ...
    try:
        ...
        # Create test events
        events = [{"time": i * 0.5} for i in range(100)]
...
def test_performance_analysis_speed():
    ...
    try:
        ...
        # Create test events
        events = [{"time": i * 0.25, "velocity": 80 + (i % 20)} for i in range(200)]
