...
class SplitDeterministicTest(unittest.TestCase):
    ...
    def test_deterministic_assignments_independent_of_input_order(self) -> None:
        ...
        split_a = split_task_records(original, seed="seed-A", train_ratio=0.8, val_ratio=0.1, test_ratio=0.1)
        split_b = split_task_records(shuffled, seed="seed-A", train_ratio=0.8, val_ratio=0.1, test_ratio=0.1)
    ...
    def test_session_records_stay_in_one_split(self) -> None:
        ...
        split_map = split_task_records(records, seed="seed-B", train_ratio=0.8, val_ratio=0.1, test_ratio=0.1)
        ...
        for split_name, items in split_map.items():
            for record in items:
                ...
                if previous is None:
                    ...
                    self.assertEqual(previous, split_name, f"session {session_id} leaked across splits")
