...
const createMockInvoke = () => {
  ...
  return {
    ...
    getAverageCallTime: () => callTimes.reduce((a, b) => a + b, 0) / callTimes.length,
  };
};
...
describe('React Hook Performance Tests', () => {
  ...
  describe('Memory Usage Performance', () => {
    it('should not leak memory with repeated operations', async () => {
      ...
      console.log(`Memory delta after ${iterations} operations: ${memoryDeltaMB.toFixed(2)}MB`);
    });
  });

  describe('Event System Performance', () => {
    it('should handle high-frequency events efficiently', async () => {
      const { useKellyBrainEvents } = await import('../../src/hooks/useKellyBrain');
...
export class FrontendPerformanceBenchmark {
  static async measureHookPerformance(hookName: string, iterations: number = 100) {
    const times: number[] = [];
  ...
  static async measureApiCallPerformance(apiCalls: string[], iterations: number = 50) {
...
describe('Frontend Performance Benchmarks', () => {
  it('should benchmark API call performance', async () => {
    ...
    const results = await FrontendPerformanceBenchmark.measureApiCallPerformance(apiCalls, 20);
    
    FrontendPerformanceBenchmark.generatePerformanceReport(results);
    
    // Verify performance requirements
    expect(results['kelly_brain_set_emotion_parameters'].averageTime).toBeLessThan(10);
    expect(results['kelly_brain_get_emotion_state'].averageTime).toBeLessThan(15);
  ...
  it('should benchmark hook initialization performance', async () => {
    const result = await FrontendPerformanceBenchmark.measureHookPerformance('useKellyBrain', 10);
...
describe('Memory Leak Detection', () => {
  it('should not leak memory with repeated hook usage', async () => {
    ...
    console.log(`Memory growth after repeated usage: ${memoryGrowthMB.toFixed(2)}MB`);
  });
});
...
describe('Integration Performance', () => {
  it('should handle complete workflow efficiently', async () => {
    const { useSimpleKellyBrain } = await import('../../src/hooks/useKellyBrain');
    ...
    await act(async () => {
      ...
      await result.current.initialize('./test-data');
      const midi = await result.current.generateMusicFromText('feeling contemplative', 8);
  ...
  it('should maintain performance under concurrent usage', async () => {
    ...
    await Promise.all(hooks.map(({ result }, i) =>
      act(async () => {
