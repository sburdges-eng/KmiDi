...
| `cursor/rust-test-env-var-race-2d5c` | 2026-03-17 | 1 | **Independent — needs decision** | Adds `serial_test` to `engine/intent_ir/Cargo.toml`. Doesn't conflict with the audit stack. Either land as a single commit on `main` after the stack, or close as not-needed if the test it serializes was deleted. |
