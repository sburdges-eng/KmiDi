...
|------|------|-----------|--------|
| `src-tauri/binaries/kmidi_brain-aarch64-apple-darwin` | ~307 MB | No — build artifact | Build via PyInstaller + `build_v1.sh`; copy to `src-tauri/binaries/kmidi_brain-<triple>`. Now in `.gitignore`. |
...
- `src-tauri/target/`, `build/`, `node_modules/`, `npm-cache/`
