...
| `MusicTheoryBridge.cpp:312` FIXME | MEDIUM | ✅ **FIXED 2026-04-14** — re-enabled C++ fallback; `getCustomLearningPath()` signature was already correct, `UserProfile{}` default-inits safely |
| WavJEPA emotion experiment | MEDIUM | ✅ Experiment run; results in `experiments/exp_002_wavjepa_emotion/output/results_wavjepa.json` |
| Stem JEPA stubs | LOW | ✅ Stubs documented; `StemJEPACompatibility` returns typed `StemCompatibilityScore` with confidence=0.0 to signal placeholder clearly |
| StructXLIP advanced losses | LOW | ✅ Already implemented — `local_structure_loss` + `consistency_edge_loss` in `music_brain/penta_core/ml/structxlip/structure_losses.py` |
| Video module TODOs | LOW | Deferred — Jespa/Unreal Engine external APIs; 70+ TODOs require external service contracts |
