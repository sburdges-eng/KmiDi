...
std::optional<EmotionNode> EmotionWheel::getSelectedEmotion() const {
    if (selectedEmotionId_ && thesaurus_) {
        return thesaurus_->findById(*selectedEmotionId_);
    }
    return std::nullopt;
}
...
void EmotionWheel::drawEmotionWheel(juce::Graphics& g, const juce::Rectangle<int>& bounds) {
    ...
    g.drawText("Negative", centerX - maxRadius - 40, centerY - 6, 40, 12, juce::Justification::centredRight);
    g.drawText("Positive", centerX + maxRadius, centerY - 6, 40, 12, juce::Justification::centredLeft);
    g.drawText("Calm", centerX - 20, centerY - maxRadius - 15, 40, 12, juce::Justification::centred);
    g.drawText("Excited", centerX - 20, centerY + maxRadius + 3, 40, 12, juce::Justification::centred);
}
...
void EmotionWheel::drawEmotionPoint(juce::Graphics& g, const EmotionPosition& pos, bool isSelected, bool isHovered) {
    ...
    juce::Colour pointColour = isSelected ? juce::Colours::yellow 
                                          : (isHovered ? juce::Colours::lightblue : juce::Colours::white);
    ...
    if (isSelected || isHovered) {
        if (thesaurus_) {
            auto emotion = thesaurus_->findById(pos.emotionId);
            if (emotion) {
                g.setColour(juce::Colours::white);
                g.setFont(9.0f);
