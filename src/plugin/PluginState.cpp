...
juce::ValueTree
PluginState::saveState(juce::AudioProcessorValueTreeState &apvts,
...
bool PluginState::loadState(juce::AudioProcessorValueTreeState &apvts,
  ...
  if (state.hasProperty("selectedEmotionId")) {
    selectedEmotionId =
        static_cast<int>(state.getProperty("selectedEmotionId"));
  } else {
...
void PluginState::saveEmotionSettings(
    juce::ValueTree &state, const CassetteState &cassetteState) const {
  // Save SideA
  auto sideATree = juce::ValueTree("SideA");
  sideATree.setProperty("description",
                        juce::String(cassetteState.sideA.description), nullptr);
  ...
  sideBTree.setProperty("description",
                        juce::String(cassetteState.sideB.description), nullptr);
...
CassetteState
PluginState::loadEmotionSettings(const juce::ValueTree &state) const {
  CassetteState cassetteState;

  // Load SideA
  auto sideATree = state.getChildWithName("SideA");
  if (sideATree.isValid()) {
    cassetteState.sideA.description =
        sideATree.getProperty("description").toString().toStdString();
    cassetteState.sideA.intensity =
        static_cast<float>(sideATree.getProperty("intensity"));
    if (sideATree.hasProperty("emotionId")) {
      cassetteState.sideA.emotionId =
          static_cast<int>(sideATree.getProperty("emotionId"));
    }
  }
  ...
  if (sideBTree.isValid()) {
    cassetteState.sideB.description =
        sideBTree.getProperty("description").toString().toStdString();
    cassetteState.sideB.intensity =
        static_cast<float>(sideBTree.getProperty("intensity"));
    if (sideBTree.hasProperty("emotionId")) {
      cassetteState.sideB.emotionId =
          static_cast<int>(sideBTree.getProperty("emotionId"));
    }
  }
...
bool PluginState::savePreset(const juce::String &presetName,
  ...
  Preset preset =
      createPresetFromState(presetName, description, author, apvts,
...
std::optional<PluginState::Preset>
PluginState::loadPreset(const juce::String &presetName) const {
  juce::File presetFile = getPresetFile(presetName);
  ...
  juce::var jsonData;
  juce::Result result =
      juce::JSON::parse(presetFile.loadFileAsString(), jsonData);
...
std::vector<juce::String> PluginState::getPresetNames() const {
  ...
  presetsDir.findChildFiles(presetFiles, juce::File::findFiles, false,
                            "*.json");
...
std::optional<PluginState::Preset>
PluginState::getPresetMetadata(const juce::String &presetName) const {
  // For now, just load the full preset (could be optimized to read only
  // metadata)
...
bool PluginState::applyPreset(const Preset &preset,
  ...
  setParameterValue(apvts, PluginProcessor::PARAM_COMPLEXITY,
                    preset.complexity);
  ...
  if (auto *barsParam = dynamic_cast<juce::AudioParameterInt *>(
          apvts.getParameter(PluginProcessor::PARAM_BARS))) {
    auto range = barsParam->getRange();
    int minValue = range.getStart();
    int maxValue = range.getEnd();
    float normalized = static_cast<float>(preset.bars - minValue) /
  ...
  if (auto *bypassParam = dynamic_cast<juce::AudioParameterBool *>(
          apvts.getParameter(PluginProcessor::PARAM_BYPASS))) {
...
PluginState::Preset PluginState::createPresetFromState(
    const juce::String &name, const juce::String &description,
    const juce::String &author, juce::AudioProcessorValueTreeState &apvts,
    const juce::String &woundDescription,
    const std::optional<int> &selectedEmotionId,
    const CassetteState &cassetteState) const {
  Preset preset;
  ...
  preset.complexity =
      getParameterValue(apvts, PluginProcessor::PARAM_COMPLEXITY);
  ...
  if (auto *barsParam = dynamic_cast<juce::AudioParameterInt *>(
    ...
    preset.bars =
        static_cast<int>(getParameterValue(apvts, PluginProcessor::PARAM_BARS));
  }
...
float PluginState::getParameterValue(juce::AudioProcessorValueTreeState &apvts,
...
void PluginState::setParameterValue(juce::AudioProcessorValueTreeState &apvts,
  ...
  if (param) {
    ...
    if (auto *floatParam = dynamic_cast<juce::AudioParameterFloat *>(param)) {
      ...
      float normalized =
          static_cast<float>(static_cast<int>(value) - minValue) /
          static_cast<float>(maxValue - minValue);
      param->setValueNotifyingHost(normalized);
    } else if (auto *boolParam =
                   dynamic_cast<juce::AudioParameterBool *>(param)) {
...
std::optional<PluginState::Preset>
PluginState::Preset::fromValueTree(const juce::ValueTree &tree) {
  ...
  if (tree.hasProperty("selectedEmotionId")) {
    preset.selectedEmotionId =
        static_cast<int>(tree.getProperty("selectedEmotionId"));
  }
  ...
  if (cassetteTree.isValid()) {
    ...
    if (sideATree.isValid()) {
      preset.cassetteState.sideA.description =
          sideATree.getProperty("description").toString().toStdString();
      preset.cassetteState.sideA.intensity =
          static_cast<float>(sideATree.getProperty("intensity"));
      if (sideATree.hasProperty("emotionId")) {
        preset.cassetteState.sideA.emotionId =
            static_cast<int>(sideATree.getProperty("emotionId"));
      }
    }
    ...
    if (sideBTree.isValid()) {
      preset.cassetteState.sideB.description =
          sideBTree.getProperty("description").toString().toStdString();
      preset.cassetteState.sideB.intensity =
          static_cast<float>(sideBTree.getProperty("intensity"));
      if (sideBTree.hasProperty("emotionId")) {
        preset.cassetteState.sideB.emotionId =
            static_cast<int>(sideBTree.getProperty("emotionId"));
      }
    }

    preset.cassetteState.isFlipped =
        static_cast<bool>(cassetteTree.getProperty("isFlipped"));
  }
...
std::optional<PluginState::Preset>
PluginState::Preset::fromJson(const juce::var &json) {
  if (!json.isObject()) {
    return std::nullopt;
  }
  ...
  if (obj->hasProperty("selectedEmotionId")) {
    preset.selectedEmotionId =
        static_cast<int>(obj->getProperty("selectedEmotionId"));
  }
      ...
      if (cassetteObj != nullptr) {
        if (cassetteObj->hasProperty("sideA")) {
          ...
          if (sideAVar.isObject()) {
            ...
            if (sideAObj != nullptr) {
              preset.cassetteState.sideA.description =
                  sideAObj->getProperty("description").toString().toStdString();
              preset.cassetteState.sideA.intensity =
                  static_cast<float>(sideAObj->getProperty("intensity"));
              if (sideAObj->hasProperty("emotionId")) {
                preset.cassetteState.sideA.emotionId =
                    static_cast<int>(sideAObj->getProperty("emotionId"));
              }
            }
          }
        }
        ...
        if (cassetteObj->hasProperty("sideB")) {
          ...
          if (sideBVar.isObject()) {
            ...
            if (sideBObj != nullptr) {
              preset.cassetteState.sideB.description =
                  sideBObj->getProperty("description").toString().toStdString();
              preset.cassetteState.sideB.intensity =
                  static_cast<float>(sideBObj->getProperty("intensity"));
              if (sideBObj->hasProperty("emotionId")) {
                preset.cassetteState.sideB.emotionId =
                    static_cast<int>(sideBObj->getProperty("emotionId"));
              }
            }
          }
        }
        ...
        preset.cassetteState.isFlipped =
            static_cast<bool>(cassetteObj->getProperty("isFlipped"));
      }
    }
  }
...
juce::File PluginState::autosaveFile() const {
  return getPresetsDirectory().getChildFile("autosave.json");
}

juce::File PluginState::autosaveBackupFile() const {
  return getPresetsDirectory().getChildFile("autosave.bak.json");
}

bool PluginState::writeAutosaveSnapshot(
    juce::AudioProcessorValueTreeState &apvts,
    const juce::String &woundDescription,
    const std::optional<int> &selectedEmotionId,
    const CassetteState &cassetteState) const {
  // Build the preset object (reuses the same shape as named presets so the
  // recovery path goes through fromJson()/applyPreset()).
  Preset snapshot = createPresetFromState(
      "__autosave__", "Automatic recovery snapshot", "KMiDi", apvts,
      woundDescription, selectedEmotionId, cassetteState);

  auto current = autosaveFile();
  auto backup = autosaveBackupFile();

  // Two-slot rotation. Move the existing snapshot aside before writing;
  // if the write fails partway, readLatestAutosaveSnapshot() can still
  // recover from the backup.
  if (current.existsAsFile()) {
    if (backup.existsAsFile()) {
      backup.deleteFile();
    }
    if (!current.moveFileTo(backup)) {
      // Rotation failed — abort rather than overwrite the only good
      // snapshot. The user will see the prior autosave on next launch.
      return false;
    }
  }

  auto json = snapshot.toJson();
  auto jsonText = juce::JSON::toString(json, true /*allOnOneLine*/);
  return current.replaceWithText(jsonText);
}

std::optional<PluginState::Preset>
PluginState::readLatestAutosaveSnapshot() const {
  auto tryLoad = [](const juce::File &f) -> std::optional<Preset> {
    if (!f.existsAsFile()) {
      return std::nullopt;
    }
    ...
    return Preset::fromJson(parsed);
  };

  if (auto p = tryLoad(autosaveFile())) {
    return p;
  }
  // Fallback: maybe the primary slot was mid-write at last crash.
  return tryLoad(autosaveBackupFile());
}

void PluginState::clearAutosaveJournal() const {
  auto current = autosaveFile();
  auto backup = autosaveBackupFile();
  if (current.existsAsFile()) {
    current.deleteFile();
