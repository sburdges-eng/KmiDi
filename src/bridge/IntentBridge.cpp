...
IntentBridge::IntentBridge()
    : bridge::PythonBridgeBase("IntentBridge"), cache_(CACHE_TTL_MS, 50) {}

IntentBridge::~IntentBridge() { shutdown(); }
...
std::string IntentBridge::processIntent(const std::string &intentJson) {
  ...
  auto intentStr = makePyOwned(PyUnicode_FromString(intentJson.c_str()));
  if (!intentStr) {
    return "{}";
  }
  PyTuple_SetItem(args.get(), 0, intentStr.release());

  auto result =
      makePyOwned(PyObject_CallObject(processIntentFunc_, args.get()));
...
std::string IntentBridge::convertToPythonIntent(const IntentResult &intent) {
  if (!isAvailable() || !convertToPythonFunc_) {
    ...
    oss << R"({"phase_1": {"mood_primary": ")"
        << intent.sourceWound.primaryEmotion.name
  ...
  intentJson << R"({"key": ")" << intent.key << R"(", "mode": ")" << intent.mode
             << R"(", "tempoBpm": )" << intent.tempoBpm << R"(, "emotion": ")"
             << intent.sourceWound.primaryEmotion.name << R"("})";
  ...
  auto cppIntentJson =
      makePyOwned(PyUnicode_FromString(intentJson.str().c_str()));
  if (!cppIntentJson) {
    return intentJson.str();
  }
  PyTuple_SetItem(args.get(), 0, cppIntentJson.release());

  auto result =
      makePyOwned(PyObject_CallObject(convertToPythonFunc_, args.get()));
...
bool IntentBridge::validateResult(const std::string &resultJson) {
  ...
  auto resultJsonObj = makePyOwned(PyUnicode_FromString(resultJson.c_str()));
  if (!resultJsonObj) {
    return false;
  }
  PyTuple_SetItem(args.get(), 0, resultJsonObj.release());

  auto result =
      makePyOwned(PyObject_CallObject(validateResultFunc_, args.get()));
  ...
  bool valid = PyObject_IsTrue(result.get());
  return valid;
#else
  return false;
#endif
...
std::string IntentBridge::getSuggestedRuleBreaks(const std::string &emotion) {
  ...
  auto emotionObj = makePyOwned(PyUnicode_FromString(emotion.c_str()));
  if (!emotionObj) {
    return R"({"rule_breaks": []})";
  }
  PyTuple_SetItem(args.get(), 0, emotionObj.release());

  auto result =
      makePyOwned(PyObject_CallObject(getRuleBreaksFunc_, args.get()));
...
IntentResult IntentBridge::parseIntentResult(const std::string &resultJson) {
  ...
  if (std::regex_search(resultJson, match, chordRegex)) {
    ...
    std::sregex_iterator iter(chordsStr.begin(), chordsStr.end(),
                              chordNameRegex);
