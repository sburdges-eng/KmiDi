...
PythonBridgeBase::~PythonBridgeBase() { shutdownPython(); }
...
bool PythonBridgeBase::ensurePythonStarted() {
  ...
  std::call_once(s_pyStartupOnce, []() {
    ...
    if (!Py_IsInitialized()) {
      std::cerr
          << "PythonBridgeBase: Failed to initialize Python interpreter\n";
      return;
    }
...
#else
  std::cerr << "PythonBridgeBase: Python not available (compiled without "
               "PYTHON_AVAILABLE)\n";
  return false;
#endif
}
...
PyObject *PythonBridgeBase::getFunction(PyObject *module,
...
std::string
PythonBridgeBase::callPythonFunction(PyObject *func,
  ...
  auto pyArgs = makePyOwned(PyTuple_New(static_cast<Py_ssize_t>(args.size())));
  if (!pyArgs) {
  ...
  for (size_t i = 0; i < args.size(); ++i) {
    auto argStr = makePyOwned(PyUnicode_FromString(args[i].c_str()));
    if (!argStr) {
      logError("Failed to create string argument " + std::to_string(i));
      return "";
    }
    PyTuple_SetItem(pyArgs.get(), static_cast<Py_ssize_t>(i), argStr.release());
  }

  // Call the function
  auto result = makePyOwned(PyObject_CallObject(func, pyArgs.get()));
  ...
  if (PyUnicode_Check(result.get())) {
    ...
    const char *cstr = PyBytes_AsString(result.get());
    resultStr = cstr ? cstr : "";
  } else {
    // Try to convert to string representation
    auto strObj = makePyOwned(PyObject_Str(result.get()));
    if (strObj) {
      const char *cstr = PyUnicode_AsUTF8(strObj.get());
      resultStr = cstr ? cstr : "";
    }
  }
