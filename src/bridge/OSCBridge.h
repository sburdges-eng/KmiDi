 ...
 */
using OSCStringResponseHandler =
 ...
 */
class OSCBridge : public BridgeBase,
...
public:
  ...
  bool initialize(const std::string &brainHost, int brainPort, int listenPort);
  void shutdown() override;

  // Legacy API (for backward compatibility with OSCClient)
  bool connect(const std::string &host = "127.0.0.1", int serverPort = 5005,
   ...
   */
  void requestGenerate(const std::string &text, const std::string &motivation,
   ...
   */
  void requestGenerate(const std::string &text, float motivation = 0.5f,
                       float chaos = 0.5f, float vulnerability = 0.5f,
   ...
   * @param callback Response handler
   */
  void requestAnalyzeChords(const std::string &progression,
                            OSCResponseHandler callback);
   ...
   * @param callback Response handler
   */
  void requestIntentProcess(const std::string &intentFile,
                            OSCResponseHandler callback);
   ...
   * @param callback Response handler
   */
  void requestIntentSuggest(const std::string &emotion,
                            OSCResponseHandler callback);
...
private:
  ...
  uint32_t generateMessageId() {
    return nextMessageId_.fetch_add(1, std::memory_order_relaxed);
  }
  ...
  /**
   * Remove a pending request, optionally returning its callbacks.
   */
  bool takePendingRequest(uint32_t msgId, PendingRequest *requestOut = nullptr);
