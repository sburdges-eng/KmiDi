 ...
 * For production: consider using a JSON library (nlohmann/json, rapidjson,
 * etc.)
 */
...
std::string ProjectFile::toJSON() const {
  auto root = makeDynamicObject();

  auto metadata = makeDynamicObject();
  ...
  metadata->setProperty("version", juce::String(metadata_.versionMajor) + "." +
                                       juce::String(metadata_.versionMinor));
  root->setProperty("metadata", juce::var(metadata));

  auto settings = makeDynamicObject();
  ...
  for (const auto &track : tracks_) {
    auto trackObj = makeDynamicObject();
    trackObj->setProperty("name", juce::String(track.name));
    trackObj->setProperty("type", track.type == TrackType::MIDI    ? "midi"
                                  : track.type == TrackType::Audio ? "audio"
                                                                   : "aux");
    ...
    if (track.type == TrackType::MIDI) {
      trackObj->setProperty("midiEvents",
                            static_cast<int>(track.midiSequence.size()));
...
bool ProjectFile::fromJSON(const std::string &json) {
  ...
  if (auto *metadata = metadataVar.getDynamicObject()) {
    ...
    metadata_.createdDate =
        metadata->getProperty("created").toString().toStdString();
    metadata_.modifiedDate =
        metadata->getProperty("modified").toString().toStdString();
  }
  ...
  if (auto *settings = settingsVar.getDynamicObject()) {
    ...
    auto timeSig =
        settings->getProperty("timeSignature").toString().toStdString();
    ...
    sampleRate_ = static_cast<SampleRate>(
        static_cast<int>(settings->getProperty("sampleRate")));
  }

  auto mixerVar = rootObj->getProperty("mixer");
  if (auto *mixer = mixerVar.getDynamicObject()) {
    mixer_.masterVolume =
        static_cast<float>(mixer->getProperty("masterVolume"));
