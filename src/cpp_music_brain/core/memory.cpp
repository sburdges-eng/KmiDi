...
void* MemoryPool::allocate() noexcept {
    ...
    if (freeCount_.load(std::memory_order_relaxed) == 0) return nullptr;
    size_t idx = freeCount_.fetch_sub(1, std::memory_order_acq_rel) - 1;
...
void MemoryPool::deallocate(void* ptr) noexcept {
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    size_t idx = freeCount_.fetch_add(1, std::memory_order_acq_rel);
    freeList_[idx] = ptr;
}
