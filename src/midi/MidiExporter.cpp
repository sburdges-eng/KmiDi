...
MidiExporter::MidiExporter() { clearError(); }
...
bool MidiExporter::exportToFileWithLyrics(
    const juce::File &file, const GeneratedMidi &midi,
    const std::vector<juce::String> &lyrics, const ExportOptions &options) {
  clearError();
  ...
  // Note: Since getTrack() returns const, we rebuild the file with lyrics
  // included
...
bool MidiExporter::exportToFileWithVocals(
    const juce::File &file, const GeneratedMidi &midi,
  ...
  if (options.includeVocals && !vocalNotes.empty()) {
    ...
    addVocalNotes(vocalTrack, vocalNotes, options.ticksPerQuarterNote,
                  MIDI_CHANNEL_MELODY);
...
juce::MidiFile
MidiExporter::buildMultiTrackFile(const GeneratedMidi &midi,
  ...
  float tempoBpm =
      midi.bpm > 0.0f ? midi.bpm : static_cast<float>(midi.tempoBpm);
  TimeSignature timeSig = {
      4, 4}; // Default, could be extracted from midi if available
  addTempoAndTimeSignature(metaTrack, tempoBpm, timeSig,
                           options.ticksPerQuarterNote);
  ...
  if (!midi.chords.empty()) {
    ...
    for (const auto &chord : midi.chords) {
      midiBuilder_.addChordToSequence(chordTrack, chord,
                                      options.ticksPerQuarterNote, channel,
                                      MIDI_VELOCITY_MEDIUM);
    }
  ...
  if (!midi.melody.empty()) {
    ...
    midiBuilder_.addNotesToSequence(melodyTrack, midi.melody,
                                    options.ticksPerQuarterNote, channel);
    if (options.includeExpression) {
  ...
  if (!midi.bass.empty()) {
    ...
    midiBuilder_.addNotesToSequence(bassTrack, midi.bass,
                                    options.ticksPerQuarterNote, channel);
    if (options.includeExpression) {
  ...
  if (!midi.counterMelody.empty()) {
    ...
    midiBuilder_.addNotesToSequence(counterMelodyTrack, midi.counterMelody,
                                    options.ticksPerQuarterNote, channel);
  ...
  if (!midi.pad.empty()) {
    ...
    midiBuilder_.addNotesToSequence(padTrack, midi.pad,
                                    options.ticksPerQuarterNote, channel);
  ...
  if (!midi.strings.empty()) {
    ...
    midiBuilder_.addNotesToSequence(stringsTrack, midi.strings,
                                    options.ticksPerQuarterNote, channel);
  ...
  if (!midi.fills.empty()) {
    ...
    midiBuilder_.addNotesToSequence(fillsTrack, midi.fills,
                                    options.ticksPerQuarterNote, channel);
  ...
  if (!midi.rhythm.empty()) {
    ...
    midiBuilder_.addNotesToSequence(rhythmTrack, midi.rhythm,
                                    options.ticksPerQuarterNote, channel);
  ...
  if (!midi.drumGroove.empty()) {
    ...
    midiBuilder_.addNotesToSequence(drumTrack, midi.drumGroove,
                                    options.ticksPerQuarterNote, channel);
...
juce::MidiFile
MidiExporter::buildSingleTrackFile(const GeneratedMidi &midi,
                                   const ExportOptions &options) const {
  juce::MidiFile file;
  file.setTicksPerQuarterNote(options.ticksPerQuarterNote);
  ...
  addTempoAndTimeSignature(mergedTrack, tempoBpm, timeSig,
                           options.ticksPerQuarterNote);
  ...
  if (!midi.chords.empty()) {
    int channel = midiBuilder_.getChannelForLayer("chords");
    for (const auto &chord : midi.chords) {
      midiBuilder_.addChordToSequence(mergedTrack, chord,
                                      options.ticksPerQuarterNote, channel,
                                      MIDI_VELOCITY_MEDIUM);
    }
  }
  ...
  if (!midi.melody.empty()) {
    int channel = midiBuilder_.getChannelForLayer("melody");
    midiBuilder_.addNotesToSequence(mergedTrack, midi.melody,
                                    options.ticksPerQuarterNote, channel);
  }
  ...
  if (!midi.bass.empty()) {
    int channel = midiBuilder_.getChannelForLayer("bass");
    midiBuilder_.addNotesToSequence(mergedTrack, midi.bass,
                                    options.ticksPerQuarterNote, channel);
  }
  ...
  if (!midi.counterMelody.empty()) {
    int channel = midiBuilder_.getChannelForLayer("counterMelody");
    midiBuilder_.addNotesToSequence(mergedTrack, midi.counterMelody,
                                    options.ticksPerQuarterNote, channel);
  }

  if (!midi.pad.empty()) {
    int channel = midiBuilder_.getChannelForLayer("pad");
    midiBuilder_.addNotesToSequence(mergedTrack, midi.pad,
                                    options.ticksPerQuarterNote, channel);
  }

  if (!midi.strings.empty()) {
    int channel = midiBuilder_.getChannelForLayer("strings");
    midiBuilder_.addNotesToSequence(mergedTrack, midi.strings,
                                    options.ticksPerQuarterNote, channel);
  }

  if (!midi.fills.empty()) {
    int channel = midiBuilder_.getChannelForLayer("fills");
    midiBuilder_.addNotesToSequence(mergedTrack, midi.fills,
                                    options.ticksPerQuarterNote, channel);
  }

  if (!midi.rhythm.empty()) {
    int channel = midiBuilder_.getChannelForLayer("rhythm");
    midiBuilder_.addNotesToSequence(mergedTrack, midi.rhythm,
                                    options.ticksPerQuarterNote, channel);
  }

  if (!midi.drumGroove.empty()) {
    int channel = MIDI_CHANNEL_DRUMS;
    midiBuilder_.addNotesToSequence(mergedTrack, midi.drumGroove,
                                    options.ticksPerQuarterNote, channel);
  }
...
void MidiExporter::addTempoAndTimeSignature(juce::MidiMessageSequence &sequence,
  ...
  // Time signature: 4/4
  sequence.addEvent(juce::MidiMessage::timeSignatureMetaEvent(
...
void MidiExporter::addLyricEvents(juce::MidiMessageSequence &sequence,
  ...
  for (size_t i = 0; i < lyrics.size(); ++i) {
    ...
    juce::MidiMessage lyricEvent =
        juce::MidiMessage::textMetaEvent(5, lyrics[i]);
...
void MidiExporter::addExpressionEvents(juce::MidiMessageSequence &sequence,
  ...
  double totalBeats =
      midi.lengthInBeats > 0.0 ? midi.lengthInBeats : (midi.bars * 4.0);

  // Add expression curve (simplified - linear increase)
  int numPoints = 8;
  for (int i = 0; i <= numPoints; ++i) {
    double beatPosition =
        (static_cast<double>(i) / static_cast<double>(numPoints)) * totalBeats;
    int tickPosition = static_cast<int>(beatPosition * ticksPerQuarterNote);
    ...
    int expressionValue =
        64 +
        static_cast<int>(
            (static_cast<double>(i) / static_cast<double>(numPoints)) * 63.0);
    ...
    for (int channel = 0; channel < 16; ++channel) {
      if (channel != 9) { // Skip drum channel
        juce::MidiMessage ccEvent = juce::MidiMessage::controllerEvent(
            channel + 1, 11, expressionValue);
...
void MidiExporter::addVocalNotes(juce::MidiMessageSequence &sequence,
                                 const std::vector<MidiNote> &vocalNotes,
                                 int ticksPerQuarterNote, int channel) const {
  for (const auto &note : vocalNotes) {
    ...
    sequence.addEvent(
        juce::MidiMessage::noteOn(channel + 1, pitch,
