...
inline void assertIRValid(const IntentFrame& frame) {
    ...
    INTENT_IR_ASSERT(
        frame.emotion.valence >= -1.0f && frame.emotion.valence <= 1.0f,
        "Valence out of range"
    );
    
    INTENT_IR_ASSERT(
        frame.emotion.arousal >= 0.0f && frame.emotion.arousal <= 1.0f,
        "Arousal out of range"
    );
    
    INTENT_IR_ASSERT(
        frame.emotion.dominance >= 0.0f && frame.emotion.dominance <= 1.0f,
        "Dominance out of range"
    );
    
    INTENT_IR_ASSERT(
        frame.music.tempo_bias >= -1.0f && frame.music.tempo_bias <= 1.0f,
...
inline void assertProvenanceValid(const IntentProvenance& provenance) {
    INTENT_IR_ASSERT(
        provenance.user_override_weight >= 0.0f && provenance.user_override_weight <= 1.0f,
