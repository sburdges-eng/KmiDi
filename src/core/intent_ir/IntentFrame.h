 ...
 *
 * This header provides C struct definitions matching the Rust Intent IR v1
 * spec. These structs are used for FFI communication and audio-thread-safe
 * access.
 *
 * Rules:
 * - No allocation
...
struct IntentProvenance {
  ...
  float user_override_weight; // [0.0, 1.0] - 0.0 = ML dominates, 1.0 = user
                              // dominates
};
...
inline void getVAD(const EmotionState &emotion, float &valence, float &arousal,
                   float &dominance) {
...
inline bool isImmediate(const TimeScope &time) { return time.start_bar == -1; }

// Check if time scope is open-ended
inline bool isOpenEnded(const TimeScope &time) { return time.end_bar == -1; }

// Check if engine is allowed
inline bool isEngineAllowed(const IntentConstraints &constraints,
