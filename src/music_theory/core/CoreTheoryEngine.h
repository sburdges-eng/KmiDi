...
public:
   ...
   */
  Interval
  calculateInterval(int semitones,
   ...
   */
  Interval
  calculateInterval(int midiNote1, int midiNote2,
                    TuningSystem tuning = TuningSystem::TwelveTET) const;
   ...
   */
  Scale generateScale(int rootNote, const std::vector<int> &pattern,
                      TuningSystem tuning = TuningSystem::TwelveTET) const;
  ...
  Scale generateMode(int rootNote,
                     int modeNumber) const; // 1=Ionian, 2=Dorian, etc.
   ...
   */
  int modulationDistance(const std::string &fromKey,
