...
CoreTheoryEngine::CoreTheoryEngine() { initializePatternDatabase(); }

//==============================================================================
// Interval Mathematics
//==============================================================================

Interval CoreTheoryEngine::calculateInterval(int semitones,
...
float CoreTheoryEngine::getFrequencyRatio(int semitones,
                                          TuningSystem tuning) const {
  switch (tuning) {
  ...
  case TuningSystem::JustIntonation:
    ...
    switch (semitones % 12) {
    case 0:
      return 1.0f; // Unison (1:1)
    case 1:
      return 16.0f / 15.0f; // Minor second
    case 2:
      return 9.0f / 8.0f; // Major second
    case 3:
      return 6.0f / 5.0f; // Minor third
    case 4:
      return 5.0f / 4.0f; // Major third
    case 5:
      return 4.0f / 3.0f; // Perfect fourth
    case 6:
      return 45.0f / 32.0f; // Tritone
    case 7:
      return 3.0f / 2.0f; // Perfect fifth (!)
    case 8:
      return 8.0f / 5.0f; // Minor sixth
    case 9:
      return 5.0f / 3.0f; // Major sixth
    case 10:
      return 9.0f / 5.0f; // Minor seventh
    case 11:
      return 15.0f / 8.0f; // Major seventh
    default:
      return 2.0f; // Octave
    }
  ...
  case TuningSystem::Pythagorean:
    ...
    return std::pow(3.0f / 2.0f, (semitones * 7) % 12) /
           std::pow(2.0f, (semitones * 7) / 12);
...
float CoreTheoryEngine::calculateConsonance(int semitones,
                                            TuningSystem tuning) const {
  // Consonance based on:
  ...
  if (simple == 1 || simple == 2 || simple == 6 || simple == 10 ||
      simple == 11) {
    return 0.2f;
  }
...
std::string CoreTheoryEngine::explainInterval(int semitones,
  ...
  case ExplanationDepth::Intermediate:
    explanation << "A " << getIntervalName(simple) << " spans " << semitones
                << " semitones.\n\n"
                << "Consonance: "
                << (calculateConsonance(simple, TuningSystem::TwelveTET) * 100)
                << "%\n"
  ...
  case ExplanationDepth::Advanced:
    explanation << "A " << getIntervalName(simple) << " (Perfect Fifth)\n\n"
                << "Acoustic properties:\n"
                << "- Frequency ratio: "
                << getFrequencyRatio(simple, TuningSystem::JustIntonation)
                << " (Just) vs "
                << getFrequencyRatio(simple, TuningSystem::TwelveTET)
                << " (12-TET)\n"
                << "- Waveforms align every 3 cycles (top note) and 2 cycles "
                   "(bottom note)\n"
  ...
  case ExplanationDepth::Expert:
    explanation << "Perfect Fifth - Deep Analysis\n\n"
                << "Mathematics:\n"
                << "- 12-TET: 2^(7/12) = "
                << getFrequencyRatio(7, TuningSystem::TwelveTET) << "\n"
                << "- Just: 3:2 = 1.500 (pure)\n"
                << "- Cents difference: "
                << (1200.0 * std::log2(1.5 / getFrequencyRatio(
                                                 7, TuningSystem::TwelveTET)))
                << " cents\n\n"
...
Scale CoreTheoryEngine::generateScale(int rootNote,
...
Scale CoreTheoryEngine::generatePentatonicScale(int rootNote,
                                                bool major) const {
  if (major) {
...
std::vector<int> CoreTheoryEngine::getScaleNotes(const Scale &scale,
...
void CoreTheoryEngine::initializeCircleOfFifths() const {
  circleOfFifths_.majorKeys = {"C",     "G",  "D",  "A",  "E",  "B",
                               "F#/Gb", "Db", "Ab", "Eb", "Bb", "F"};

  circleOfFifths_.minorKeys = {"Am",      "Em",  "Bm", "F#m", "C#m", "G#m",
                               "D#m/Ebm", "Bbm", "Fm", "Cm",  "Gm",  "Dm"};
...
CoreTheoryEngine::suggestModulations(const std::string &currentKey,
...
double CoreTheoryEngine::getFrequency(int midiNote, TuningSystem tuning,
                                      double concertA) const {
  switch (tuning) {
  case TuningSystem::TwelveTET:
...
double CoreTheoryEngine::calculateFrequency12TET(int midiNote,
                                                 double concertA) const {
  // A4 = MIDI note 69
  return concertA * std::pow(TWELVE_TET_RATIO, midiNote - 69);
}

double CoreTheoryEngine::calculateFrequencyJust(int midiNote,
                                                double concertA) const {
...
double CoreTheoryEngine::calculateFrequencyPythagorean(int midiNote,
                                                       double concertA) const {
...
std::string CoreTheoryEngine::midiToNoteName(int midiNote,
...
std::string CoreTheoryEngine::generateAcousticExplanation(int semitones) const {
  ...
  switch (semitones) {
  case 0:
    return "Unison - same frequency, perfect alignment.";
  case 7:
    return "Perfect Fifth - 3:2 ratio. Waveforms align every 3:2 cycles.";
  case 5:
    return "Perfect Fourth - 4:3 ratio. Strong consonance.";
  case 4:
    return "Major Third - 5:4 ratio. Imperfect but pleasant consonance.";
  case 3:
    return "Minor Third - 6:5 ratio. Darker color than major third.";
  default:
    return "Frequency ratio: " + std::to_string(getFrequencyRatio(
                                     semitones, TuningSystem::JustIntonation));
  }
}
...
std::string CoreTheoryEngine::generatePerceptualEffect(int semitones) const {
  switch (semitones) {
  case 0:
    return "Identity, unity";
  case 1:
    return "Harsh, dissonant, tense";
  case 2:
    return "Whole step, gentle dissonance";
  case 3:
    return "Minor third, melancholic";
  case 4:
    return "Major third, bright, happy";
  case 5:
    return "Perfect fourth, stable, open";
  case 6:
    return "Tritone, unstable, needs resolution";
  case 7:
    return "Perfect fifth, powerful, hollow";
  case 8:
    return "Minor sixth, bittersweet";
  case 9:
    return "Major sixth, warm, expansive";
  case 10:
    return "Minor seventh, bluesy, unresolved";
  case 11:
    return "Major seventh, jazzy, sophisticated";
  case 12:
    return "Octave, completion, identity";
  default:
...
std::string CoreTheoryEngine::generateHistoricalUsage(int semitones) const {
  switch (semitones) {
  case 7:
    return "Medieval organum (parallel fifths). Power chords in rock. "
           "Dominant-tonic in classical harmony.";
  case 4:
    return "Renaissance sweet sound. Beatles melodies. Pop music foundation.";
  case 6:
    return "Diabolus in musica (devil in music) in medieval times. Jazz "
           "tritone substitution. Wagner's instability.";
  default:
    return "Used across all eras of Western music.";
  }
}
...
float CoreTheoryEngine::calculateHarmonicPosition(int semitones) const {
  ...
  switch (semitones % 12) {
  case 0:
    return 1.0f; // Fundamental
  case 12:
    return 2.0f; // 2nd harmonic (octave)
  case 7:
    return 3.0f; // 3rd harmonic (P5 + octave)
  case 4:
    return 5.0f; // 5th harmonic (M3 + 2 octaves)
  default:
    return 0.0f; // Not directly in series
  }
}

std::string
CoreTheoryEngine::identifyScalePattern(const std::vector<int> &pattern) const {
  if (pattern == std::vector<int>{2, 2, 1, 2, 2, 2, 1})
    return "Major (Ionian)";
  if (pattern == std::vector<int>{2, 1, 2, 2, 1, 2, 2})
    return "Natural Minor (Aeolian)";
  if (pattern == std::vector<int>{2, 1, 2, 2, 2, 1, 2})
    return "Dorian";
  if (pattern == std::vector<int>{1, 2, 2, 2, 1, 2, 2})
    return "Phrygian";
  if (pattern == std::vector<int>{2, 2, 2, 1, 2, 2, 1})
    return "Lydian";
  if (pattern == std::vector<int>{2, 2, 1, 2, 2, 1, 2})
    return "Mixolydian";
  if (pattern == std::vector<int>{1, 2, 2, 1, 2, 2, 2})
    return "Locrian";
  if (pattern == std::vector<int>{2, 2, 3, 2, 3})
    return "Major Pentatonic";
  if (pattern == std::vector<int>{3, 2, 2, 3, 2})
    return "Minor Pentatonic";
  if (pattern == std::vector<int>{3, 2, 1, 1, 3, 2})
    return "Blues";
  return "Custom";
}

std::optional<CoreTheoryEngine::ParsedNote>
CoreTheoryEngine::parseNoteName(const std::string &noteName) const {
