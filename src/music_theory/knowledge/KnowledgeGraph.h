...
public:
  ...
  getConceptsByCategory(const std::string &category) const;
  ...
  getPrerequisites(const std::string &conceptName) const;
  ...
  struct ConceptRelationship {
    ...
    std::string
        relationshipType; // "Prerequisite", "Related", "Opposite", "Example"
    float strength;       // 0-1 (how strong the relationship)
  };

  std::vector<ConceptRelationship>
  getRelatedConcepts(const std::string &conceptName, int maxDepth = 1) const;
   ...
   */
  std::vector<std::string>
  getApplications(const std::string &conceptName) const;
   ...
   */
  std::vector<std::string>
  findDependencies(const std::string &conceptName) const;
   ...
   */
  bool hasPrerequisites(const std::string &conceptName,
   ...
   */
  std::vector<std::string>
  getMissingPrerequisites(const std::string &conceptName,
                          const std::set<std::string> &masteredConcepts) const;
  ...
  std::vector<LearningPathStep>
  getLearningPath(const std::string &fromConcept,
  ...
  Curriculum generateCurriculum(const UserProfile &userProfile,
                                const std::vector<std::string> &targetConcepts,
                                int maxLessons = 20) const;
  ...
  std::vector<ConceptSuggestion>
  suggestNextConcepts(const std::set<std::string> &masteredConcepts,
                      int numSuggestions = 5) const;
   ...
   */
  std::vector<std::string>
  getOptimalLearningOrder(const std::vector<std::string> &concepts) const;
   ...
   * @param type Explanation type (Intuitive, Acoustic, Mathematical,
   * Historical)
   ...
   */
  std::string explainConcept(const std::string &conceptName,
   ...
   */
  std::vector<ExplanationType>
  getAvailableExplanationTypes(const std::string &conceptName) const;
  ...
  getMusicalExamples(const std::string &conceptName, int maxExamples = 5) const;
   ...
   */
  std::string explainRelationship(const std::string &concept1Name,
                                  const std::string &concept2Name) const;
  ...
  std::vector<ConceptMatch>
  findConceptByDescription(const std::string &description) const;
  ...
  std::vector<IdentifiedConcept>
  identifyConceptsFromMIDI(const std::vector<int> &midiNotes,
   ...
   */
  Exercise generateExercise(const std::string &conceptName,
   ...
   */
  Feedback validateAnswer(const Exercise &exercise,
   ...
   */
  float updateMastery(const std::string &conceptName, bool correct,
                      float timeSeconds, UserProfile &userProfile) const;
   ...
   */
  std::vector<std::string>
  identifyStrugglingConcepts(const UserProfile &userProfile) const;
   ...
   */
  std::vector<std::string> suggestReview(const UserProfile &userProfile) const;
   ...
   * Save user profile
   */
  bool saveUserProfile(const std::string &filePath,
...
private:
  ...
  // Graph algorithms
  std::vector<std::string>
  topologicalSort(const std::string &startConcept) const;

  std::vector<std::string> breadthFirstSearch(const std::string &start,
                                              const std::string &goal) const;

  std::vector<std::string>
  depthFirstSearch(const std::string &start,
  ...
  // Cycle detection
  bool hasCycleDFS(const std::string &node, std::set<std::string> &visited,
                   std::set<std::string> &recursionStack) const;

  // Path finding
  std::vector<std::string> findShortestPath(const std::string &from,
                                            const std::string &to) const;

  // Relationship scoring
  float calculateRelationshipStrength(const std::string &concept1Name,
                                      const std::string &concept2Name) const;

  // Concept matching
  float calculateConceptSimilarity(const std::string &description,
                                   const KnowledgeNode &conceptNode) const;

  // Learning path optimization
  int estimateLearningTime(const std::string &conceptName) const;

  std::vector<std::string>
  optimizePathOrder(const std::vector<std::string> &concepts) const;

  // Mastery calculations
  float calculateMasteryDecay(float currentMastery,
                              float daysSinceLastPractice) const;

  float calculateNewMastery(float currentMastery, bool correct,
                            float responseTime, int attemptCount) const;
