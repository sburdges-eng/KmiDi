    ...
    const std::string& category) const
{
    ...
    if (it != categoryIndex_.end()) {
        for (const auto& conceptName : it->second) {
            auto conceptNode = getConcept(conceptName);
            if (conceptNode) {
                results.push_back(*conceptNode);
            }
        }
    }
    ...
    const std::string& conceptName) const
{
    auto conceptNode = getConcept(conceptName);
    if (conceptNode) {
        return conceptNode->prerequisites;
    }
    return {};
}
    ...
    int maxDepth) const
{
    ...
    auto conceptNode = getConcept(conceptName);
    if (!conceptNode) return relationships;

    // Direct relationships
    for (const auto& related : conceptNode->relatedConcepts) {
    ...
    for (const auto& prereq : conceptNode->prerequisites) {
        ConceptRelationship rel;
    ...
    const std::string& conceptName) const
{
    auto conceptNode = getConcept(conceptName);
    if (conceptNode) {
        return conceptNode->applications;
    }
    return {};
}
    ...
    int maxLessons) const
{
    ...
    for (const auto& [conceptName, masteryLevel] : userProfile.conceptMastery) {
        if (masteryLevel > 0.7f) {
            mastered.insert(conceptName);
        }
    }
    ...
    for (size_t i = 4; i < curriculum.lessons.size(); i += 5) {
        curriculum.milestones.push_back(curriculum.lessons[i].conceptName);
    }
    ...
    ExplanationDepth depth) const
{
    ...
    auto it = conceptNode->explanations.find(type);
    if (it != conceptNode->explanations.end()) {
        return it->second;
    }

    // Default explanation
    return "The concept of " + conceptName + " in the " + conceptNode->category + " category.";
}
    ...
    const std::string& conceptName) const
{
    ...
    auto conceptNode = getConcept(conceptName);
    if (conceptNode) {
        for (const auto& [type, _] : conceptNode->explanations) {
    ...
    int maxExamples) const
{
    ...
    auto examples = conceptNode->examples;
    ...
    const std::vector<float>& onsetTimes) const
{
    ...
    if (midiNotes.size() >= 2) {
        ...
        IdentifiedConcept conceptItem;
        conceptItem.category = "Interval";
        conceptItem.relevantNotes = {midiNotes[0], midiNotes[1]};

        if (interval == 7) {
            conceptItem.conceptName = "Perfect Fifth";
            conceptItem.evidence = "7 semitones between notes";
        } else if (interval == 4) {
            conceptItem.conceptName = "Major Third";
            conceptItem.evidence = "4 semitones between notes";
        } else if (interval == 3) {
            conceptItem.conceptName = "Minor Third";
            conceptItem.evidence = "3 semitones between notes";
        }

        if (!conceptItem.conceptName.empty()) {
            identified.push_back(conceptItem);
        }
    }
    ...
    if (midiNotes.size() >= 3) {
        IdentifiedConcept conceptItem;
        conceptItem.category = "Chord";
        conceptItem.relevantNotes = {midiNotes[0], midiNotes[1], midiNotes[2]};
        ...
        if (intervals.size() >= 2) {
            if (intervals[0] == 4 && intervals[1] == 7) {
                conceptItem.conceptName = "Major Triad";
                conceptItem.evidence = "Major third + Perfect fifth structure";
            } else if (intervals[0] == 3 && intervals[1] == 7) {
                conceptItem.conceptName = "Minor Triad";
                conceptItem.evidence = "Minor third + Perfect fifth structure";
            }
        }
    ...
    DifficultyLevel difficultyLevel) const
{
    ...
    if (conceptNode->category == "Interval") {
        return generateIntervalExercise(difficultyLevel);
    } else if (conceptNode->category == "Chord") {
        return generateChordExercise(difficultyLevel);
    } else if (conceptNode->category == "Progression") {
        return generateProgressionExercise(difficultyLevel);
    } else if (conceptNode->category == "Rhythm") {
    ...
    const UserProfile& userProfile) const
{
    ...
    if (feedback.correct) {
        ...
        // Suggest review
        feedback.suggestedReview.push_back(exercise.conceptName);

        // Check prerequisites
        auto prereqs = getPrerequisites(exercise.conceptName);
        for (const auto& prereq : prereqs) {
    ...
    const UserProfile& userProfile) const
{
    ...
    for (const auto& [conceptName, mastery] : userProfile.conceptMastery) {
        if (mastery < 0.4f) {
            struggling.push_back(conceptName);
        }
    }
    ...
    const UserProfile& userProfile) const
{
    ...
    for (const auto& [conceptName, mastery] : userProfile.conceptMastery) {
        report << "  " << conceptName << ": " << (mastery * 100) << "%\n";
    }

    report << "\nStruggling Concepts:\n";
    for (const auto& conceptName : userProfile.strugglingConcepts) {
        report << "  - " << conceptName << "\n";
    }
    ...
    const std::string& startConcept) const
{
    ...
    std::function<void(const std::string&)> dfs = [&](const std::string& conceptName) {
        visited.insert(conceptName);

        auto it = prerequisiteGraph_.find(conceptName);
    ...
    const KnowledgeNode& conceptNode) const
{
    ...
    auto conceptTokens = tokenize(conceptNode.conceptName);
    ...
    const std::vector<std::string>& concepts) const
{
    ...
    for (const auto& conceptName : concepts) {
        if (processed.find(conceptName) != processed.end()) continue;

        // Get all dependencies
        auto deps = findDependencies(conceptName);
        ...
        ordered.push_back(conceptName);
        processed.insert(conceptName);
    }
...
Exercise KnowledgeGraph::generateProgressionExercise(DifficultyLevel level) const {
    ...
    exercise.level = level;
    exercise.conceptName = "Chord Progression";
