...
Chord HarmonyEngine::buildChord(int rootNote, ChordQuality quality,
                                VoicingType voicing) const {
  Chord chord;
  ...
  case ChordQuality::Major:
    ...
    chord.symbol = coreTheory_->midiToNoteName(rootNote).substr(
        0, coreTheory_->midiToNoteName(rootNote).length() - 1);
    break;
...
Chord HarmonyEngine::analyzeChord(const std::vector<int> &notes) const {
  ...
  for (int note : normalizedNotes) {
    int pc = note % 12;
    if (std::find(pitchClasses.begin(), pitchClasses.end(), pc) ==
        pitchClasses.end()) {
...
HarmonyEngine::identifyExtensions(const std::vector<int> &notes) const {
...
std::vector<Chord> HarmonyEngine::getAllVoicings(int rootNote,
                                                 ChordQuality quality) const {
  std::vector<Chord> voicings;

  // Generate all voicing types
  std::vector<VoicingType> types = {VoicingType::Close, VoicingType::Open,
                                    VoicingType::Drop2, VoicingType::Drop3};
...
HarmonyEngine::ProgressionAnalysis
HarmonyEngine::analyzeProgression(const std::vector<Chord> &chords,
  ...
  for (size_t i = 0; i < analysis.functions.size(); ++i) {
    if (analysis.functions[i] == "Tonic")
      funcStream << "T";
    else if (analysis.functions[i] == "Dominant")
      funcStream << "D";
    else if (analysis.functions[i] == "Subdominant")
      funcStream << "S";

    if (i < analysis.functions.size() - 1)
      funcStream << "-";
  }
  ...
  if (chords.size() >= 2) {
    auto cadence =
        detectCadence(chords[chords.size() - 2], chords[chords.size() - 1]);
  ...
  for (size_t i = 0; i < analysis.romanNumerals.size(); ++i) {
    ...
    if (i < analysis.romanNumerals.size() - 1)
      explain << " - ";
  }
...
HarmonyEngine::Cadence HarmonyEngine::detectCadence(const Chord &penultimate,
  ...
  if (tension1 > 0.7f && tension2 < 0.3f) {
    ...
    cadence.explanation =
        "Strong dominant to tonic motion creates complete resolution";
...
std::string HarmonyEngine::getHarmonicFunction(const Chord &chord,
                                               const std::string &key) const {
  int degree = getScaleDegree(chord.rootNote, key);
  return getFunctionFromDegree(degree);
}

std::string HarmonyEngine::getRomanNumeral(const Chord &chord,
                                           const std::string &key) const {
  int degree = getScaleDegree(chord.rootNote, key);
...
float HarmonyEngine::calculateTension(const Chord &chord,
                                      const std::string &key) const {
  int degree = getScaleDegree(chord.rootNote, key);
...
HarmonyEngine::suggestVoiceLeading(const Chord &fromChord, const Chord &toChord,
...
std::vector<HarmonyEngine::VoiceLeadingError>
HarmonyEngine::checkVoiceLeading(const std::vector<std::vector<int>> &voices,
  ...
  for (size_t i = 0; i < voices.size() - 1; ++i) {
    for (size_t j = i + 1; j < voices.size(); ++j) {
      ...
      for (size_t chord = 0; chord < voices[i].size() - 1; ++chord) {
        ...
        int interval2 =
            std::abs(voices[i][chord + 1] - voices[j][chord + 1]) % 12;
...
std::vector<int> HarmonyEngine::findCommonTones(const Chord &chord1,
                                                const Chord &chord2) const {
...
float HarmonyEngine::calculateSmoothness(
...
HarmonyEngine::suggestSubstitutions(const Chord &chord, const std::string &key,
  ...
  if (type == SubstitutionType::TritoneSub || type == SubstitutionType::All) {
    if (chord.quality == ChordQuality::Dominant7) {
      ...
      sub.explanation =
          "Tritone substitution: shares tritone with original dominant";
  ...
  if (type == SubstitutionType::Secondary || type == SubstitutionType::All) {
    ...
    sub.explanation =
        "Secondary dominant: temporarily tonicizes the target chord";
...
std::vector<Chord>
HarmonyEngine::getBorrowedChords(const std::string &key) const {
...
ChordProgression
HarmonyEngine::reharmonizeMelody(const std::vector<int> &melody,
...
ChordProgression HarmonyEngine::generateProgressionForEmotion(
    const std::string &emotion, const std::string &key, int numChords,
...
HarmonyEngine::EmotionalHarmony
HarmonyEngine::getEmotionalHarmony(const std::string &emotion) const {
  EmotionalHarmony eh;
...
ChordProgression
HarmonyEngine::getCommonProgression(const std::string &name,
                                    const std::string &key) const {
  ChordProgression progression;
  progression.key = key;
...
std::vector<ChordProgression>
HarmonyEngine::findSimilarProgressions(const ChordProgression &progression,
  ...
  for (const auto &tmpl : progressionDatabase_) {
    ...
    float similarity =
        ...
        std::max(progression.romanNumerals.size(), tmpl.romanNumerals.size());
...
ChordProgression
HarmonyEngine::transposeProgression(const ChordProgression &progression,
                                    int semitones) const {
...
std::vector<int>
HarmonyEngine::chordSymbolToMidi(const std::string &chordSymbol,
                                 int octave) const {
  // Simplified parser
  // Parse root note
...
std::string
HarmonyEngine::midiToChordSymbol(const std::vector<int> &notes) const {
  auto chord = analyzeChord(notes);
  return chord.symbol;
}

std::string
HarmonyEngine::explainProgression(const ChordProgression &progression,
  ...
  case ExplanationDepth::Simple:
    explanation << "A chord progression that moves from "
                << progression.romanNumerals.front() << " to "
                << progression.romanNumerals.back();
    break;
...
void HarmonyEngine::initializeVoiceLeadingRules() {
  voiceLeadingRules_ = {
      ...
      {"No parallel octaves", VoiceLeadingStyle::Bach, true,
       "Parallel octaves reduce to single voice", "Common in unison doubling"},
      {"Resolve tendency tones", VoiceLeadingStyle::Bach, false,
       "Leading tone up, 7th down", "Can be suspended for effect"},
      {"Smooth voice leading preferred", VoiceLeadingStyle::Jazz, false,
       "Prefer stepwise motion", "Leaps okay for dramatic effect"}};
}
...
ChordQuality
HarmonyEngine::identifyQuality(const std::vector<int> &intervals) const {
  if (intervals.empty())
...
std::vector<int>
HarmonyEngine::getIntervalsFromRoot(const std::vector<int> &notes,
                                    int root) const {
  std::vector<int> intervals;
  int rootPC = root % 12;
...
bool HarmonyEngine::hasParallelMotion(int voice1, int voice2,
...
float HarmonyEngine::calculateTensionFromFunction(
...
std::vector<std::string>
HarmonyEngine::selectChordQualities(const std::string &emotion) const {
  auto it = emotionMappings_.find(emotion);
  if (it != emotionMappings_.end()) {
...
std::vector<std::string>
HarmonyEngine::selectProgressionPattern(const std::string &emotion) const {
  auto it = emotionMappings_.find(emotion);
  if (it != emotionMappings_.end()) {
...
float HarmonyEngine::getEmotionalTensionTarget(
...
std::vector<Chord>
HarmonyEngine::getDiatonicSubstitutes(const Chord &chord,
                                      const std::string &key) const {
...
std::vector<Chord>
HarmonyEngine::getModalInterchange(const Chord &chord,
                                   const std::string &key) const {
  std::vector<Chord> borrowed;
