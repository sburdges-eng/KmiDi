...
public:
  ...
  std::vector<TimeSignatureSuggestion>
  suggestTimeSignatureForEmotion(const std::string &emotion) const;
   ...
   */
  GrooveAnalysis analyzeGroove(const std::vector<float> &onsetTimes,
   ...
   */
  std::vector<float> quantizeWithGroove(const std::vector<float> &onsetTimes,
                                        float quantizeStrength = 0.8f,
                                        bool preserveSwing = true) const;
   ...
   * @return Swing ratio (1.0 = straight, 2.0 = triplet swing, 1.5 = medium
   * swing)
   *
   * Example:
   ...
   */
  std::vector<float>
  analyzeMicroTiming(const std::vector<float> &onsetTimes,
  ...
  PolyrhythmAnalysis
  detectPolyrhythm(const std::vector<std::vector<float>> &layers) const;
  ...
  PolymeterAnalysis
  detectPolymeter(const std::vector<std::vector<float>> &layers,
                  float duration) const;
   ...
   */
  float
  calculateRhythmicTension(const std::vector<std::vector<float>> &layers) const;

  /**
   * Find meeting points (where polyrhythms align)
   */
  std::vector<float> findMeetingPoints(const std::vector<int> &ratios,
                                       float duration) const;
   ...
   */
  RhythmicPattern generatePatternForEmotion(const std::string &emotion,
   ...
   */
  RhythmicPattern
  generatePatternFromDescription(const std::string &description,
                                 const TimeSignature &timeSignature,
                                 int numBars = 4) const;
   ...
   */
  RhythmicPattern addSyncopation(const RhythmicPattern &pattern,
   ...
   */
  RhythmicPattern applySwing(const RhythmicPattern &pattern,
   ...
   */
  RhythmicPattern
  humanizePattern(const RhythmicPattern &pattern, float humanization = 0.3f,
                  const std::string &pocketStyle = "On beat") const;
  ...
  std::vector<RhythmicMotif> detectMotifs(const std::vector<float> &onsetTimes,
                                          float minOccurrences = 2) const;
  ...
  detectClavePattern(const std::vector<float> &onsetTimes) const;
   ...
   */
  std::vector<float>
  analyzeBeatStrength(const std::vector<float> &onsetTimes,
                      const std::vector<int> &velocities,
                      const TimeSignature &timeSignature) const;
  ...
  std::vector<TempoChange>
  detectTempoChanges(const std::vector<float> &onsetTimes) const;

  /**
   * Explain why a rhythm feels the way it does
   */
  std::string explainRhythmicFeel(const RhythmicPattern &pattern,
                                  ExplanationDepth depth) const;
   ...
   */
  RhythmicPattern getCommonPattern(const std::string &name,
                                   const TimeSignature &timeSignature) const;
  ...
  findSimilarPatterns(const RhythmicPattern &pattern,
   ...
   */
  std::vector<float> quantizeToGrid(const std::vector<float> &onsetTimes,
                                    int subdivision = 16) const;
  ...
  detectTimeSignatureChanges(const std::vector<float> &onsetTimes,
                             float duration) const;
...
private:
  ...
  // Pattern matching
  float calculatePatternSimilarity(const std::vector<float> &pattern1,
                                   const std::vector<float> &pattern2) const;

  // Swing detection
  std::vector<float> findSwingPairs(const std::vector<float> &onsetTimes,
                                    int subdivision) const;

  // Beat detection
  std::vector<float> detectBeats(const std::vector<float> &onsetTimes,
                                 const TimeSignature &timeSignature) const;
