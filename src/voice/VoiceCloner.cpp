...
VoiceCloner::FormantProfile
VoiceCloner::analyzeVoice(const std::vector<float> &audioSamples,
                          double sampleRate, float pitchEstimate) {
  ...
  for (size_t i = 0; i < window.size(); ++i) {
    float hann = 0.5f * (1.0f - std::cos(2.0f * static_cast<float>(M_PI) * i /
                                         (window.size() - 1)));
    window[i] *= hann;
  }
  ...
  float pitch = pitchEstimate > 0.0f ? pitchEstimate
                                     : estimatePitch(preEmphasized, sampleRate);
  if (pitch > 0.0f) {
    // Analyze spectral envelope (simplified)
    profile.glottalShape =
        0.5f; // Default, could be refined with spectral analysis
  }
...
VoiceCloner::extractFormants(const std::vector<float> &audioSamples,
                             double sampleRate) {
  auto profile = analyzeVoice(audioSamples, sampleRate);
  return {profile.frequencies, profile.bandwidths};
}

bool VoiceCloner::saveProfile(const FormantProfile &profile,
...
VoiceCloner::FormantProfile
VoiceCloner::loadProfile(const std::string &filePath) {
  FormantProfile profile;
  ...
  if (auto *obj = var.getDynamicObject()) {
    ...
    auto formantShiftProp = obj->getProperty("formantShift");
    profile.formantShift =
        formantShiftProp.isVoid() ? 1.0f : static_cast<float>(formantShiftProp);
    auto glottalShapeProp = obj->getProperty("glottalShape");
    profile.glottalShape =
        glottalShapeProp.isVoid() ? 0.5f : static_cast<float>(glottalShapeProp);
    auto brightnessProp = obj->getProperty("brightness");
    profile.brightness =
        brightnessProp.isVoid() ? 0.5f : static_cast<float>(brightnessProp);
  }
...
VoiceCloner::FormantProfile
VoiceCloner::blendProfiles(const FormantProfile &profile1,
                           const FormantProfile &profile2, float blendFactor) {
  blendFactor = std::clamp(blendFactor, 0.0f, 1.0f);
  float invBlend = 1.0f - blendFactor;

  FormantProfile blended;
  for (size_t i = 0; i < 4; ++i) {
    blended.frequencies[i] = profile1.frequencies[i] * invBlend +
                             profile2.frequencies[i] * blendFactor;
    blended.bandwidths[i] = profile1.bandwidths[i] * invBlend +
                            profile2.bandwidths[i] * blendFactor;
  }

  blended.formantShift =
      profile1.formantShift * invBlend + profile2.formantShift * blendFactor;
  blended.glottalShape =
      profile1.glottalShape * invBlend + profile2.glottalShape * blendFactor;
  blended.brightness =
      profile1.brightness * invBlend + profile2.brightness * blendFactor;

  return blended;
}

std::vector<float> VoiceCloner::computeLPC(const std::vector<float> &samples,
                                           int order) {
...
VoiceCloner::findFormantRoots(const std::vector<float> &lpcCoeffs,
                              double sampleRate) {
  ...
  // Use polynomial root finding (simplified - in production, use a proper root
  // finder) For now, use a simplified approach: find peaks in LPC spectrum
  ...
  for (size_t k = 0; k < fftSize / 2; ++k) {
    ...
    for (size_t i = 1; i < lpcCoeffs.size(); ++i) {
      sum +=
          lpcCoeffs[i] *
          std::exp(std::complex<float>(0.0f, -static_cast<float>(i) * omega));
    }
  ...
  for (size_t k = 2; k < fftSize / 2 - 2; ++k) {
    ...
    if (mag > std::abs(fftInput[k - 1]) && mag > std::abs(fftInput[k + 1]) &&
      ...
      float frequency =
          static_cast<float>(k) * static_cast<float>(sampleRate) / fftSize;
      ...
      size_t upperK = k;
      while (upperK < fftSize / 2 - 1 &&
             std::abs(fftInput[upperK]) > halfPower) {
        upperK++;
      }

      float bandwidth = static_cast<float>(upperK - lowerK) *
                        static_cast<float>(sampleRate) / fftSize;
      bandwidth = std::max(bandwidth, 20.0f); // Minimum bandwidth

      // Filter out unrealistic formants
      if (frequency > 100.0f && frequency < sampleRate / 2.0f &&
          bandwidth < 500.0f) {
  ...
  formants.erase(std::remove_if(formants.begin(), formants.end(),
                                [](const std::pair<float, float> &f) {
                                  return f.first > 5000.0f;
                                }),
                 formants.end());
...
float VoiceCloner::estimatePitch(const std::vector<float> &samples,
                                 double sampleRate) {
  ...
  for (size_t period = minPeriod;
       period < std::min(maxPeriod, samples.size() / 2); ++period) {
    float corr = 0.0f;
    size_t count = 0;
...
std::vector<float>
VoiceCloner::preEmphasize(const std::vector<float> &samples) {
