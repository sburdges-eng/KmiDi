 ...
 * - Undo/redo system
 */
class MidiEditor : public PianoRollPreview, public juce::ComponentListener {
public:
  enum class Tool {
  ...
  void selectAll();
  void deselectAll();
  void selectInRange(double startBeat, double endBeat, int minPitch,
                     int maxPitch);
  ...
  // Property editing
  void setSelectedNoteProperty(size_t noteIndex, const juce::String &property,
                               float value);
...
private:
  ...
  // Note operations
  void addNoteAt(double beat, int pitch, double duration = 1.0,
                 int velocity = 100);
  ...
  void updateSelectionBox(const juce::Point<int> &start,
