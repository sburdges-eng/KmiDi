 ...
 *
 * Purpose: UI for natural language feedback input with preview-before-apply
 * functionality. Part of Phase 6 of the "All-Knowing Interactive Musical
 ...
 *
 * Part of Phase 6 of the "All-Knowing Interactive Musical Customization
 * System". Provides text input for user feedback with preview-before-apply
 * functionality.
 */
...
public:
  ...
  std::function<void(const juce::String &feedback)>
      onFeedbackChanged; // Real-time as user types

  juce::String getFeedbackText() const { return feedbackInput_.getText(); }
  void setFeedbackText(const juce::String &text) {
    feedbackInput_.setText(text);
  }
  void clear() { feedbackInput_.clear(); }

  // Show interpreted changes preview
  void showPreview(const std::map<std::string, float> &parameterChanges,
                   float confidence);
  void clearPreview();
