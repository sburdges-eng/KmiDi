...
EmotionWheel::EmotionWheel() { setOpaque(false); }
...
void EmotionWheel::onEmotionSelected(
    std::function<void(const EmotionNode &)> callback) {
...
juce::Point<float>
EmotionWheel::polarToCartesian(float angle, float radius,
                               const juce::Rectangle<int> &bounds) const {
  float centerX = bounds.getCentreX();
  float centerY = bounds.getCentreY();
  float maxRadius =
      std::min(bounds.getWidth(), bounds.getHeight()) / 2.0f - 10.0f;
...
void EmotionWheel::drawEmotionWheel(juce::Graphics &g,
  ...
  g.fillEllipse(centerX - maxRadius, centerY - maxRadius, maxRadius * 2,
                maxRadius * 2);

  // Draw axes using documented text colors
  g.setColour(KellyLookAndFeel::textPrimary.withAlpha(0.5f));
  g.drawLine(centerX - maxRadius, centerY, centerX + maxRadius, centerY,
             1.0f); // Horizontal (valence)
  g.drawLine(centerX, centerY - maxRadius, centerX, centerY + maxRadius,
             1.0f); // Vertical (arousal)

  // Draw labels using documented text colors
  g.setFont(10.0f);
  g.setColour(KellyLookAndFeel::textPrimary);
  g.drawText("Negative", (int)(centerX - maxRadius - 40), (int)(centerY - 6),
             40, 12, juce::Justification::centredRight);
  g.drawText("Positive", (int)(centerX + maxRadius), (int)(centerY - 6), 40, 12,
             juce::Justification::centredLeft);
  g.drawText("Calm", (int)(centerX - 20), (int)(centerY - maxRadius - 15), 40,
             12, juce::Justification::centred);
  g.drawText("Excited", (int)(centerX - 20), (int)(centerY + maxRadius + 3), 40,
             12, juce::Justification::centred);
}

void EmotionWheel::drawEmotionPoint(juce::Graphics &g,
                                    const EmotionPosition &pos, bool isSelected,
                                    bool isHovered) {
  float pointSize = isSelected ? 8.0f : (isHovered ? 6.0f : 4.0f);
  // Use documented colors: Creative Purple for selected, Focus Blue for
  // hovered, textPrimary for normal
  juce::Colour pointColour =
      isSelected ? juce::Colour(0xFFA855F7) // Creative Purple #A855F7
                 : (isHovered ? juce::Colour(0xFF3B82F6) // Focus Blue #3B82F6
                              : KellyLookAndFeel::textPrimary);

  g.setColour(pointColour);
  g.fillEllipse(pos.screenPos.x - pointSize / 2,
                pos.screenPos.y - pointSize / 2, pointSize, pointSize);
  ...
  if (isSelected || isHovered) {
    if (thesaurusRef_) {
      ...
      if (emotion) {
        g.setColour(KellyLookAndFeel::textPrimary);
        g.setFont(9.0f);
        auto textBounds = juce::Rectangle<float>(pos.screenPos.x - 30,
                                                 pos.screenPos.y - 20, 60, 12);
...
std::optional<int>
EmotionWheel::getEmotionAtPoint(juce::Point<float> point) const {
