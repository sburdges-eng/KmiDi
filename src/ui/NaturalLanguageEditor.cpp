...
NaturalLanguageEditor::NaturalLanguageEditor() { setupComponents(); }

void NaturalLanguageEditor::setupComponents() {
  feedbackLabel_.setText("Describe what you'd like to change:",
  ...
  feedbackInput_.setTextToShowWhenEmpty(
      "e.g., 'make it more melancholic', 'bass line doesn't slap'",
      juce::Colours::grey);
...
void NaturalLanguageEditor::paint(juce::Graphics &g) {
  ...
  if (showPreview_ && !previewChanges_.empty()) {
    ...
    g.drawText("Preview Changes (Confidence: " +
                   juce::String(previewConfidence_ * 100.0f, 1) + "%)",
...
void NaturalLanguageEditor::showPreview(
    const std::map<std::string, float> &parameterChanges, float confidence) {
