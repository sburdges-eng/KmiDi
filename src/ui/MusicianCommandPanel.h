...
private:
  ...
  generateWalkingBass(const ParsedCommand &parsed);

  std::vector<juce::MidiMessage> generateArpeggio(const ParsedCommand &parsed);

  std::vector<juce::MidiMessage>
  generateChordVoicing(const ParsedCommand &parsed);

  std::vector<juce::MidiMessage>
  generateDrumPattern(const ParsedCommand &parsed);

  std::vector<juce::MidiMessage>
  generateMelodicLine(const ParsedCommand &parsed);

  // Effects as MIDI
  std::vector<juce::MidiMessage> createPickslideMidi(int bar, float beat,
                                                     int channel);

  std::vector<juce::MidiMessage> createBendMidi(int note, int startBend,
                                                int endBend, int bar,
                                                float beat, int channel);
  ...
  TimingInfo getTimingForSection(const std::string &section,
                                 const std::string &position = "start") const;
  ...
  std::string
  detectMusicTheoryConcept(const std::vector<std::string> &tokens) const;
  ...
  getAutocompleteSuggestions(const std::string &partial) const;
