...
void VirtualKeyboard::paint(juce::Graphics &g) {
  ...
  for (const auto &key : keys_) {
    ...
    bool isHighlighted =
        highlightedNotes_.find(key.midiNote) != highlightedNotes_.end();
    drawKey(g, key, isPressed, isHighlighted);
  }
}

void VirtualKeyboard::resized() { setupKeys(); }
...
void VirtualKeyboard::drawKey(juce::Graphics &g, const Key &key, bool isPressed,
                              bool isHighlighted) {
...
juce::Rectangle<float> VirtualKeyboard::getKeyBounds(int midiNote) const {
  ...
  int whiteKeyMap[] = {0, 0, 1, 1, 2, 3,
                       3, 4, 4, 5, 5, 6}; // MIDI note % 12 to white key
