...
AIGenerationDialog::AIGenerationDialog() {
  ...
  numTracksLabel_.setColour(juce::Label::textColourId,
                            KellyLookAndFeel::textSecondary);
  numTracksLabel_.setTooltip("How many MIDI tracks to generate. Each track "
                             "will be unique based on variability setting.");
  ...
  numTracksSlider_.setTooltip(
      "Set how many different MIDI tracks to generate (1-16)");
  addAndMakeVisible(numTracksSlider_);

  // Use Side A (Theory)
  useSideALabel_.setText("Use A-Side (Music Theory)",
                         juce::dontSendNotification);
  useSideALabel_.setFont(juce::FontOptions(12.0f));
  useSideALabel_.setColour(juce::Label::textColourId,
                           KellyLookAndFeel::textSecondary);
  useSideALabel_.setTooltip("Include music theory settings (key, scale, tempo, "
                            "instruments) from A-side");
  ...
  useSideAToggle_.setTooltip(
      "Toggle to include/exclude music theory settings in AI generation");
  ...
  useSideBLabel_.setColour(juce::Label::textColourId,
                           KellyLookAndFeel::textSecondary);
  useSideBLabel_.setTooltip(
      "Include emotion settings (valence, arousal, intensity) from B-side");
  ...
  useSideBToggle_.setTooltip(
      "Toggle to include/exclude emotion settings in AI generation");
  ...
  blendSidesLabel_.setColour(juce::Label::textColourId,
                             KellyLookAndFeel::textSecondary);
  blendSidesLabel_.setTooltip("When enabled, combines both A-side and B-side "
                              "settings for richer generation");
  ...
  blendSidesToggle_.setTooltip(
      "Enable to blend music theory and emotion settings together");
  addAndMakeVisible(blendSidesToggle_);

  // Variability
  variabilityLabel_.setText("Variability (Different Each Time)",
                            juce::dontSendNotification);
  variabilityLabel_.setFont(juce::FontOptions(12.0f));
  variabilityLabel_.setColour(juce::Label::textColourId,
                              KellyLookAndFeel::textSecondary);
  variabilityLabel_.setTooltip("How different each generation will be: 0 = "
                               "consistent, 1 = very variable");
  ...
  variabilitySlider_.setTooltip("Control randomness: lower = similar outputs, "
                                "higher = more variation each time");
  ...
  barsPerTrackLabel_.setColour(juce::Label::textColourId,
                               KellyLookAndFeel::textSecondary);
  barsPerTrackLabel_.setTooltip(
      "Length of each generated track in bars (4-32 bars)");
  ...
  barsPerTrackSlider_.setTextBoxStyle(juce::Slider::TextBoxRight, false, 60,
                                      20);
  ...
  apiKeyEditor_.setTextToShowWhenEmpty("Enter API key for AI learning...",
                                       juce::Colours::grey);
  apiKeyEditor_.setPasswordCharacter('*');
  apiKeyEditor_.setFont(juce::FontOptions(11.0f));
  apiKeyEditor_.setTooltip("Optional: Enter your LLM API key (OpenAI, "
                           "Anthropic, etc.) to enable AI learning features");
  ...
  saveApiKeyLabel_.setColour(juce::Label::textColourId,
                             KellyLookAndFeel::textSecondary);
  saveApiKeyLabel_.setTooltip(
      "Save API key locally for future use (stored securely)");
  ...
  saveApiKeyToggle_.setTooltip(
      "Enable to save your API key for future sessions");
  addAndMakeVisible(saveApiKeyToggle_);

  // Buttons - Use documented colors
  generateButton_.setButtonText("GENERATE");
  generateButton_.setColour(juce::TextButton::buttonColourId,
                            juce::Colour(0xFF3B82F6)); // Focus Blue
  generateButton_.setTooltip(
      "Generate AI MIDI tracks based on current settings");
...
AIGenerationDialog::AIGenerationRequest
AIGenerationDialog::showDialog(juce::Component *parent) {
  ...
  // Use modal state - the dialog will call exitModalState when buttons are
  // clicked
...
void AIGenerationDialog::paint(juce::Graphics &g) {
  ...
  juce::ColourGradient headerGradient(
      juce::Colour(0xFF3B82F6).withAlpha(0.2f),
      headerBounds.getTopLeft(), // Focus Blue
      juce::Colour(0xFFA855F7).withAlpha(0.15f),
      headerBounds.getBottomLeft(), // Creative Purple
      false);
  ...
  g.setColour(
      juce::Colour(0xFF22C55E).withAlpha(0.6f)); // Balance Green #22C55E
