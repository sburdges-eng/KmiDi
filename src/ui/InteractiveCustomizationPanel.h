 ...
 * - UI Layer: MidiEditor (MIDI editing), NaturalLanguageEditor (natural
 * language feedback)
 ...
 *          Integrates preference visualization, suggestions, parameter
 * morphing, MIDI editing, and natural language feedback.
 *
 * Features:
...
/**
 * InteractiveCustomizationPanel - Unified interface combining all customization
 * components
 *
 * Integrates:
 * - User preference visualization
...
public:
  ...
  NaturalLanguageEditor &getNaturalLanguageEditor() {
    return naturalLanguageEditor_;
  }
  EmotionWorkstation &getWorkstation() { return workstation_; }

  // Preference visualization
  void showPreferences(bool show) {
    showPreferences_ = show;
    repaint();
  }
