...
PianoRollPreview::PianoRollPreview() { setOpaque(false); }
...
void PianoRollPreview::drawGrid(juce::Graphics &g,
...
void PianoRollPreview::drawNotes(juce::Graphics &g,
                                 const juce::Rectangle<int> &bounds) {
  struct NoteDisplay {
    int pitch;
    double start;
  ...
  for (const auto &track : {midiData_.melody, midiData_.bass}) {
    for (const auto &note : track) {
      if (note.startBeat >= timeStart_ && note.startBeat <= timeEnd_) {
        notes.push_back({note.pitch, note.startBeat, note.duration,
...
void PianoRollPreview::drawPlayhead(juce::Graphics &g,
                                    const juce::Rectangle<int> &bounds) {
  if (playheadPosition_ <= 0.0f)
    return;
...
void PianoRollPreview::resized() { repaint(); }
