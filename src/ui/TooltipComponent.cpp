...
TooltipComponent *TooltipComponent::getOrCreateShared() {
  ...
  if (sharedInstance_ == nullptr) {
    auto tooltip = std::make_unique<TooltipComponent>();
    tooltip->addToDesktop(juce::ComponentPeer::windowIsTemporary);
    tooltip->setVisible(false);
    sharedInstance_ = tooltip.release();
  }
...
void TooltipComponent::showTooltip(juce::Component *target,
                                   const juce::String &text, int timeoutMs) {
  if (text.isEmpty()) {
    return;
  }
  ...
  if (target != nullptr) {
    ...
    targetBounds = juce::Desktop::getInstance()
                       .getDisplays()
                       .getPrimaryDisplay()
                       ->totalArea;
  }
...
void TooltipComponent::paint(juce::Graphics &g) {
  ...
  g.drawText(tooltipText_, bounds.reduced(8.0f),
