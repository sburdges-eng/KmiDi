...
WorkstationPanel::WorkstationPanel() : viewport_("WorkstationViewport") {
  setOpaque(true);
  ...
  for (size_t i = 0; i < trackDefs.size(); ++i) {
    ...
    control->muteButton.onClick = [this, i] {
      ...
      if (onTrackMuteChanged)
        onTrackMuteChanged(static_cast<int>(i), tracks_[i].muted);
      repaint();
    };
    ...
    control->soloButton.onClick = [this, i] {
      ...
      if (onTrackSoloChanged)
        onTrackSoloChanged(static_cast<int>(i), tracks_[i].soloed);
      repaint();
    };
    ...
    control->volumeSlider.onValueChange = [this, i] {
      tracks_[i].volume =
          static_cast<float>(trackControls_[i]->volumeSlider.getValue());
      if (onTrackVolumeChanged)
        onTrackVolumeChanged(static_cast<int>(i), tracks_[i].volume);
      repaint();
    };
    ...
    control->panSlider.onValueChange = [this, i] {
      tracks_[i].pan =
          static_cast<float>(trackControls_[i]->panSlider.getValue());
      if (onTrackPanChanged)
        onTrackPanChanged(static_cast<int>(i), tracks_[i].pan);
      repaint();
    };
    ...
    control->nameLabel.setColour(juce::Label::textColourId,
                                 juce::Colours::white);
    trackList_.addAndMakeVisible(control->nameLabel);

    // Instrument label
    control->instrumentLabel.setText("Ch " + juce::String(track.channel + 1),
                                     juce::dontSendNotification);
    control->instrumentLabel.setFont(juce::FontOptions(11.0f));
    control->instrumentLabel.setColour(juce::Label::textColourId,
...
void WorkstationPanel::updateTracks(const GeneratedMidi &midi) {
  ...
  for (const auto &chord : midi.chords) {
    for (const auto &pitch : chord.pitches) {
      ...
      note.pitch = pitch;
      note.velocity =
          100; // Default velocity (Chord struct doesn't have velocity field)
  ...
  trackList_.setSize(getWidth(),
                     static_cast<int>(tracks_.size()) * TRACK_HEIGHT);
...
void WorkstationPanel::paint(juce::Graphics &g) {
  ...
  g.fillAll(KellyLookAndFeel::backgroundDark); // #121212 - Optimal dark mode
                                               // background
  ...
  juce::ColourGradient headerGradient(
      ...
      static_cast<float>(headerBounds.getY()), KellyLookAndFeel::surfaceColor,
      static_cast<float>(headerBounds.getX()),
      static_cast<float>(headerBounds.getBottom()), false);
  ...
  g.drawText("MIDI Workstation", headerBounds.removeFromLeft(200),
             juce::Justification::centredLeft);

  // Draw track count with secondary text color
  g.setFont(juce::FontOptions(12.0f));
  g.setColour(KellyLookAndFeel::textSecondary); // #B8B8B8
  g.drawText(juce::String(tracks_.size()) + " Tracks",
             headerBounds.removeFromRight(100),
...
void WorkstationPanel::resized() {
  ...
  for (size_t i = 0; i < tracks_.size(); ++i) {
    auto trackBounds = juce::Rectangle<int>(
        0, static_cast<int>(i) * TRACK_HEIGHT, getWidth(), TRACK_HEIGHT);
...
void WorkstationPanel::paintTrack(juce::Graphics &g, const TrackInfo &track,
                                  int index,
  ...
  g.setColour(isSelected ? track.trackColour
                         : KellyLookAndFeel::borderColor); // #404040
  g.drawRect(bounds, 1);
...
void WorkstationPanel::paintTrackHeader(juce::Graphics &g,
                                        const TrackInfo &track, int index,
                                        const juce::Rectangle<int> &bounds) {
  // Icon and name with documented text colors
  auto workingBounds = bounds;
  g.setColour(track.trackColour);
  g.setFont(juce::FontOptions(20.0f));
  g.drawText(track.icon, workingBounds.removeFromLeft(30),
             juce::Justification::centred);

  g.setColour(KellyLookAndFeel::textPrimary); // #E8E8E8 - Off-white
  g.setFont(juce::FontOptions(14.0f).withStyle("Bold"));
  g.drawText(track.name, workingBounds.removeFromLeft(100),
             juce::Justification::centredLeft);
}

void WorkstationPanel::paintTrackContent(juce::Graphics &g,
  ...
  if (pitchRange == 0)
    pitchRange = 12; // At least one octave
  ...
  for (const auto &note : track.notes) {
    double x = bounds.getX() +
               (note.startBeat - minTime) / timeRange * bounds.getWidth();
    double width = note.duration / timeRange * bounds.getWidth();
    double y = bounds.getBottom() - (note.pitch - minPitch) /
...
int WorkstationPanel::getTrackIndexFromPosition(
