...
public:
   ...
   *  Lifetime is guaranteed by parent (PluginProcessor owns IntentPipeline
   * which owns thesaurus).
   */
...
private:
  // Non-owning observer - lifetime guaranteed by parent
  // (IntentPipeline/PluginProcessor)
  ...
  juce::Point<float> polarToCartesian(float angle, float radius,
                                      const juce::Rectangle<int> &bounds) const;
  void drawEmotionWheel(juce::Graphics &g, const juce::Rectangle<int> &bounds);
  void drawEmotionPoint(juce::Graphics &g, const EmotionPosition &pos,
