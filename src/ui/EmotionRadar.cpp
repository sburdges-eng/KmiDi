...
EmotionRadar::EmotionRadar() { setOpaque(false); }
...
void EmotionRadar::setTargetEmotion(float valence, float arousal,
                                    float intensity) {
...
void EmotionRadar::paint(juce::Graphics &g) {
  ...
  // Draw current
  drawPoint(g, valence_, arousal_, intensity_, KellyLookAndFeel::accentColor,
            false);
}

void EmotionRadar::drawRadar(juce::Graphics &g,
  ...
  g.drawText("Valence+", center.x + radius + 5, center.y - 8, 50, 16,
             juce::Justification::left);
  g.drawText("Valence-", center.x - radius - 55, center.y - 8, 50, 16,
             juce::Justification::right);
  g.drawText("Arousal+", center.x - 25, center.y - radius - 20, 50, 16,
             juce::Justification::centred);
  g.drawText("Arousal-", center.x - 25, center.y + radius + 4, 50, 16,
             juce::Justification::centred);
}

void EmotionRadar::drawPoint(juce::Graphics &g, float valence, float arousal,
                             float intensity, juce::Colour colour,
                             bool isTarget) {
  auto bounds = getLocalBounds();
  ...
  g.setColour(colour);
  g.fillEllipse(point.x - pointSize, point.y - pointSize, pointSize * 2,
                pointSize * 2);

  g.setColour(colour.brighter(0.5f));
  g.drawEllipse(point.x - pointSize, point.y - pointSize, pointSize * 2,
                pointSize * 2, 1.5f);
}

juce::Point<float>
EmotionRadar::emotionToPoint(float valence, float arousal, float intensity,
  ...
  float y = center.y -
            std::sin(angle) * distance; // Negative because screen Y is inverted

  return juce::Point<float>(x, y);
}

void EmotionRadar::resized() { repaint(); }
