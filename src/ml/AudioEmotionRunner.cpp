...
// Note: SSE/NEON headers included transitively via penta/common/Platform.h
// (T6.4)
...
struct AudioEmotionRunnerImpl {
  ...
  std::unique_ptr<moodycamel::ReaderWriterQueue<EmotionRunnerResult>>
      resultQueue;
  ...
  // Emotion mapping weights: (4, 256) → [valence, arousal, dominance,
  // confidence]
  ...
  EmotionResult mapLatentToEmotion(const float *pooled, size_t dim) {
    assert(dim <= 256 &&
           "mapLatentToEmotion: dim exceeds allocated weight dimensions");
    assert(dim * 4 <= emotionWeights.size() &&
           "mapLatentToEmotion: weight buffer too small for dim");
  ...
  void workerLoop() {
    ...
    while (running.load(std::memory_order_relaxed)) {
      ...
      melComputer.compute(sampleBuffer.data(), samplesAccumulated,
                          melBuffer.data());
      ...
      if (probeOnnx && probeOnnx->isLoaded()) {
        ...
        result.emotion.arousal =
            std::clamp((probeOut[1] + 1.0f) * 0.5f, 0.0f, 1.0f);
        result.emotion.dominance =
            std::clamp(0.5f + 0.3f * result.emotion.arousal +
                           0.2f * std::abs(result.emotion.valence),
                       0.0f, 1.0f);
...
AudioEmotionRunner::~AudioEmotionRunner() { shutdown(); }
...
bool AudioEmotionRunner::initialize(const AudioEmotionRunnerConfig &config) {
  ...
  impl_->resultQueue =
      std::make_unique<moodycamel::ReaderWriterQueue<EmotionRunnerResult>>(16);

  // Pre-allocate buffers
  impl_->sampleBuffer.resize(MelSpectrogram::kRequiredSamples +
...
void AudioEmotionRunner::pushSamples(const float *samples,
...
void AudioEmotionRunner::updateParams(penta::RTState &state,
  ...
  state.dominance.store(impl_->slewDominance.process(),
                        std::memory_order_relaxed);
  state.emotionConfidence.store(impl_->slewConfidence.process(),
                                std::memory_order_relaxed);

  // DSP suggestions go into trackParams[0..2]
  state.trackParams[0].store(impl_->slewFilterCutoff.process(),
                             std::memory_order_relaxed);
  state.trackParams[1].store(impl_->slewReverbWet.process(),
                             std::memory_order_relaxed);
  state.trackParams[2].store(impl_->slewDriveAmount.process(),
                             std::memory_order_relaxed);
  state.end_publish();
}
...
float AudioEmotionRunner::dropRate() const {
  ...
  const uint64_t current =
      impl_->droppedSamples.load(std::memory_order_relaxed);
  ...
  const double dtSec =
      std::chrono::duration<double>(now - impl_->dropRateLastTime).count();
  if (dtSec < 0.001)
    return impl_->dropRateEma; // avoid divide-by-zero on rapid calls

  const uint64_t delta = current - impl_->dropRateLastSamples;
  const float instantRate =
      static_cast<float>(delta) / static_cast<float>(dtSec);

  constexpr float kAlpha = 0.2f;
  impl_->dropRateEma =
      kAlpha * instantRate + (1.0f - kAlpha) * impl_->dropRateEma;
  impl_->dropRateLastSamples = current;
  impl_->dropRateLastTime = now;
