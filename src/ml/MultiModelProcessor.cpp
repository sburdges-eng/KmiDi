  ...
  try {
    ...
    if (!model) {
      juce::Logger::writeToLog(
          juce::String("Failed to parse RTNeural model JSON: ") +
          file.getFullPathName());
      jsonStream.close();
      loaded_ = true; // Use fallback
      return false;
    ...
    juce::Logger::writeToLog(juce::String("Loaded model: ") + spec_.name +
                             " (" + juce::String(spec_.estimatedParams) +
                             " params)");
    jsonStream.close();
    return true;
  } catch (const std::exception &e) {
    juce::Logger::writeToLog(juce::String("Exception loading model ") +
                             spec_.name + ": " + juce::String(e.what()));
    loaded_ = true; // Use fallback
    return false;
  } catch (...) {
    juce::Logger::writeToLog(juce::String("Unknown exception loading model: ") +
                             spec_.name);
    loaded_ = true; // Use fallback
    return false;
  }
#else
  loaded_ = true;
  juce::Logger::writeToLog(juce::String("Loaded model (fallback mode): ") +
                           spec_.name);
  return true;
#endif
}
...
std::vector<float> ModelWrapper::forward(const float *input, size_t inputSize) {
  ...
  if (inputSize != spec_.inputSize) {
    juce::Logger::writeToLog(
        juce::String("ModelWrapper: Input size mismatch. Expected ") +
  ...
  if (rtModel_) {
    try {
      ...
      // Note: RTNeural API may vary - check if getOutputs() exists or use
      // copyOutput()
...
void ModelWrapper::computeFallback(const float *input) {
  ...
  case ModelType::MelodyTransformer:
    ...
    for (size_t i = 0; i < output_.size(); ++i) {
      ...
      output_[i] =
          1.0f /
          (1.0f + std::exp(-(note * 2.0f - 1.0f + emotionInfluence * 0.5f)));
    }
    break;
...
bool MultiModelProcessor::initialize(const juce::File &modelsDir) {
  ...
  if (!modelsDir.isDirectory()) {
    juce::Logger::writeToLog("Models directory not found: " +
                             modelsDir.getFullPathName());
...
std::vector<float> MultiModelProcessor::infer(ModelType type,
                                              const std::vector<float> &input) {
  std::lock_guard<std::mutex> lock(mutex_);
  return models_[static_cast<size_t>(type)]->forward(input.data(),
                                                     input.size());
}

InferenceResult MultiModelProcessor::runFullPipeline(
    const std::array<float, 128> &audioFeatures) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (models_[0]->isEnabled()) {
    ...
    std::copy_n(emotion.begin(), std::min(emotion.size(), size_t(64)),
                result.emotionEmbedding.begin());
  }

  // 2. MelodyTransformer: emotion → melody
  if (models_[1]->isEnabled()) {
    auto melody = models_[1]->forward(result.emotionEmbedding.data(), 64);
    std::copy_n(melody.begin(), std::min(melody.size(), size_t(128)),
                result.melodyProbabilities.begin());
  }

  // 3. HarmonyPredictor: context (emotion + audio) → harmony
  if (models_[2]->isEnabled()) {
    std::array<float, 128> context{};
    std::copy_n(result.emotionEmbedding.begin(),
                std::min(result.emotionEmbedding.size(), size_t(64)),
                context.begin());
    std::copy_n(audioFeatures.begin(),
                std::min(audioFeatures.size(), size_t(64)),
                context.begin() + 64);
    auto harmony = models_[2]->forward(context.data(), 128);
    std::copy_n(harmony.begin(), std::min(harmony.size(), size_t(64)),
                result.harmonyPrediction.begin());
  }
  ...
  if (models_[3]->isEnabled()) {
    ...
    std::copy_n(result.emotionEmbedding.begin(),
                std::min(result.emotionEmbedding.size(), size_t(32)),
                compact.begin());
    auto dynamics = models_[3]->forward(compact.data(), 32);
    std::copy_n(dynamics.begin(), std::min(dynamics.size(), size_t(16)),
                result.dynamicsOutput.begin());
  }

  // 5. GroovePredictor: emotion → groove
  if (models_[4]->isEnabled()) {
    auto groove = models_[4]->forward(result.emotionEmbedding.data(), 64);
    std::copy_n(groove.begin(), std::min(groove.size(), size_t(32)),
                result.grooveParameters.begin());
  }
...
void MultiModelProcessor::logStats() const {
  ...
  juce::Logger::writeToLog(
      "  Total memory: " + juce::String(getTotalMemoryKB()) + " KB");
...
public:
  ...
  void run() override { pipeline_.run(*this); }
...
AsyncMLPipeline::~AsyncMLPipeline() { stop(); }

void AsyncMLPipeline::start() {
  std::lock_guard<std::mutex> lock(lifecycleMutex_);
  if (running_.load(std::memory_order_acquire)) {
    return; // worker already live
  }

  // A leftover worker here has already been asked to exit (stop() joins
  // before resetting); reclaim it before launching a replacement.
  if (thread_) {
    thread_->waitForThreadToExit(-1);
    thread_.reset();
  }

  thread_ = std::make_unique<InferenceThread>(*this);
  // Publish running_ before launch so the worker's first loop check passes;
  // roll back if the OS refuses the thread so a later start() can retry.
  running_.store(true, std::memory_order_release);
  if (!thread_->startThread()) {
...
void AsyncMLPipeline::stop() {
  ...
  if (thread_) {
    // Cooperative shutdown only. Force-killing (stopThread with a
    // timeout) could terminate the worker while it holds the processor
    // mutex inside runFullPipeline(), poisoning every later infer call.
...
uint64_t
AsyncMLPipeline::submitFeatures(const std::array<float, 128> &features) {
  ...
  const uint64_t requestId =
      nextRequestId_.fetch_add(1, std::memory_order_acq_rel);
...
void AsyncMLPipeline::run(juce::Thread &self) {
  while (running_.load(std::memory_order_acquire) && !self.threadShouldExit()) {
    if (hasRequest_.load(std::memory_order_acquire)) {
      // Snapshot the request while the slot is still owned by this worker;
      // releasing hasRequest_ first would let a producer overwrite
      // pendingFeatures_ mid-inference (torn compound state).
      const std::array<float, 128> features = pendingFeatures_;
      const uint64_t requestId =
          pendingRequestId_.load(std::memory_order_acquire);
      ...
      resultBuffers_[writeIdx] = processor_.runFullPipeline(features);
      latestResultId_.store(requestId, std::memory_order_release);
      ...
      // Interruptible 1ms idle wait: stop() wakes it via notify() so
      // shutdown does not have to ride out the poll interval.
      self.wait(1);
    }
  }
}
