...
class ModuleMigrator:
    ...
    def migrate_module(self, module_config: ModuleConfig):
        ...
        print("  Creating temporary clone...")
        if not self.dry_run:
            subprocess.run(
        ...
        if not self.dry_run and not self.check_paths_exist(module_config.source_paths):
            print("  ⚠️  Some source paths not found, skipping...")
        ...
        print("  Merging into target repo...")
        if not self.dry_run:
    ...
    def merge_history(self, temp_repo: Path, config: ModuleConfig):
        ...
        result = self.run_command(
            ["git", "merge", "--allow-unrelated-histories", "--no-commit", f"{remote_name}/main"]
        )
...
def main():
    ...
    parser.add_argument("--source", type=str, help="Path to DAiW source repository")
    parser.add_argument(
        "--target",
        type=str,
        default=".",
    ...
    parser.add_argument("--module", type=str, help='Module to migrate (or "all" for all modules)')
    parser.add_argument("--list-modules", action="store_true", help="List available modules")
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would be done without making changes"
    )
    parser.add_argument("--report", type=str, help="Output migration report to file")
