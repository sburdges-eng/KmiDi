...
private:
    ...
    // Interval explanation helpers
    std::string generateAcousticExplanation(int semitones) const;
    std::string generatePerceptualEffect(int semitones) const;
    std::string generateHistoricalUsage(int semitones) const;
    float calculateHarmonicPosition(int semitones) const;
    std::string identifyScalePattern(const std::vector<int>& intervals) const;
