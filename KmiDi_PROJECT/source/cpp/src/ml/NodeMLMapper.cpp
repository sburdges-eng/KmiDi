...
std::optional<kelly::EmotionNode> NodeMLMapper::embeddingToNode(const std::vector<float>& embedding) const {
    ...
    kelly::EmotionNode nearestNode = findNearestNodeByVAD(valence, arousal, dominance);
...
std::vector<float> NodeMLMapper::nodeToMLInput(const kelly::EmotionNode& node) const {
    return vadToMLInput(node.valence, node.arousal, node.dominance, node.musicalAttributes);
}

kelly::EmotionNode NodeMLMapper::findNearestNodeByVAD(float valence, float arousal, float dominance) const {
    // Use thesaurus's built-in nearest neighbor search
    return thesaurus_.findNearestVAD(valence, arousal, dominance);
}

float NodeMLMapper::calculateVADDistance(const kelly::EmotionNode& node1, const kelly::EmotionNode& node2) const {
    return calculateVADDistance(node1.valence, node1.arousal, node1.dominance, node2);
}

float NodeMLMapper::calculateVADDistance(float valence, float arousal, float dominance, const kelly::EmotionNode& node) const {
...
kelly::EmotionNode NodeMLMapper::enhanceNodeWithML(const kelly::EmotionNode& node,
                                              const std::vector<float>& embedding,
                                              float confidence) const {
    kelly::EmotionNode enhanced = node;
