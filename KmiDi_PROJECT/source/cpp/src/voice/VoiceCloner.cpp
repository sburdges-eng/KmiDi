...
VoiceCloner::FormantProfile VoiceCloner::loadProfile(const std::string& filePath) {
    ...
    if (auto* obj = var.getDynamicObject()) {
        ...
        profile.formantShift = formantShiftProp.isVoid() ? 1.0f : static_cast<float>(formantShiftProp);
        auto glottalShapeProp = obj->getProperty("glottalShape");
        profile.glottalShape = glottalShapeProp.isVoid() ? 0.5f : static_cast<float>(glottalShapeProp);
        auto brightnessProp = obj->getProperty("brightness");
        profile.brightness = brightnessProp.isVoid() ? 0.5f : static_cast<float>(brightnessProp);
    }
