...
from ..production.dynamics_engine import DynamicsEngine, SongStructure, EmotionMatch, DynamicLevel
from ..production.drum_humanizer import DrumHumanizer
from ..structure.comprehensive_engine import render_plan_to_midi, HarmonyPlan, TherapySession
from ..utils.midi_io import extract_notes, save_midi
...
class MIDIGenerationPipeline:
    ...
    def generate_midi(self, intent: CompleteSongIntent, output_dir: str = "./") -> Dict[str, Any]:
        # Ensure mido is available
        if not mido:
            raise ImportError("The 'mido' package is required for MIDI generation. Install with: pip install mido")
        ...
        self.therapy_session.process_core_input(intent.song_root.core_event or intent.song_intent.mood_primary or "neutral")
        ...
        if intent.song_intent.mood_primary:
            ...
            if intent.song_intent.mood_primary in ["Rage", "Desperation", "Defiance"]:
                motivation = 8
                chaos_tolerance = 0.7
            elif intent.song_intent.mood_primary in ["Grief", "Melancholy", "Dissociation"]:
                motivation = 3
                chaos_tolerance = 0.2
        ...
        harmony_plan.tempo_bpm = random.randint(*intent.technical_constraints.technical_tempo_range) if intent.technical_constraints.technical_tempo_range else harmony_plan.tempo_bpm
        harmony_plan.root_note = intent.technical_constraints.technical_key or harmony_plan.root_note
        ...
        for bar in range(bars):
            # Kick on 1 and 3
            drum_events.append({"start_tick": bar * PPQ * 4, "velocity": 100, "pitch": 36, "duration_ticks": PPQ})
            drum_events.append({"start_tick": bar * PPQ * 4 + PPQ * 2, "velocity": 100, "pitch": 36, "duration_ticks": PPQ})
            # Snare on 2 and 4
            drum_events.append({"start_tick": bar * PPQ * 4 + PPQ, "velocity": 90, "pitch": 38, "duration_ticks": PPQ})
            drum_events.append({"start_tick": bar * PPQ * 4 + PPQ * 3, "velocity": 90, "pitch": 38, "duration_ticks": PPQ})
            # Hi-hats on all 8th notes
            for beat in range(8):
                drum_events.append({"start_tick": bar * PPQ * 4 + beat * PPQ // 2, "velocity": 70, "pitch": 42, "duration_ticks": PPQ // 4})
        ...
        groove_settings = GrooveSettings(
            ...
            vulnerability=intent.song_intent.mood_secondary_tension # Use mood tension for vulnerability
        )
        ...
        song_structure = SongStructure(sections=["intro", "chorus", "outro"], section_bars=[8, 16, 8])
        emotion_match = EmotionMatch(base_emotion=intent.song_intent.mood_primary or "neutral", score=1.0, intensity_tier=5)
        arrangement_profile = self.dynamics_engine.get_arrangement_profile(song_structure, emotion_match)

        # 5. Render to MIDI file
        output_file_name = f"kmidi_generated_{intent.title.replace(' ', '_')}.mid" if intent.title else "kmidi_generated.mid"
        ...
        for note_pitch in melody_notes:
            ...
            melody_track.append(mido.Message('note_off', note=note_pitch, velocity=0, time=PPQ)) # Quarter note duration
            current_tick += PPQ
        ...
        for event in humanized_drum_events:
            ...
            drum_track.append(mido.Message('note_on', note=event["pitch"], velocity=event["velocity"], time=delta_time, channel=9))
            # Assuming a short duration for drums for now
            drum_track.append(mido.Message('note_off', note=event["pitch"], velocity=0, time=PPQ // 4, channel=9))
            current_tick = event["start_tick"] + PPQ // 4 # Advance current_tick by note_on time + duration
