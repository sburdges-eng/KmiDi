...
class EmotionEmbedding:
    ...
    def from_output(cls, output: List[float], confidence: float = 0.0) -> EmotionEmbedding:
        ...
        emotion_map = [
            "neutral", "grief", "anxiety", "anger",
            "calm", "hope", "nostalgia", "tension",
        ]
        if len(output) > 0:
...
class MLPipeline:
    ...
    def submit_dynamics(
        ...
        features = (
            emotion_embedding[:32] if len(emotion_embedding) >= 32
    ...
    def submit_groove(
        ...
        features = (
            emotion_embedding if len(emotion_embedding) >= 64
            else np.zeros(64, dtype=np.float32)
        )
    ...
    def _generate_fallback_result(
        ...
        if model_type == ModelType.EmotionRecognizer:
            ...
            output = (
                list(features[:64]) if len(features) >= 64
                else list(features) + [0.0] * (64 - len(features))
            )
            result = {
                "model_type": model_type, "output": output,
