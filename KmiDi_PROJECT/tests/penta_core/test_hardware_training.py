...
# Skip entire module if the hardware benchmarks script is not available
if not HARDWARE_SCRIPT_PATH.exists():
    pytest.skip(
        f"hardware benchmarks not found at {HARDWARE_SCRIPT_PATH}; skipping hardware training tests",
        allow_module_level=True,
    )

def import_hardware_benchmarks():
    """Import the hardware_benchmarks module dynamically."""
    spec = importlib.util.spec_from_file_location("hardware_benchmarks", str(HARDWARE_SCRIPT_PATH))
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module

# Only import if the script exists (guarded by the skip above)
hardware_benchmarks = None
if HARDWARE_SCRIPT_PATH.exists():
