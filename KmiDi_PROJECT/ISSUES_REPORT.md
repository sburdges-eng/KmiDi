...
20) Dataset downloader uses network requests without timeouts.
...
21) Registry manifest validation is silently skipped when `jsonschema` is missing.
...
22) Review artifacts claim critical issues are fixed, but current backup scripts still include hardcoded paths and dummy data.
