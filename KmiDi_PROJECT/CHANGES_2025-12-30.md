...
- ✅ `config/train-mac-smoke.yaml` - Added missing fields
...
1. ✅ Test smoke config end-to-end:
   ```bash
   python scripts/train.py --config config/train-mac-smoke.yaml --dry-run
   ```
...
3. ✅ Commit changes:
   ```bash
   git add config/build-dev-mac.yaml config/train-mac-smoke.yaml scripts/train.py
