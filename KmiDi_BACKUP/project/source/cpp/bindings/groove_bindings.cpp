...
void bind_groove(py::module_ &m)
{
    ...
    py::enum_<RhythmQuantizer::GridResolution>(m, "GridResolution")
        ...
        .value("EIGHTH_TRIPLET", RhythmQuantizer::GridResolution::EighthTriplet)
        .value("SIXTEENTH", RhythmQuantizer::GridResolution::Sixteenth)
        .value("SIXTEENTH_TRIPLET", RhythmQuantizer::GridResolution::SixteenthTriplet)
        .value("THIRTY_SECOND", RhythmQuantizer::GridResolution::ThirtySecond)
        .value("THIRTY_SECOND_TRIPLET", RhythmQuantizer::GridResolution::ThirtySecondTriplet)
        .export_values();
    ...
    py::class_<GrooveEngine>(m, "GrooveEngine")
        ...
        .def("process_audio", [](GrooveEngine &self, py::array_t<float> buffer)
            ...
            self.processAudio(static_cast<float*>(info.ptr), info.shape[0]); }, py::arg("buffer"), "Process audio buffer for groove analysis")
        .def("get_analysis", &GrooveEngine::getAnalysis, py::return_value_policy::copy, "Get current groove analysis results")
        .def("quantize_to_grid", &GrooveEngine::quantizeToGrid, py::arg("timestamp"), "Quantize timestamp to rhythmic grid")
        .def("apply_swing", &GrooveEngine::applySwing, py::arg("position"), "Apply swing to position")
        .def("update_config", &GrooveEngine::updateConfig, py::arg("config"), "Update engine configuration")
        .def("reset", &GrooveEngine::reset, "Reset analysis state");
