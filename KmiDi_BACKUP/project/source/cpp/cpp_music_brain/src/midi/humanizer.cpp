 ...
 */
const HumanizePreset PRESET_LOFI = {"lo-fi-depression",
...
const HumanizePreset PRESET_MECHANICAL = {"mechanical-dissociation",
                                          0.05f, // Very tight
                                          0.1f,
                                          0.0f, // No ghost notes
                                          0.0f,
 ...
 */
std::vector<NoteEvent> humanize(const std::vector<NoteEvent> &notes,
  ...
  for (const auto &note : notes) {
    ...
    humanized.velocity = static_cast<MidiVelocity>(std::clamp(newVel, 1, 127));
