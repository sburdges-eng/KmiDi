...
TimeSignature
RhythmEngine::analyzeTimeSignature(const std::vector<float> &onsetTimes,
...
std::string RhythmEngine::detectMeterFeel(int numerator,
...
std::string RhythmEngine::explainTimeSignature(const TimeSignature &ts,
  ...
  case ExplanationDepth::Advanced:
    explanation << "Time signature: " << ts.numerator << "/" << ts.denominator
                << "\n"
                << "Metric feel: " << ts.feel << "\n"
                << "Each beat subdivides into " << ts.denominator
                << " note values.\n"
                << "Physical embodiment: " << ts.bodyResponse << "\n"
                << "This creates a "
                << (ts.numerator % 2 == 0 ? "symmetrical" : "asymmetrical")
                << " rhythmic structure.";
    break;
  ...
  case ExplanationDepth::Expert:
    ...
    explanation << "\nPsychoacoustic grouping tendencies favor "
                << (ts.numerator % 2 == 0 ? "binary" : "ternary")
                << " subdivisions.";
    break;
  }
  ...
  if (it != emotionRhythmMappings_.end()) {
    for (const auto &meter : it->second.preferredMeters) {
      ...
      suggestion.explanation =
          "This meter creates the rhythmic feel associated with " + emotion;
...
GrooveAnalysis
RhythmEngine::analyzeGroove(const std::vector<float> &onsetTimes,
  ...
  analysis.microTimingShifts =
      analyzeMicroTiming(onsetTimes, analysis.quantizedOnsets);
...
std::vector<float>
RhythmEngine::quantizeWithGroove(const std::vector<float> &onsetTimes,
...
float RhythmEngine::calculateSwingRatio(const std::vector<float> &onsetTimes,
...
std::string
RhythmEngine::detectGrooveType(const GrooveAnalysis &analysis) const {
...
RhythmEngine::PolyrhythmAnalysis RhythmEngine::detectPolyrhythm(
  ...
  if (noteCounts.size() >= 2) {
    ...
    analysis.description =
        std::to_string(ratio1) + " against " + std::to_string(ratio2);
    ...
    // Find meeting points
    if (layers.size() >= 2) {
      float duration = std::max(layers[0].empty() ? 0.0f : layers[0].back(),
...
RhythmEngine::PolymeterAnalysis
RhythmEngine::detectPolymeter(const std::vector<std::vector<float>> &layers,
                              float duration) const {
  ...
  if (analysis.meters.size() >= 2) {
    ...
    analysis.explanation = "The meters align every " +
                           std::to_string(analysis.barsUntilAlignment) +
                           " bars";
  }
...
std::vector<float>
RhythmEngine::findMeetingPoints(const std::vector<int> &ratios,
                                float duration) const {
...
RhythmicPattern
RhythmEngine::generatePatternForEmotion(const std::string &emotion,
                                        const TimeSignature &timeSignature,
                                        float density, int numBars) const {
  RhythmicPattern pattern;
  ...
  int numNotes =
      static_cast<int>(totalDuration * targetDensity * timeSignature.numerator);
...
RhythmicPattern
RhythmEngine::generatePatternFromDescription(const std::string &description,
                                             const TimeSignature &timeSignature,
                                             int numBars) const {
    ...
    if (tmpl.name == description || tmpl.feel == description) {
      ...
      for (size_t i = 0; i < pattern.onsetTimes.size(); ++i) {
        ...
        if (i < pattern.onsetTimes.size() - 1) {
          duration = std::min(0.25f, pattern.onsetTimes[i + 1] -
                                         pattern.onsetTimes[i]);
        }
...
RhythmicPattern RhythmEngine::addSyncopation(const RhythmicPattern &pattern,
                                             float syncopationAmount) const {
  RhythmicPattern syncopated = pattern;

  if (syncopationAmount < 0.1f)
    return syncopated;
...
RhythmicPattern RhythmEngine::applySwing(const RhythmicPattern &pattern,
...
RhythmicPattern
RhythmEngine::humanizePattern(const RhythmicPattern &pattern,
                              float humanization,
                              const std::string &pocketStyle) const {
  RhythmicPattern humanized = pattern;

  if (humanization < 0.01f)
    return humanized;
  ...
  std::uniform_real_distribution<> timingDis(-humanization * 20.0f,
                                             humanization * 20.0f);
...
float RhythmEngine::detectSyncopation(
  ...
  for (float time : onsetTimes) {
    if (!isOnStrongBeat(time, timeSignature) &&
        !isOnWeakBeat(time, timeSignature)) {
      syncopatedNotes++;
    }
  }
...
float RhythmEngine::calculateDensity(const std::vector<float> &onsetTimes,
                                     float duration) const {
  if (duration <= 0.0f)
    return 0.0f;
...
std::vector<RhythmEngine::RhythmicMotif>
RhythmEngine::detectMotifs(const std::vector<float> &onsetTimes,
  ...
  for (size_t patternLength = 2; patternLength <= ioi.size() / 2;
       ++patternLength) {
...
RhythmEngine::detectClavePattern(const std::vector<float> &onsetTimes) const {
  if (onsetTimes.size() < 5)
    return std::nullopt;
  ...
  for (const auto &[name, pattern] : clavePatterns_) {
    if (normalized.size() != pattern.size())
      continue;
...
std::vector<float>
RhythmEngine::analyzeBeatStrength(const std::vector<float> &onsetTimes,
                                  const std::vector<int> &velocities,
                                  const TimeSignature &timeSignature) const {
  ...
  for (size_t i = 0; i < onsetTimes.size(); ++i) {
    int beatPosition =
        static_cast<int>(onsetTimes[i]) % timeSignature.numerator;
    beatStrengths[beatPosition] += velocities[i] / 127.0f;
  }

  // Normalize
  float maxStrength =
      *std::max_element(beatStrengths.begin(), beatStrengths.end());
  if (maxStrength > 0.0f) {
...
float RhythmEngine::calculateStability(
...
std::vector<RhythmEngine::TempoChange>
RhythmEngine::detectTempoChanges(const std::vector<float> &onsetTimes) const {
  std::vector<TempoChange> changes;

  if (onsetTimes.size() < 10)
    return changes;
  ...
  for (size_t i = 1; i < localTempos.size(); ++i) {
    ...
    if (percentChange > 0.1f) { // 10% change
      ...
      change.endTime =
          onsetTimes[std::min(i + windowSize, onsetTimes.size() - 1)];
...
std::string RhythmEngine::explainRhythmicFeel(const RhythmicPattern &pattern,
                                              ExplanationDepth depth) const {
  std::ostringstream explanation;
  ...
  case ExplanationDepth::Expert:
    ...
    explanation << "\nSwing: " << pattern.swingRatio << ":1 ratio\n"
                ...
                << "Micro-timing characteristics create: "
                << pattern.perceptualGroove;
    break;
  }
...
RhythmicPattern
RhythmEngine::getCommonPattern(const std::string &name,
                               const TimeSignature &timeSignature) const {
  for (const auto &tmpl : patternDatabase_) {
    if (tmpl.name == name) {
      RhythmicPattern pattern;
...
std::vector<RhythmicPattern>
RhythmEngine::findSimilarPatterns(const RhythmicPattern &pattern,
...
std::vector<float>
RhythmEngine::onsetsToIOI(const std::vector<float> &onsetTimes) const {
...
std::vector<float>
RhythmEngine::ioiToOnsets(const std::vector<float> &intervals) const {
...
std::vector<float>
RhythmEngine::quantizeToGrid(const std::vector<float> &onsetTimes,
                             int subdivision) const {
  std::vector<float> quantized;
...
std::vector<std::pair<float, TimeSignature>>
RhythmEngine::detectTimeSignatureChanges(const std::vector<float> &onsetTimes,
                                         float duration) const {
...
void RhythmEngine::initializePatternDatabase() {
  patternDatabase_ = {{"four-on-the-floor",
                       {0.0f, 1.0f, 2.0f, 3.0f},
                       "Electronic",
...
void RhythmEngine::initializeEmotionRhythmMappings() {
  emotionRhythmMappings_["excitement"] = {"excitement",
                                          4.0f, // High density
...
std::vector<float> RhythmEngine::createGrid(float duration,
                                            int subdivision) const {
...
std::vector<float>
RhythmEngine::findSwingPairs(const std::vector<float> &onsetTimes,
                             int subdivision) const {
...
std::vector<float>
RhythmEngine::detectBeats(const std::vector<float> &onsetTimes,
                          const TimeSignature &timeSignature) const {
...
bool RhythmEngine::isOnWeakBeat(float time, const TimeSignature &ts) const {
  ...
  return !isOnStrongBeat(time, ts) && std::abs(beatTime - roundedBeat) < 0.1f;
}

float RhythmEngine::calculateAverageShift(
    const std::vector<float> &shifts) const {
  return calculateMean(shifts);
}

std::string RhythmEngine::classifyPocketStyle(float avgShift,
                                              float stdDev) const {
  if (stdDev < 5.0f) {
