...
public:
  ...
  Chord buildChord(int rootNote, ChordQuality quality,
  ...
  identifyExtensions(const std::vector<int> &notes) const;
   ...
   */
  std::string getHarmonicFunction(const Chord &chord,
                                  const std::string &key) const;
  ...
  suggestVoiceLeading(const Chord &fromChord, const Chord &toChord,
                      VoiceLeadingStyle style = VoiceLeadingStyle::Bach,
                      int numVoices = 4) const;
  ...
  std::vector<VoiceLeadingError>
  checkVoiceLeading(const std::vector<std::vector<int>> &voices,
                    VoiceLeadingStyle style) const;

  /**
   * Find common tones between chords (for smooth transitions)
   */
  std::vector<int> findCommonTones(const Chord &chord1,
  ...
  suggestSubstitutions(const Chord &chord, const std::string &key,
                       SubstitutionType type = SubstitutionType::All) const;
   ...
   */
  ChordProgression reharmonizeMelody(const std::vector<int> &melody,
                                     const std::string &key,
                                     const std::string &style = "Pop") const;
   ...
   */
  ChordProgression
  generateProgressionForEmotion(const std::string &emotion,
                                const std::string &key, int numChords = 4,
  ...
  analyzeEmotionalContent(const ChordProgression &progression) const;
   ...
   */
  ChordProgression getCommonProgression(const std::string &name,
                                        const std::string &key) const;
  ...
  findSimilarProgressions(const ChordProgression &progression,
   ...
   */
  ChordProgression transposeProgression(const ChordProgression &progression,
                                        int semitones) const;
...
private:
  ...
  std::vector<int> getIntervalsFromRoot(const std::vector<int> &notes,
                                        int root) const;

  // Voice leading calculation
  float
  calculateVoiceDistance(const std::vector<std::vector<int>> &voices) const;
  bool hasParallelMotion(int voice1, int voice2,
                         const std::vector<int> &fromChord,
                         const std::vector<int> &toChord, int interval) const;
  ...
  // Emotion mapping
  std::vector<std::string>
  selectChordQualities(const std::string &emotion) const;
  std::vector<std::string>
  selectProgressionPattern(const std::string &emotion) const;
