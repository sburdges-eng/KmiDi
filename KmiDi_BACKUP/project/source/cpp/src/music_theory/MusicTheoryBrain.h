...
public:
  ...
  CompleteAnalysis analyzeMIDI(const std::vector<int> &midiNotes,
  ...
  std::string analyzeAndExplain(
      ...
      ExplanationDepth depth = ExplanationDepth::Intermediate) const;
  ...
  GeneratedMusic generateFromEmotion(const std::string &emotion,
                                     int numBars = 4,
                                     bool allowRuleBreaking = true) const;
   ...
   */
  GeneratedMusic generateFromDescription(const std::string &description,
                                         int numBars = 4) const;
  ...
  reharmonizeMelody(const std::vector<int> &melody,
                    const std::string &style = "Jazz") const;
  ...
  getCustomLearningPath(const std::string &targetConcept,
                        const UserProfile &userProfile) const;
  ...
  PracticeSession generatePracticeSession(UserProfile &userProfile,
                                          int duration = 30) const;
   ...
   */
  std::string explainEmotionalEffect(const std::vector<int> &midiNotes,
                                     const std::vector<float> &onsetTimes,
  ...
  TheoryValidation validateTheory(const std::vector<int> &midiNotes,
                                  const std::vector<float> &onsetTimes,
                                  const std::string &style = "General") const;
  ...
  std::vector<ImprovementSuggestion>
  suggestImprovements(const std::vector<int> &midiNotes,
                      const std::vector<float> &onsetTimes,
  ...
  suggestSubstitutions(const ChordProgression &progression,
   ...
   */
  std::vector<Chord>
  suggestNextChord(const std::vector<Chord> &progressionSoFar,
...
private:
  ...
  std::vector<Chord>
  detectChordsFromMIDI(const std::vector<int> &notes,
                       const std::vector<float> &onsetTimes) const;

  // Generation helpers
  std::vector<int> progressionToMIDI(const ChordProgression &progression,
                                     int octave = 4) const;

  std::vector<float> rhythmToOnsetTimes(const RhythmicPattern &rhythm) const;

  std::vector<int> generateVelocities(size_t numNotes,
  ...
  std::string explainWithStyle(const std::string &conceptName,
  ...
  std::vector<MIDIGroup>
  groupSimultaneousNotes(const std::vector<int> &notes,
                         const std::vector<float> &onsetTimes,
                         float tolerance = 0.05f) const;

  // Emotion mapping
  std::vector<std::string>
  getEmotionalCharacteristics(const std::string &emotion) const;
