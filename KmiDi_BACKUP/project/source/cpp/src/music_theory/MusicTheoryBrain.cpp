...
MusicTheoryBrain::CompleteAnalysis
MusicTheoryBrain::analyzeMIDI(const std::vector<int> &midiNotes,
  ...
  if (!onsetTimes.empty()) {
    ...
    analysis.timeSignature =
        rhythm_->analyzeTimeSignature(onsetTimes, duration);

    if (!velocities.empty()) {
      analysis.grooveAnalysis = rhythm_->analyzeGroove(onsetTimes, velocities,
                                                       analysis.timeSignature);
    }
  }
  ...
  analysis.concepts =
      knowledge_->identifyConceptsFromMIDI(midiNotes, onsetTimes);
  ...
  explanation << "Time Signature: " << analysis.timeSignature.numerator << "/"
  ...
  for (const auto &concept : analysis.concepts) {
    analysis.learningOpportunities.push_back("Learn about: " + concept.concept +
                                             " (" + concept.explanation + ")");
  }
...
std::string
MusicTheoryBrain::analyzeAndExplain(const std::vector<int> &midiNotes,
                                    const std::vector<float> &onsetTimes,
...
MusicTheoryBrain::GeneratedMusic
MusicTheoryBrain::generateFromEmotion(const std::string &emotion, int numBars,
  ...
  result.whyThisProgression = "This progression was chosen to evoke " +
                              emotion + " through " +
                              result.progression.harmonyExplanation;

  result.whyThisRhythm = "The rhythmic pattern supports " + emotion + " with " +
                         result.rhythm.perceptualGroove;
...
MusicTheoryBrain::GeneratedMusic
MusicTheoryBrain::generateFromDescription(const std::string &description,
                                          int numBars) const {
  GeneratedMusic result;
  ...
  if (!parsed.progression.empty()) {
    result.progression =
        harmony_->getCommonProgression(parsed.progression, result.key);
  } else {
    // Default progression
  ...
  if (!parsed.rhythmPattern.empty()) {
    ...
    result.rhythm =
        rhythm_->generatePatternFromDescription("steady", ts, numBars);
  }
...
MusicTheoryBrain::ReharmonizationResult
MusicTheoryBrain::reharmonizeMelody(const std::vector<int> &melody,
  ...
  result.originalProgression =
      harmony_->reharmonizeMelody(melody, key, "Simple");
  ...
  explanation << "Made " << result.substitutionsMade.size()
              << " substitutions.\n";

  for (const auto &sub : result.substitutionsMade) {
    explanation << "- " << sub.originalChord.symbol << " → "
                << sub.substituteChord.symbol << ": " << sub.explanation
                << "\n";
  }
...
std::string
MusicTheoryBrain::askQuestion(const std::string &question,
  ...
  if (!concepts.empty()) {
    ...
    return knowledge_->explainConcept(concept.concept,
...
MusicTheoryBrain::getCustomLearningPath(const std::string &targetConcept,
                                        const UserProfile &userProfile) const {
  return knowledge_->generateCurriculum(userProfile, {targetConcept}, 20);
}

MusicTheoryBrain::PracticeSession
MusicTheoryBrain::generatePracticeSession(UserProfile &userProfile,
                                          int duration) const {
  ...
  session.sessionGoal =
      "Practice " + std::to_string(session.exercises.size()) + " concepts";

  return session;
}

std::string MusicTheoryBrain::explainEmotionalEffect(
    const std::vector<int> &midiNotes, const std::vector<float> &onsetTimes,
  ...
  explanation << "RHYTHM:\n";
  explanation << "The rhythmic feel supports " << perceivedEmotion
              << " with:\n";
...
MusicTheoryBrain::TheoryValidation
MusicTheoryBrain::validateTheory(const std::vector<int> &midiNotes,
                                 const std::vector<float> &onsetTimes,
                                 const std::string &style) const {
  ...
  for (size_t i = 0; i < chords.size() - 1; ++i) {
    ...
    auto suggestion =
        harmony_->suggestVoiceLeading(chords[i], chords[i + 1], voiceStyle);

    // Check for errors
    auto errors = harmony_->checkVoiceLeading(suggestion.voices, voiceStyle);

    validation.voiceLeadingIssues.insert(validation.voiceLeadingIssues.end(),
...
MusicTheoryBrain::suggestImprovements(const std::vector<int> &midiNotes,
                                      const std::vector<float> &onsetTimes,
  ...
  if (!current.chords.empty()) {
    ...
    auto emotionAnalysis =
        harmony_->analyzeEmotionalContent(current.progressionAnalysis);
    ...
    if (!matchesTarget) {
      ...
      suggestion.suggestion =
          "Adjust chord progression to better match " + targetEmotion;
      suggestion.reasoning =
          "Current progression doesn't strongly evoke " + targetEmotion;
...
std::vector<HarmonyEngine::Substitution>
MusicTheoryBrain::suggestSubstitutions(const ChordProgression &progression,
                                       const std::string &style) const {
...
std::string
MusicTheoryBrain::identifyChord(const std::vector<int> &notes) const {
  auto chord = harmony_->analyzeChord(notes);
  return chord.symbol;
}

std::string
MusicTheoryBrain::identifyScale(const std::vector<int> &notes) const {
  auto scale = detectScaleFromNotes(notes);
  return scale.name;
}

std::string
MusicTheoryBrain::identifyRhythm(const std::vector<float> &onsetTimes) const {
  TimeSignature ts{4, 4, "Duple", "Walking rhythm"};
...
std::vector<Chord>
MusicTheoryBrain::suggestNextChord(const std::vector<Chord> &progressionSoFar,
  ...
  if (function == "Tonic") {
    ...
    suggestions.push_back(
        harmony_->buildChord(67, ChordQuality::Dominant7)); // V7
...
std::string
MusicTheoryBrain::detectKeyFromNotes(const std::vector<int> &notes) const {
  if (notes.empty())
    return "C Major";
  ...
  bool hasMajorThird =
      pitchClassCounts.find((tonicPC + 4) % 12) != pitchClassCounts.end();
  bool hasMinorThird =
      pitchClassCounts.find((tonicPC + 3) % 12) != pitchClassCounts.end();
...
Scale MusicTheoryBrain::detectScaleFromNotes(
...
std::vector<Chord> MusicTheoryBrain::detectChordsFromMIDI(
    const std::vector<int> &notes, const std::vector<float> &onsetTimes) const {
...
std::vector<int>
MusicTheoryBrain::progressionToMIDI(const ChordProgression &progression,
                                    int octave) const {
...
std::vector<float>
MusicTheoryBrain::rhythmToOnsetTimes(const RhythmicPattern &rhythm) const {
  return rhythm.onsetTimes;
}

std::vector<int>
MusicTheoryBrain::generateVelocities(size_t numNotes,
...
MusicTheoryBrain::ParsedDescription
MusicTheoryBrain::parseDescription(const std::string &description) const {
...
std::string
MusicTheoryBrain::answerIntervalQuestion(const std::string &question,
...
std::string
MusicTheoryBrain::answerChordQuestion(const std::string &question,
                                      const UserProfile &profile) const {
  return "Chord questions coming soon. Ask about specific chord types.";
}

std::string
MusicTheoryBrain::answerProgressionQuestion(const std::string &question,
                                            const UserProfile &profile) const {
  return "Progression questions coming soon. Ask about specific progressions.";
}

std::string
MusicTheoryBrain::answerRhythmQuestion(const std::string &question,
                                       const UserProfile &profile) const {
...
MusicTheoryBrain::groupSimultaneousNotes(const std::vector<int> &notes,
                                         const std::vector<float> &onsetTimes,
                                         float tolerance) const {
  std::vector<MIDIGroup> groups;

  if (notes.size() != onsetTimes.size())
    return groups;
...
std::vector<std::string> MusicTheoryBrain::getEmotionalCharacteristics(
  ...
  if (emotion == "joy") {
    ...
    characteristics = {"Major tonality", "Ascending melodies",
                       "Building dynamics"};
  }
...
MusicTheoryBrain::tokenize(const std::string &str) const {
...
bool MusicTheoryBrain::containsKeyword(const std::string &text,
