...
MidiBuilder::MidiBuilder() { initializeChannelAssignments(); }
...
juce::MidiFile MidiBuilder::buildMidiFile(const GeneratedMidi &midi) {
  ...
  int microsecondsPerBeat =
      MIDI_MICROSECONDS_PER_MINUTE / static_cast<int>(midi.bpm);
  ...
  if (!midi.chords.empty()) {
    ...
    for (const auto &chord : midi.chords) {
      addChordToSequence(chordTrack, chord, MIDI_PPQ, channel,
  ...
  if (!midi.counterMelody.empty()) {
    ...
    addNotesToSequence(counterMelodyTrack, midi.counterMelody, MIDI_PPQ,
                       channel);
...
juce::MidiBuffer MidiBuilder::buildMidiBuffer(const GeneratedMidi &midi,
  ...
  if (!midi.chords.empty()) {
    ...
    for (const auto &chord : midi.chords) {
      ...
      int endSample =
          beatsToSamples(chord.startBeat + chord.duration, sampleRate, bpm);
      ...
      for (int pitch : chord.pitches) {
        ...
        buffer.addEvent(
            juce::MidiMessage::noteOn(channel, pitch,
                                      clampVelocity(MIDI_VELOCITY_MEDIUM)),
            startSample);
        buffer.addEvent(juce::MidiMessage::noteOff(channel, pitch), endSample);
      }
    }
  }
  ...
  if (!midi.melody.empty()) {
    ...
    for (const auto &note : midi.melody) {
      ...
      int endSample =
          beatsToSamples(note.startBeat + note.duration, sampleRate, bpm);

      int pitch = std::clamp(note.pitch, MIDI_PITCH_MIN, MIDI_PITCH_MAX);

      buffer.addEvent(juce::MidiMessage::noteOn(channel, pitch,
...
void MidiBuilder::addChordToSequence(juce::MidiMessageSequence &seq,
                                     const Chord &chord, int ticksPerBeat,
                                     int channel, int velocity) {
  ...
  for (int pitch : chord.pitches) {
    ...
    seq.addEvent(juce::MidiMessage::noteOn(channel, pitch, vel), startTick);
    seq.addEvent(juce::MidiMessage::noteOff(channel, pitch), endTick);
  }
}

void MidiBuilder::addNotesToSequence(juce::MidiMessageSequence &seq,
                                     const std::vector<MidiNote> &notes,
                                     int ticksPerBeat, int channel) {
  channel = clampChannel(channel);
...
void MidiBuilder::setChannelForLayer(const std::string &layerName,
                                     int channel) {
  std::lock_guard<std::mutex> lock(mutex_);
...
int MidiBuilder::beatsToSamples(double beats, double sampleRate,
                                float bpm) const {
  double samplesPerBeat = (sampleRate * 60.0) / bpm;
...
juce::uint8 MidiBuilder::clampVelocity(int velocity) const {
  return static_cast<juce::uint8>(
      std::clamp(velocity, MIDI_VELOCITY_MIN, MIDI_VELOCITY_MAX));
}
