...
/**
 * Complete MIDI Generator - Orchestrates all MIDI engines to generate full
 * arrangements.
 *
 * This is the central orchestrator that takes an IntentResult and generates
 * complete MIDI using all available engines:
 *
 ...
 *
 * All generation is driven by the IntentResult's emotion, rule breaks, and
 * musical parameters.
 */
class MidiGenerator {
public:
   ...
   *
   * @param intent The processed emotional intent (contains emotion, rule
   * breaks, musical params)
   * @param bars Number of bars to generate (default 8, can be 4-32)
   * @param complexity 0.0 (simple) to 1.0 (complex) - affects chord extensions,
   * melody density
   * @param humanize 0.0 (quantized) to 1.0 (loose) - affects timing and
   * velocity
   * @param feel -1.0 (pull) to 1.0 (push) - affects timing feel
   * @param dynamics 0.0 to 1.0 - affects velocity range
   * @return Complete MIDI with all layers (chords, melody, bass, pads, strings,
   * etc.)
   */
  GeneratedMidi generate(const IntentResult &intent, int bars = 8,
                         float complexity = 0.5f, float humanize = 0.4f,
                         float feel = 0.0f, float dynamics = 0.75f);
...
private:
   ...
   */
  std::vector<MidiNote> generateMelody(const std::vector<Chord> &chords,
                                       const IntentResult &intent,
                                       float complexity, float dynamics);

  /**
   * Generate bass using BassEngine.
   */
  std::vector<MidiNote> generateBass(const std::vector<Chord> &chords,
                                     const IntentResult &intent,
                                     float complexity, float dynamics);
   ...
   */
  std::vector<MidiNote> generatePads(const std::vector<Chord> &chords,
                                     const IntentResult &intent,
                                     float complexity);
   ...
   */
  std::vector<MidiNote> generateStrings(const std::vector<Chord> &chords,
                                        const IntentResult &intent,
                                        float complexity);
   ...
   */
  std::vector<MidiNote>
  generateCounterMelody(const std::vector<MidiNote> &primaryMelody,
                        const IntentResult &intent, float complexity);

  /**
   * Generate rhythmic patterns using RhythmEngine.
   */
  std::vector<MidiNote> generateRhythm(const IntentResult &intent, int bars,
                                       float complexity, int tempoBpm);

  /**
   * Generate drum groove patterns using DrumGrooveEngine.
   */
  std::vector<MidiNote> generateDrumGroove(const IntentResult &intent, int bars,
                                           float complexity, int tempoBpm);

  /**
   * Generate transitions using TransitionEngine.
   */
  std::vector<MidiNote> generateTransitions(const IntentResult &intent,
                                            int bars, int tempoBpm);

  /**
   * Generate fills using FillEngine.
   */
  std::vector<MidiNote> generateFills(const IntentResult &intent, int bars,
                                      float complexity);
   ...
   */
  ArrangementOutput generateArrangement(const IntentResult &intent, int bars);
   ...
   */
  void applyGrooveAndHumanize(GeneratedMidi &midi, float humanize,
                              const EmotionNode &emotion, float feel);

  /**
   * Apply dynamics to all MIDI layers based on emotion and rule breaks.
   */
  void applyDynamics(GeneratedMidi &midi, const IntentResult &intent,
                     float dynamics);
   ...
   */
  void applyRuleBreaks(GeneratedMidi &midi, const IntentResult &intent);

  /**
   * Apply tension curve from TensionEngine.
   */
  void applyTension(GeneratedMidi &midi, const IntentResult &intent, int bars);

  /**
   * Apply variations using VariationEngine (post-processing).
   */
  void applyVariations(GeneratedMidi &midi, const IntentResult &intent,
                       float complexity);
