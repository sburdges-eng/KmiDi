...
/**
 * Groove Template - Predefined groove patterns with timing and velocity
 * characteristics. Based on genre-specific patterns (funk, jazz, rock, hiphop,
 * etc.)
 */
struct GrooveTemplateData {
  ...
  std::vector<float>
      timingDeviations;           // Per-16th-note timing offsets (in beats)
...
public:
   ...
   */
  std::vector<MidiNote> applyGroove(const std::vector<MidiNote> &notes,
   ...
   */
  std::vector<MidiNote> applyGrooveTemplate(const std::vector<MidiNote> &notes,
   ...
   * @return Swung notes
   */
  std::vector<MidiNote> applySwing(const std::vector<MidiNote> &notes,
                                   float swingAmount, float intensity = 1.0f);
   ...
   */
  std::vector<MidiNote> applyTimingFeel(const std::vector<MidiNote> &notes,
                                        float feel, float intensity = 1.0f);
   ...
   */
  std::vector<MidiNote> applyEmotionTiming(const std::vector<MidiNote> &notes,
...
private:
  ...
  float getTimingDeviation(const GrooveTemplateData &grooveTemplate,
                           double beatPosition) const;

  // Get velocity from template for a specific beat position
  int getTemplateVelocity(const GrooveTemplateData &grooveTemplate,
                          double beatPosition, int originalVelocity) const;
