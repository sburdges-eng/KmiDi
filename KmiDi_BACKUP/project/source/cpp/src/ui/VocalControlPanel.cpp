...
VocalControlPanel::VocalControlPanel() : currentVoiceType_(VoiceType::Neutral) {
  setupSliders();
...
void VocalControlPanel::resized() {
  ...
  // Voice type selector
  voiceTypeLabel_.setBounds(bounds.getX(), yPos, bounds.getWidth(),
                            labelHeight);
  yPos += labelHeight + spacing;
  voiceTypeComboBox_.setBounds(bounds.getX() + 10, yPos, bounds.getWidth() - 20,
                               25);
  yPos += 30 + spacing;
...
void VocalControlPanel::setupSliders() {
  ...
  vibratoDepthSlider_.setTextBoxStyle(juce::Slider::TextBoxRight, false, 60,
                                      20);
  ...
  articulationSlider_.setTextBoxStyle(juce::Slider::TextBoxRight, false, 60,
                                      20);
  articulationSlider_.onValueChange = [this] { updateExpressionFromSliders(); };
}

VoiceType VocalControlPanel::getVoiceType() const { return currentVoiceType_; }
...
void VocalControlPanel::setExpression(const VocalExpression &expression) {
  ...
  vibratoDepthSlider_.setValue(expression.vibratoDepth,
                               juce::dontSendNotification);
  vibratoRateSlider_.setValue(expression.vibratoRate,
                              juce::dontSendNotification);
  breathinessSlider_.setValue(expression.breathiness,
                              juce::dontSendNotification);
  ...
  articulationSlider_.setValue(expression.articulation,
                               juce::dontSendNotification);
}

void VocalControlPanel::updateExpressionFromSliders() {
  currentExpression_.vibratoDepth =
      static_cast<float>(vibratoDepthSlider_.getValue());
  currentExpression_.vibratoRate =
      static_cast<float>(vibratoRateSlider_.getValue());
  currentExpression_.breathiness =
      static_cast<float>(breathinessSlider_.getValue());
  currentExpression_.brightness =
      static_cast<float>(brightnessSlider_.getValue());
  currentExpression_.dynamics = static_cast<float>(dynamicsSlider_.getValue());
  currentExpression_.articulation =
      static_cast<float>(articulationSlider_.getValue());
