...
// Based on studies: Blue (focus), Green (balance/creativity), Purple
// (imagination)

// Background colors - Dark gray instead of pure black for reduced eye strain
const juce::Colour KellyLookAndFeel::backgroundDark(
    0xFF121212); // #121212 - Optimal dark mode background
const juce::Colour KellyLookAndFeel::backgroundLight(
    0xFF1E1E1E); // #1E1E1E - Slightly lighter for depth
const juce::Colour
    KellyLookAndFeel::surfaceColor(0xFF2A2A2A); // #2A2A2A - Elevated surfaces

// Primary colors based on research - Exact documented colors
const juce::Colour KellyLookAndFeel::primaryColor(
    0xFF3B82F6); // Focus Blue #3B82F6 - Focus & Productivity (85% stress
                 // reduction)
const juce::Colour KellyLookAndFeel::secondaryColor(
    0xFFA855F7); // Creative Purple #A855F7 - Creativity & Imagination (65%
                 // enhanced thinking)
const juce::Colour KellyLookAndFeel::accentColor(
    0xFF22C55E); // Balance Green #22C55E - Balance & Creative Solutions (40%
                 // increase)

// Text colors - Off-white for reduced contrast strain
const juce::Colour KellyLookAndFeel::textPrimary(
    0xFFE8E8E8); // #E8E8E8 - Off-white for readability
const juce::Colour
    KellyLookAndFeel::textSecondary(0xFFB8B8B8); // #B8B8B8 - Secondary text

// UI elements
const juce::Colour KellyLookAndFeel::borderColor(0xFF404040); // Subtle borders
const juce::Colour KellyLookAndFeel::successColor(
    0xFF22C55E); // Balance Green #22C55E - Success states
const juce::Colour
    KellyLookAndFeel::warningColor(0xFFF59E0B); // Amber - Warnings

// Glass aesthetic colors
const juce::Colour
    KellyLookAndFeel::glassBorder(0x20FFFFFF); // Glass border with transparency
const juce::Colour
    KellyLookAndFeel::glassHighlight(0x08FFFFFF); // Glass highlight
const juce::Colour KellyLookAndFeel::accentAlt(
    0xFFF472B6); // Pink for Side B / secondary accent
const juce::Colour
    KellyLookAndFeel::accentTertiary(0xFF22D3EE); // Cyan for tertiary accent
...
static const juce::Colour
    creativePurple(0xFFA855F7); // Vibrant purple for creative zones
...
void KellyLookAndFeel::drawButtonBackground(
  ...
  juce::ColourGradient highlightGrad(glassHighlight, bounds.getX(),
                                     bounds.getY(),
                                     juce::Colours::transparentBlack,
                                     bounds.getX(), bounds.getY() + 20, false);
  g.setGradientFill(highlightGrad);
  g.fillRoundedRectangle(bounds.removeFromTop(20), cornerSize);
}

void KellyLookAndFeel::drawButtonText(juce::Graphics &g,
...
void KellyLookAndFeel::drawLinearSlider(juce::Graphics &g, int x, int y,
  ...
  if (style == juce::Slider::LinearHorizontal ||
    ...
    if (isHorizontal) {
      ...
      g.fillRoundedRectangle((float)x, trackY, (float)width, trackWidth,
                             trackWidth * 0.5f);

      // Value track with gradient
      auto fillWidth = sliderPos - (float)x;
      if (fillWidth > 0) {
        juce::ColourGradient gradient(primaryColor, (float)x, trackY, accentAlt,
                                      sliderPos, trackY, false);
        g.setGradientFill(gradient);
        g.fillRoundedRectangle((float)x, trackY, fillWidth, trackWidth,
                               trackWidth * 0.5f);
      }

      // Glass border
      g.setColour(glassBorder);
      g.drawRoundedRectangle((float)x, trackY, (float)width, trackWidth,
                             trackWidth * 0.5f, 0.5f);
    } else {
      auto trackX = (float)x + (float)width * 0.5f - trackWidth * 0.5f;

      // Background track (glass)
      g.setColour(surfaceColor.withAlpha(0.6f));
      g.fillRoundedRectangle(trackX, (float)y, trackWidth, (float)height,
                             trackWidth * 0.5f);

      // Value track
      auto fillHeight = (float)y + (float)height - sliderPos;
      if (fillHeight > 0) {
        juce::ColourGradient gradient(accentAlt, trackX,
                                      (float)y + (float)height, primaryColor,
                                      trackX, sliderPos, false);
        g.setGradientFill(gradient);
        g.fillRoundedRectangle(trackX, sliderPos, trackWidth, fillHeight,
                               trackWidth * 0.5f);
      }

      // Glass border
      g.setColour(glassBorder);
      g.drawRoundedRectangle(trackX, (float)y, trackWidth, (float)height,
                             trackWidth * 0.5f, 0.5f);
    }
...
void KellyLookAndFeel::drawLinearSliderBackground(
  ...
  auto trackBounds =
      slider.isHorizontal()
          ? juce::Rectangle<float>(
                static_cast<float>(x), static_cast<float>(y + height * 0.4f),
                static_cast<float>(width), static_cast<float>(height * 0.2f))
          : juce::Rectangle<float>(
                static_cast<float>(x + width * 0.4f), static_cast<float>(y),
                static_cast<float>(width * 0.2f), static_cast<float>(height));

  drawModernSliderTrack(g, trackBounds,
                        slider.findColour(juce::Slider::trackColourId));
}

void KellyLookAndFeel::drawLinearSliderThumb(
    juce::Graphics &g, int x, int y, int width, int height, float sliderPos,
    float minSliderPos, float maxSliderPos,
    const juce::Slider::SliderStyle style, juce::Slider &slider) {
  // Larger thumb for better visibility and easier interaction
  auto thumbSize = slider.isHorizontal() ? static_cast<float>(height) * 1.0f
                                         : static_cast<float>(width) * 1.0f;
  if (thumbSize < 16.0f)
    thumbSize = 16.0f; // Minimum size for visibility
  auto thumbBounds =
      slider.isHorizontal()
          ? juce::Rectangle<float>(sliderPos - thumbSize * 0.5f,
                                   static_cast<float>(y) +
                                       (height - thumbSize) * 0.5f,
                                   thumbSize, thumbSize)
          : juce::Rectangle<float>(
                static_cast<float>(x) + (width - thumbSize) * 0.5f,
  ...
  if (sliderName.containsIgnoreCase("valence") ||
      sliderName.containsIgnoreCase("arousal")) {
    thumbColour = creativePurple; // Purple for emotion/creativity
  } else if (sliderName.containsIgnoreCase("complexity") ||
             sliderName.containsIgnoreCase("dynamics")) {
...
void KellyLookAndFeel::drawComboBox(juce::Graphics &g, int width, int height,
  ...
  juce::ColourGradient highlightGrad(glassHighlight, bounds.getX(),
                                     bounds.getY(),
                                     juce::Colours::transparentBlack,
                                     bounds.getX(), bounds.getY() + 30, false);
  g.setGradientFill(highlightGrad);
  g.fillRoundedRectangle(bounds.removeFromTop(30), cornerSize);

  // Border with focus state
  auto borderColour =
      box.hasKeyboardFocus(true) ? primaryColor.withAlpha(0.8f) : glassBorder;
  ...
  g.drawText(box.getText(), textBounds, juce::Justification::centredLeft,
             false);
...
void KellyLookAndFeel::drawTextEditorOutline(juce::Graphics &g, int width,
                                             int height,
...
void KellyLookAndFeel::drawToggleButton(juce::Graphics &g,
  ...
  auto bgColour =
      isOn ? balanceGreen : surfaceColor; // Balance Green #22C55E for active
                                          // (balance/creativity)
  ...
  // Subtle gradient
  juce::ColourGradient gradient(bgColour.brighter(0.1f), bounds.getTopLeft(),
...
juce::Font KellyLookAndFeel::getTextButtonFont(juce::TextButton &,
                                               int buttonHeight) {
  return juce::FontOptions(static_cast<float>(buttonHeight) * 0.55f)
      .withStyle("SemiBold"); // Larger
}
...
juce::Font KellyLookAndFeel::getSliderPopupFont(juce::Slider &) {
  return juce::FontOptions(16.0f).withStyle(
      "SemiBold"); // Large, bold for slider values
}
...
int KellyLookAndFeel::getTextButtonWidthToFitText(juce::TextButton &button,
                                                  int height) {
  return getTextButtonFont(button, height)
             .getStringWidth(button.getButtonText()) +
         height;
}

void KellyLookAndFeel::drawModernButton(juce::Graphics &g,
  ...
  juce::ColourGradient gradient(colour.brighter(0.15f), bounds.getTopLeft(),
...
void KellyLookAndFeel::drawModernSliderTrack(
    juce::Graphics &g, const juce::Rectangle<float> &trackBounds,
...
void KellyLookAndFeel::drawModernSliderThumb(
    juce::Graphics &g, const juce::Rectangle<float> &thumbBounds,
  ...
  juce::ColourGradient thumbGradient(
      ...
      juce::Colours::transparentBlack, thumbBounds.getCentre(), true);
  g.setGradientFill(thumbGradient);
  g.fillEllipse(thumbBounds.reduced(thumbBounds.getWidth() * 0.2f,
                                    thumbBounds.getHeight() * 0.2f));

  // Inner dot
  g.setColour(colour);
  g.fillEllipse(thumbBounds.reduced(thumbBounds.getWidth() * 0.4f,
                                    thumbBounds.getHeight() * 0.4f));
