 ...
 *
 * Purpose: Command pattern implementation for undo/redo functionality in MIDI
 * editor. All edit operations are wrapped in command objects for undo/redo
 * support.
 *
 * Features:
...
public:
  enum Property { Pitch, Velocity, StartTime, Duration, Channel };

  ModifyNoteCommand(GeneratedMidi &midi, size_t noteIndex, Property property,
                    float newValue);
  void execute() override;
  void undo() override;
...
public:
  MoveNoteCommand(GeneratedMidi &midi, size_t noteIndex, double newStartBeat,
                  int newPitch);
  void execute() override;
  void undo() override;
