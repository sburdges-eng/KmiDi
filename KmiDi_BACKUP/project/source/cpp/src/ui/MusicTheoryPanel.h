...
public:
  ...
  struct TheorySettings {
    ...
    bool strictCustomProgression =
        false; // If true, use EXACT progression with NO modifications
  };

  TheorySettings getSettings() const;

  /** Set custom progression (comma-separated degrees, e.g., "1,4,5,1") */
  void setCustomProgression(const juce::String &progression,
                            bool strict = false);
...
private:
  ...
  std::vector<int>
  parseProgressionString(const juce::String &progression) const;
