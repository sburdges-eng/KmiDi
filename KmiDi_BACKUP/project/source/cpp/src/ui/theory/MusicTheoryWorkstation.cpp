...
MusicTheoryWorkstation::MusicTheoryWorkstation(
    midikompanion::theory::MusicTheoryBrain *brain)
    : brain_(brain), conceptBrowser_(brain), learningPanel_(brain),
      virtualKeyboard_() {
...
void MusicTheoryWorkstation::setupTabs() {
  tabs_.addTab("Learning", juce::Colours::transparentBlack, &learningPanel_,
               false);
  tabs_.addTab("Concepts", juce::Colours::transparentBlack, &conceptBrowser_,
               false);
  tabs_.addTab("Analysis", juce::Colours::transparentBlack, &analysisDisplay_,
               false);
  tabs_.addTab("Practice", juce::Colours::transparentBlack, &practiceDisplay_,
               false);
  tabs_.addTab("Keyboard", juce::Colours::transparentBlack, &virtualKeyboard_,
               false);
...
void MusicTheoryWorkstation::analyzeMIDI(const juce::MidiFile &midi) {
  ...
  for (int track = 0; track < midi.getNumTracks(); ++track) {
    ...
    for (const auto *event : *sequence) {
      if (event->message.isNoteOn()) {
        ...
        onsetTimes.push_back(event->message.getTimeStamp() /
  ...
  result +=
      "Time Signature: " + juce::String(analysis.timeSignature.numerator) +
...
void MusicTheoryWorkstation::startPracticeSession() {
  ...
  result +=
      "Duration: " + juce::String(session.estimatedDuration) + " minutes\n\n";
