...
void LearningPanel::resized() {
  ...
  // Title at top
  conceptTitle_.setBounds(margin, margin, bounds.getWidth() - 2 * margin,
                          titleHeight);
  ...
  // Buttons
  playExampleButton_.setBounds(bounds.getWidth() - buttonWidth - margin,
                               controlTop, buttonWidth, controlHeight);
  nextExerciseButton_.setBounds(bounds.getWidth() - 2 * buttonWidth -
                                    2 * margin,
                                controlTop, buttonWidth, controlHeight);
...
void LearningPanel::setupComponents() {
  ...
  conceptTitle_.setText("Select a concept to learn",
  ...
  styleSelector_.onChange = [this] {
    ...
    if (selected == 1)
      currentStyle_ = midikompanion::theory::ExplanationType::Intuitive;
    else if (selected == 2)
      currentStyle_ = midikompanion::theory::ExplanationType::Mathematical;
    else if (selected == 3)
      currentStyle_ = midikompanion::theory::ExplanationType::Historical;
    else if (selected == 4)
      currentStyle_ = midikompanion::theory::ExplanationType::Acoustic;
  ...
  explanationDisplay_.setText(
      "Select a concept from the Concepts tab to begin learning.");
...
void LearningPanel::displayExplanation(
    const std::string &text, midikompanion::theory::ExplanationType style) {
  juce::String displayText;
  displayText +=
      "Style: " +
      juce::String(style == midikompanion::theory::ExplanationType::Intuitive
                       ? "Intuitive"
                   : style ==
                           midikompanion::theory::ExplanationType::Mathematical
                       ? "Mathematical"
                   : style == midikompanion::theory::ExplanationType::Historical
                       ? "Historical"
                   : style == midikompanion::theory::ExplanationType::Acoustic
                       ? "Acoustic"
                       : "Practical") +
      "\n\n";
...
void LearningPanel::setMusicTheoryBrain(
    midikompanion::theory::MusicTheoryBrain *brain) {
  brain_ = brain;
...
void LearningPanel::setExplanationStyle(
...
void LearningPanel::loadExplanation(const std::string &conceptName) {
  ...
  if (!conceptNode.has_value()) {
    explanationDisplay_.setText("Concept not found: " +
                                juce::String(conceptName));
    return;
  }
...
void LearningPanel::playCurrentConceptExample() {
  ...
  if (!examples.empty()) {
    exampleDescription = "Example: " + juce::String(examples.front().song) +
                         " @ " + juce::String(examples.front().timestamp, 2) +
...
void LearningPanel::displayExercise(
    const midikompanion::theory::Exercise &exercise) {
  juce::String text;
...
std::vector<int> LearningPanel::buildExampleNotes() const {
  if (!currentExercise_.notes.empty() &&
      currentExercise_.conceptName == currentConcept_) {
