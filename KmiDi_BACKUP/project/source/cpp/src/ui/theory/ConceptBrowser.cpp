...
void ConceptBrowser::resized() {
  ...
  // Search box at top
  searchLabel_.setBounds(margin, margin, bounds.getWidth() - 2 * margin,
                         labelHeight);
  searchBox_.setBounds(margin, margin + labelHeight,
                       bounds.getWidth() - 2 * margin, searchHeight);

  // Concept list in middle
  int listTop = margin + labelHeight + searchHeight + margin;
  int listHeight =
      bounds.getHeight() - listTop - detailsHeight - labelHeight - 2 * margin;
  conceptList_.setBounds(margin, listTop, bounds.getWidth() - 2 * margin,
                         listHeight);

  // Details at bottom
  int detailsTop = listTop + listHeight + margin;
  detailsLabel_.setBounds(margin, detailsTop, bounds.getWidth() - 2 * margin,
                          labelHeight);
...
void ConceptBrowser::setupComponents() {
  ...
  searchBox_.onTextChange = [this] { searchConcepts(searchBox_.getText()); };
...
void ConceptBrowser::setMusicTheoryBrain(
...
void ConceptBrowser::loadConceptsFromGraph() {
  ...
  for (const auto &category : categories) {
    ...
    for (const auto &conceptNode : concepts) {
      ConceptItem item;
      item.name =
          conceptNode.conceptName; // KnowledgeNode uses 'conceptName' field
      item.category = category;
      item.prerequisites = knowledge.getPrerequisites(conceptNode.conceptName);

      // getRelatedConcepts returns ConceptRelationship structs, extract concept
      // names
      auto relatedRelationships =
          knowledge.getRelatedConcepts(conceptNode.conceptName);
