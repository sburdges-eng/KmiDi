...
LyricDisplay::LyricDisplay() : fontSize_(16.0f), highlightSyllables_(true) {
  setOpaque(true);
...
void LyricDisplay::paintLine(juce::Graphics &g, const LyricLine &line,
                             int lineIndex, const juce::Rectangle<int> &bounds,
                             bool isHighlighted) {
...
void LyricDisplay::paintSyllables(juce::Graphics &g, const LyricLine &line,
  ...
  for (size_t i = 0; i < line.syllables.size(); ++i) {
    ...
    g.setFont(fontSize_ *
              (syllable.stress == 2 ? 1.1f : 1.0f)); // Larger for stressed
    ...
    if (syllable.stress > 0) {
      g.drawLine(static_cast<float>(xOffset), static_cast<float>(yPos + 2),
                 static_cast<float>(xOffset + textWidth),
...
int LyricDisplay::getCurrentLineIndex() const {
  ...
  // (This is simplified - in a full implementation, we'd track which item
