...
void SuggestionOverlay::resized() { layoutCards(); }

void SuggestionOverlay::setSuggestions(
...
void SuggestionOverlay::rebuildCards() {
  ...
  for (const auto &suggestion : suggestions_) {
    ...
    card->titleLabel->setColour(juce::Label::textColourId,
                                juce::Colours::white);
    addAndMakeVisible(card->titleLabel.get());

    // Description label
    card->descriptionLabel = std::make_unique<juce::Label>();
    card->descriptionLabel->setText(suggestion.description,
    ...
    card->applyButton->setColour(juce::TextButton::buttonColourId,
                                 juce::Colour(0xFF4CAF50));
    card->applyButton->setColour(juce::TextButton::textColourOffId,
                                 juce::Colours::white);
    int cardIndex = static_cast<int>(cards_.size());
    card->applyButton->onClick = [this, cardIndex] {
      applyButtonClicked(cardIndex);
    };
    addAndMakeVisible(card->applyButton.get());

    // Dismiss button
    card->dismissButton = std::make_unique<juce::TextButton>("×");
    card->dismissButton->setColour(juce::TextButton::buttonColourId,
                                   juce::Colour(0xFF666666));
    card->dismissButton->setColour(juce::TextButton::textColourOffId,
                                   juce::Colours::white);
    card->dismissButton->onClick = [this, cardIndex] {
      dismissButtonClicked(cardIndex);
    };
    addAndMakeVisible(card->dismissButton.get());

    // Expand button (for explanation)
    card->expandButton = std::make_unique<juce::TextButton>("...");
    card->expandButton->setColour(juce::TextButton::buttonColourId,
                                  juce::Colour(0xFF555555));
    card->expandButton->setColour(juce::TextButton::textColourOffId,
                                  juce::Colour(0xFFCCCCCC));
    card->expandButton->onClick = [this, cardIndex] {
      expandButtonClicked(cardIndex);
    };
...
void SuggestionOverlay::layoutCards() {
  ...
  for (auto &card : cards_) {
    ...
    if (card->expanded) {
      card->explanationLabel->setBounds(cardX + 15, y + 105, cardWidth - 30,
                                        40);
...
void SuggestionOverlay::expandButtonClicked(int cardIndex) {
  if (cardIndex >= 0 && cardIndex < static_cast<int>(cards_.size()) &&
    ...
    cards_[cardIndex]->explanationLabel->setVisible(
        cards_[cardIndex]->expanded);
    layoutCards();
    repaint();
  }
}
