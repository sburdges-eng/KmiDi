 ...
 *          Provides consistent visual styling across the entire plugin
 * interface.
 *
 * Features:
...
public:
  ...
  juce::Colour getHoverColor() const {
    return juce::Colour(0x14FFFFFF);
  } // rgba(255,255,255,0.08)
  juce::Colour getActiveColor() const {
    return juce::Colour(0x1FFFFFFF);
  } // rgba(255,255,255,0.12)
  juce::Colour getFocusColor() const { return primaryColor; }
  juce::Colour getFocusRingColor() const {
    return primaryColor.withAlpha(0.3f);
  }
  ...
  int getTextButtonWidthToFitText(juce::TextButton &button,
                                  int height) override;
...
private:
  void drawModernButton(juce::Graphics &g, const juce::Rectangle<float> &bounds,
                        const juce::Colour &baseColour, bool isHighlighted,
                        bool isDown);

  void drawModernSliderTrack(juce::Graphics &g,
                             const juce::Rectangle<float> &trackBounds,
                             const juce::Colour &trackColour);

  void drawModernSliderThumb(juce::Graphics &g,
                             const juce::Rectangle<float> &thumbBounds,
                             const juce::Colour &thumbColour,
                             bool isHighlighted);
