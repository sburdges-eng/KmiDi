...
ModifyNoteCommand::ModifyNoteCommand(GeneratedMidi &midi, size_t noteIndex,
                                     Property property, float newValue)
    : midi_(midi), noteIndex_(noteIndex), property_(property),
      newValue_(newValue), oldValue_(0.0f) {
  if (noteIndex_ < midi_.notes.size()) {
...
void ModifyNoteCommand::execute() {
  ...
  case Velocity:
    note.velocity =
        static_cast<int>(juce::jlimit(0, 127, static_cast<int>(newValue_)));
    break;
  case StartTime:
  ...
  case Channel:
    note.channel =
        static_cast<int>(juce::jlimit(0, 15, static_cast<int>(newValue_)));
    break;
  }
}
...
MoveNoteCommand::MoveNoteCommand(GeneratedMidi &midi, size_t noteIndex,
                                 double newStartBeat, int newPitch)
    : midi_(midi), noteIndex_(noteIndex), newStartBeat_(newStartBeat),
      newPitch_(newPitch), oldStartBeat_(0.0), oldPitch_(60) {
  if (noteIndex_ < midi_.notes.size()) {
    const auto &note = midi_.notes[noteIndex_];
