...
int dynamicToVelocity(Dynamic dynamic) {
  switch (dynamic) {
  case Dynamic::Pianissimo:
    return 40;
  case Dynamic::Piano:
    return 55;
  case Dynamic::MezzoPiano:
    return 70;
  case Dynamic::MezzoForte:
    return 90;
  case Dynamic::Forte:
    return 110;
  case Dynamic::Fortissimo:
    return 120;
  default:
    return 90;
  }
...
int defaultBeatsPerBar(const std::vector<TimeSignatureChange> &timeSigs,
                       int measure) {
  if (timeSigs.empty()) {
    return 4;
  }
...
ScoreEntryPanel::ScoreEntryPanel()
    : entryMode_(EntryMode::Standard), viewMode_(ViewMode::SingleStaff),
      currentClef_(Clef::Treble), currentNoteValue_(NoteValue::Quarter),
      currentDynamic_(Dynamic::MezzoForte),
      currentArticulation_(Articulation::None), cursorMeasure_(1),
      cursorBeat_(1.0f), showChordSymbols_(true), showLyrics_(false),
      showDynamics_(true), zoomFactor_(1.0f) {
...
void ScoreEntryPanel::initializeComponents() {
  ...
  entryModeSelector_->onChange = [this] {
    ...
    if (id == 1)
      setEntryMode(EntryMode::Simple);
    else if (id == 2)
      setEntryMode(EntryMode::Standard);
    else if (id == 3)
      setEntryMode(EntryMode::Professional);
    else if (id == 4)
      setEntryMode(EntryMode::Chord);
  };
  ...
  auto makeNoteButton = [this](std::unique_ptr<juce::TextButton> &btn,
                               const juce::String &text, NoteValue value) {
  ...
  auto makeDynamicButton = [this](std::unique_ptr<juce::TextButton> &btn,
                                  const juce::String &text, Dynamic dyn) {
    btn = std::make_unique<juce::TextButton>(text);
  ...
  auto makeArticulationButton = [this](std::unique_ptr<juce::TextButton> &btn,
...
void ScoreEntryPanel::paint(juce::Graphics &g) {
  ...
  auto scoreArea = bounds.removeFromBottom(
      std::max(120, bounds.getHeight() - controlHeight));
  ...
  for (const auto &note : notes_) {
    int measureWidth =
        std::max(120, static_cast<int>(scoreArea.getWidth() /
                                       std::max(1, getMaxMeasureIndex())));
    int beatsPerBar = defaultBeatsPerBar(timeSignatures_, note.measure);
    float beatOffset = (note.beat - 1.0f) / std::max(1, beatsPerBar);
    int x =
        scoreArea.getX() + static_cast<int>((note.measure - 1) * measureWidth +
                                            beatOffset * measureWidth + 20);
  ...
  if (showChordSymbols_) {
    for (const auto &chord : chordSymbols_) {
      int measureWidth =
          std::max(120, static_cast<int>(scoreArea.getWidth() /
                                         std::max(1, getMaxMeasureIndex())));
      float beatOffset = (chord.beat - 1.0f) /
                         defaultBeatsPerBar(timeSignatures_, chord.measure);
      int x = scoreArea.getX() +
              static_cast<int>((chord.measure - 1) * measureWidth +
                               beatOffset * measureWidth + 12);
...
void ScoreEntryPanel::mouseDown(const juce::MouseEvent &event) {
  ...
  addNote(pitch, currentNoteValue_,
          dottedButton_ && dottedButton_->getToggleState());
}
...
void ScoreEntryPanel::setEntryMode(EntryMode mode) { entryMode_ = mode; }
...
void ScoreEntryPanel::setTimeSignature(int numerator, int denominator) {
  ...
  timeSignatureLabel_->setText(juce::String(numerator) + "/" +
...
void ScoreEntryPanel::setTempo(float bpm, const std::string &description) {
  ...
  tempoLabel_->setText(juce::String(bpm, 1) + " BPM",
                       juce::dontSendNotification);
}
...
void ScoreEntryPanel::addChord(const std::vector<int> &pitches,
                               NoteValue duration) {
...
void ScoreEntryPanel::addRest(NoteValue duration) {
  float beats = noteValueToBeats(duration);
  if (dottedButton_ && dottedButton_->getToggleState())
    beats *= 1.5f;
  if (tripletButton_ && tripletButton_->getToggleState())
    beats *= (2.0f / 3.0f);
...
void ScoreEntryPanel::setDynamic(Dynamic dynamic) { currentDynamic_ = dynamic; }
...
void ScoreEntryPanel::toggleDot() {
  if (!notes_.empty()) {
    ...
    dottedButton_->setToggleState(!dottedButton_->getToggleState(),
                                  juce::dontSendNotification);
  }
}
...
void ScoreEntryPanel::moveCursorBackward() {
  ...
  if (cursorBeat_ < 1.0f) {
    ...
    cursorBeat_ =
        static_cast<float>(defaultBeatsPerBar(timeSignatures_, cursorMeasure_));
  }
  repaint();
}
...
std::vector<ScoreEntryPanel::ScoreTemplate>
ScoreEntryPanel::getAvailableTemplates() const {
  return templates_;
}
...
juce::MidiBuffer ScoreEntryPanel::toMidiBuffer() const {
  ...
  for (const auto &note : notes_) {
    ...
    buffer.addEvent(
        ...
        startTick);
    buffer.addEvent(juce::MidiMessage::noteOff(channel, note.pitch),
  ...
  for (const auto metadata : buffer) {
    ...
    if (msg.isNoteOn()) {
      ...
      if (it != noteOnTick.end()) {
        ...
        note.beat =
            ((start % ticksPerBar) / static_cast<float>(ticksPerQuarter)) +
            1.0f;
        notes_.push_back(note);
        noteOnTick.erase(it);
      }
...
void ScoreEntryPanel::playFromCursor() { onPlayClicked(); }

void ScoreEntryPanel::stop() { onStopClicked(); }

void ScoreEntryPanel::toggleMetronome() { onMetronomeToggled(); }
...
bool ScoreEntryPanel::exportMusicXML(const juce::File &outputFile) {
  ...
  const auto durationToDivisions = [](const NotationNote &note) -> int {
    ...
    const float beats = noteValueToBeats(note.duration) *
                        (note.dotted ? 1.5f : 1.0f) *
                        (note.triplet ? 2.0f / 3.0f : 1.0f);
  ...
  const auto durationToType = [](NoteValue value) -> const char * {
    ...
    case NoteValue::ThirtySecond:
      return "32nd";
    case NoteValue::Dotted:
      return "quarter"; // fallback; dotted flag handled separately
    default:
      return "quarter";
    }
  };
  ...
  for (int measure = 1; measure <= maxMeasure; ++measure) {
    ...
    for (const auto &chord : chordSymbols_) {
      if (chord.measure != measure)
        continue;
      stream << "      <harmony><root><root-step>" << chord.symbol
             << "</root-step></root></harmony>\n";
    }
    stream << "    </measure>\n";
  }
...
bool ScoreEntryPanel::exportPDF(const juce::File &outputFile) {
  ...
  for (const auto &note : notes_) {
    stream << "Note " << note.pitch << " dur "
           << noteValueToBeats(note.duration) << " beat " << note.beat
           << " bar " << note.measure;
    if (!note.lyric.empty())
      stream << " lyric \"" << note.lyric << "\"";
    stream << "\n";
  }
  stream.flush();
  return true;
...
bool ScoreEntryPanel::exportMIDI(const juce::File &outputFile) {
  ...
  for (const auto &note : notes_) {
    ...
    seq.addEvent(
        juce::MidiMessage::noteOn(channel, note.pitch, (juce::uint8)velocity),
        startTick);
    seq.addEvent(juce::MidiMessage::noteOff(channel, note.pitch),
                 startTick + durationTicks);
  }
...
void ScoreEntryPanel::drawStaff(juce::Graphics &g, juce::Rectangle<int> area) {
  ...
  const int measureWidth =
      std::max(120, static_cast<int>(area.getWidth() / totalMeasures));
...
void ScoreEntryPanel::drawClef(juce::Graphics &g, Clef clef,
  ...
  if (clef == Clef::Bass)
    ...
    clefSymbol = "C";
  else if (clef == Clef::Tenor)
    clefSymbol = "C";
  g.setColour(juce::Colours::white);
  g.setFont(juce::Font(22.0f));
  g.drawText(clefSymbol, position.x, position.y, 20, 40,
             juce::Justification::centred);
}

void ScoreEntryPanel::drawTimeSignature(juce::Graphics &g, int numerator,
                                        int denominator,
                                        juce::Point<int> position) {
  g.setColour(juce::Colours::white);
  g.setFont(juce::Font(18.0f));
  g.drawFittedText(juce::String(numerator) + "\n" + juce::String(denominator),
                   position.x, position.y, 20, 40, juce::Justification::centred,
                   2);
}

void ScoreEntryPanel::drawKeySignature(juce::Graphics &g,
...
void ScoreEntryPanel::drawNote(juce::Graphics &g, const NotationNote &note,
  ...
  g.setColour(juce::Colours::white);
  const int size = 12;
  g.fillEllipse((float)position.x, (float)position.y - size / 2.0f, (float)size,
                (float)size);

  if (note.tie) {
    g.setColour(juce::Colours::lightgrey);
    g.drawArc((float)position.x, (float)position.y - 6.0f, (float)size + 12.0f,
              12.0f, juce::MathConstants<float>::pi,
              2.0f * juce::MathConstants<float>::pi, 1.0f);
  }

  if (showLyrics_ && !note.lyric.empty()) {
    g.setColour(juce::Colours::lightgrey);
    g.drawText(note.lyric, position.x - 6, position.y + 14, 40, 16,
               juce::Justification::left);
  }
}

void ScoreEntryPanel::drawChordSymbol(juce::Graphics &g,
                                      const ChordSymbol &chord,
                                      juce::Point<int> position) {
  g.setColour(juce::Colours::lightgreen);
  g.setFont(juce::Font(14.0f, juce::Font::bold));
  g.drawText(chord.symbol, position.x, position.y, 60, 20,
             juce::Justification::left);
}
...
void ScoreEntryPanel::drawCursor(juce::Graphics &g) {
  ...
  int x = area.getX() + static_cast<int>((cursorMeasure_ - 1) * measureWidth +
                                         beatOffset * measureWidth);
...
int ScoreEntryPanel::staffPositionToPitch(int yPosition, Clef clef) const {
  ...
  int semitoneSteps =
      static_cast<int>(std::round((centerLine - yPosition) / halfStep));
...
void ScoreEntryPanel::onPlayClicked() {
  if (onPlayRequested) {
    ...
    juce::Logger::writeToLog("ScoreEntryPanel: play from measure " +
                             juce::String(cursorMeasure_));
  }
}
...
void ScoreEntryPanel::onMetronomeToggled() {
  ...
  if (onMetronomeToggledCallback) {
    ...
    juce::Logger::writeToLog(juce::String("Metronome ") +
