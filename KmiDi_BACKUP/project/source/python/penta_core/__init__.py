...
try:
    ...
    if so_name:
        ...
        spec = importlib.util.spec_from_file_location(
            "penta_core_native", os.path.join(pkg_dir, so_name)
        )
...
class HarmonyEngine:
    ...
    def suggest_voice_leading(self, target_chord_root: int, current_voices: List[int]) -> List[int]:
        """
...
class GrooveEngine:
    ...
    def __init__(
        self, sample_rate: float = 48000.0, min_tempo: float = 60.0, max_tempo: float = 180.0
    ):
        if native is None:
            raise RuntimeError("Native C++ module not available")
...
__all__ = ["HarmonyEngine", "GrooveEngine", "DiagnosticsEngine", "OSCHub", "PentaCore"]
