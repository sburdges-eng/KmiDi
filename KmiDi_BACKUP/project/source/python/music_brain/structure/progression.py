    ...
    for i, chord in enumerate(chords):
        ...
        if interval not in scale:
            if interval == 3 and mode == "major":
                ...
                issues.append(
                    f"{chord.original}: non-diatonic root ({NOTE_NAMES[interval]} in {key_name} {mode})"
                )
        else:
        ...
        if i > 0:
            ...
            if root_motion == 6:  # Tritone motion
                suggestions.append(
                    f"Tritone motion between {prev_chord.original} and {chord.original} - can feel unstable"
                )
