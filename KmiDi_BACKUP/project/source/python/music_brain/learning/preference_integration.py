...
class PreferenceTracker:
    ...
    def on_parameter_change(self, parameter_name: str, old_value: float, new_value: float):
    ...
    def on_generation_accepted(self, parameters: Dict[str, float], emotion: Optional[str] = None):
        ...
        self.model.record_generation(parameters=parameters, emotion=emotion, accepted=True)

    def on_generation_rejected(self, parameters: Dict[str, float], emotion: Optional[str] = None):
        """Record rejected generation."""
        self.model.record_generation(parameters=parameters, emotion=emotion, accepted=False)

    def on_rule_break_added(self, rule_break_type: str):
        """Record rule-break addition."""
        self.model.record_rule_break_modification(
            rule_break_type=rule_break_type, action="added", context=self.current_context.copy()
        )

    def on_rule_break_removed(self, rule_break_type: str):
        """Record rule-break removal."""
        self.model.record_rule_break_modification(
            rule_break_type=rule_break_type, action="removed", context=self.current_context.copy()
        )

    def on_midi_edit(self, edit_type: str, target_part: str, change_details: Dict[str, Any]):
        """Record MIDI edit."""
        self.model.record_midi_edit(
            edit_type=edit_type, target_part=target_part, change_details=change_details
        )
