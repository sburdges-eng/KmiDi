...
# Import DAiW modules
from music_brain.groove import apply_groove, extract_groove
        ...
        return [
            Tool(
                ...
                inputSchema={
                    "type": "object",
                    "properties": {
                        "midi_file": {"type": "string", "description": "Path to MIDI file"}
                    },
                    "required": ["midi_file"],
                },
            ),
            Tool(
                ...
                inputSchema={
                    "type": "object",
                    "properties": {
                        "midi_file": {"type": "string", "description": "Path to input MIDI file"},
                        "genre": {
                            "type": "string",
        ...
        try:
            if name == "extract_groove":
                ...
                if not Path(midi_file).exists():
                    return [
                        TextContent(type="text", text=f"Error: MIDI file not found: {midi_file}")
                    ]
                ...
                try:
                    ...
                    for track in mid.tracks:
                        ...
                        for msg in track:
                            if msg.type in ["note_on", "note_off"]:
                                ...
                                blended_tick = int(
                                    current_tick * (1.0 - strength) + quantized_tick * strength
                                )
                                ...
                                # Track statistics
                                if (
                                    abs(current_tick - quantized_tick) > mid.ticks_per_beat // 32
                    ...
                    result = {"input_file": midi_file, "error": str(e), "status": "error"}
