    ...
    def score_template(
        ...
        if energy_diff < 0.2:
            ...
            mismatch_reasons.append(
                f"Energy mismatch (template {'higher' if template_energy > profile_energy else 'lower'})"
            )
        ...
        if swing_diff < 0.05:
            ...
            mismatch_reasons.append(
                f"Swing mismatch ({self._classify_swing(template_swing).value} vs {profile.swing_feel.value})"
            )
        ...
        if profile.ghost_density > 0.15 and template_ghosts > 0.15:
            match_reasons.append("Both use ghost notes")
