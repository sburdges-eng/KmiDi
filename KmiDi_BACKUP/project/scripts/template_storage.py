...
class TemplateMerger:
    ...
    def merge(self, templates: List[dict]) -> dict:
        ...
        if "push_pull" in templates[0]:
            merged["push_pull"] = self._merge_nested_dicts(
        ...
        # Add merge metadata
        merged["_merged"] = {"count": len(templates), "timestamp": datetime.now().isoformat()}
...
class TemplateStore:
    ...
    def save(
        ...
        with open(version_file, "w") as f:
            fcntl.flock(f.fileno(), fcntl.LOCK_EX)
            json.dump({"metadata": asdict(meta), "template": template}, f, indent=2)
            fcntl.flock(f.fileno(), fcntl.LOCK_UN)
    ...
    def load(
        self, genre: str, version: Optional[int] = None, target_ppq: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        if version is None:
            ...
            if latest.exists():
                ...
                raise FileNotFoundError(
                    f"No template for '{genre}'. Available: {', '.join(self.list_genres())}"
                )
        else:
            version_file = genre_dir / f"v{version:03d}.json"
    ...
    def _prune_old_versions(self, genre_dir: Path):
        ...
        if len(versions) > self.MAX_VERSIONS:
            ...
            metadata["versions"] = [
                v for v in metadata["versions"] if f"v{v['version']:03d}" in kept_versions
            ]
            self._save_metadata(genre_dir, metadata)
