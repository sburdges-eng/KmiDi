...
class DAiWGenerator:
    ...
    def _generate_track(self, name: str, data: dict) -> MidiTrack:
        ...
        for event in pattern:
            ...
            track.append(
                Message("note_on", note=pitch, velocity=velocity, time=delta, channel=channel)
            )
            track.append(
...
class DAiWMenubarApp(rumps.App):
    ...
    def process_instruction(self, json_path: Path):
        ...
        try:
            ...
            rumps.notification(title="DAiW Error", subtitle="Generation failed", message=str(e))
            self.title = "🎵❌"
...
if __name__ == "__main__":
    print("DAiW Listener starting...")
    print(f"Output: {OUTPUT_DIR}")
    print(f"Dropbox: {DROPBOX_DIR}")
    print("Drop .json instruction files into the dropbox folder.")
