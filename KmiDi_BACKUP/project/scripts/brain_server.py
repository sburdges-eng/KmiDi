...
class DAiWBrainServer:
    ...
    def __init__(self, listen_port: int = 9000, send_port: int = 9001):
        ...
        if not OSC_AVAILABLE:
            raise RuntimeError("pythonosc is required. Install with: pip install python-osc")
    ...
    def _handle_generate(self, address: str, *args: Any) -> None:
        """
        ...
        try:
            ...
            if len(args) < 4:
                self._send_error(
                    "Invalid arguments. Expected: text, motivation, chaos, vulnerability"
                )
                return
    ...
    def start(self) -> None:
        ...
        try:
            self.server = osc_server.ThreadingOSCUDPServer(
                ("127.0.0.1", self.listen_port), self.dispatcher
            )

            print("[INFO] DAiW Brain Server starting...")
            print(f"[INFO] Listening on port {self.listen_port}")
            print(f"[INFO] Sending to port {self.send_port}")
            print("[INFO] Ready to receive OSC messages")
...
def main():
    ...
    parser.add_argument(
        "--send-port", type=int, default=9001, help="Port to send OSC responses (default: 9001)"
    )
    ...
    server = DAiWBrainServer(listen_port=args.listen_port, send_port=args.send_port)
