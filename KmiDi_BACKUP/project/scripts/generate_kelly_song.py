...
def generate_complete_arrangement(intent_path, output_dir):
    ...
    print("✓ Sections: Intro → Verse → Chorus → Verse → Chorus → Bridge → Chorus → Outro")
    ...
    print(
        f"   Emotional arc: {intent_data['song_intent']['mood_primary']} with {intent_data['song_intent']['narrative_arc']}"
    )
...
def generate_rule_breaking_examples(output_dir):
    ...
    for example in examples:
        ...
        # Chord track
        mid.tracks.append(
            create_chord_track(example["chords"], example["key"], bars_per_chord=2, tempo_bpm=90)
        )
...
if __name__ == "__main__":
    ...
    intent_path = (
        repo_root / "examples_music-brain" / "intents" / "kelly_when_i_found_you_sleeping.json"
    )
