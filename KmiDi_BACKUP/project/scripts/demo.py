...
def demo_timing_pockets():
    ...
    if dilla_pocket:
        ...
        print(
            f"   Overall feel: {'dragging' if dilla_pocket.push_pull_tendency > 0 else 'rushing'} "
    ...
    for genre in genres:
        ...
        if pocket:
            feel = (
                "dragging"
                if pocket.push_pull_tendency > 5
                else "rushing"
                if pocket.push_pull_tendency < -5
                else "locked"
            )
            print(
                f"   {genre:<12} {pocket.swing_ratio:<8.2f} "
...
def demo_integration():
    ...
    if pocket:
        print(
            f"   Motown pocket: {pocket.swing_ratio:.2f} swing, "
    ...
    if pocket:
        ...
        for orig, modified in zip(midi_with_vel, timed_kicks):
            print(
                f"   {orig[1]:>6.1f}ms → {modified[1]:>6.1f}ms "
