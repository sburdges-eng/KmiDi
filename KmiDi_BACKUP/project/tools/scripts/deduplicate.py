...
class DuplicateFinder:
    ...
    def __init__(self, root_path: str, exclude_patterns: List[str] = None):
        ...
        self.exclude_patterns = exclude_patterns or [
            ...
            ".pytest_cache",
            "external",
            "JUCE",  # Exclude external dependencies
        ]
    ...
    def scan_directory(self):
        ...
        for filepath in self.root_path.rglob("*"):
            ...
            file_info = FileInfo(path=str(rel_path), size=size, md5=md5, sha256=sha256)
    ...
    def find_exact_duplicates(self):
        ...
        for sha256, files in self.files_by_hash.items():
            ...
            recommendation = f"Keep: {files[0].path}\nDelete: " + ", ".join(
                f.path for f in files[1:]
            )

            self.duplicate_groups.append(
                DuplicateGroup(files=files, reason="exact_match", recommendation=recommendation)
            )
            count += 1
    ...
    def find_semantic_duplicates(self):
        ...
        for normalized, files in content_groups.items():
            ...
            files.sort(key=lambda f: len(f.path))
            recommendation = f"Keep: {files[0].path}\nDelete or merge: " + ", ".join(
                f.path for f in files[1:]
            )

            self.duplicate_groups.append(
                DuplicateGroup(files=files, reason="semantic_match", recommendation=recommendation)
            )
            count += 1
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Find and analyze duplicate files in a repository")
    parser.add_argument(
        "--scan", type=str, default=".", help="Directory to scan (default: current directory)"
    )
