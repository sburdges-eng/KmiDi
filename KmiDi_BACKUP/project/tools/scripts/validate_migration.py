...
class MigrationValidator:
    ...
    def validate_build_system(self) -> bool:
        ...
        # Configure
        result = subprocess.run(
            ["cmake", "-B", str(build_dir), "-DBUILD_TESTS=OFF", "-DBUILD_PLUGINS=OFF"],
            cwd=self.repo_path,
            capture_output=True,
            text=True,
        )
        ...
        # Build
        result = subprocess.run(
            ["cmake", "--build", str(build_dir)], cwd=self.repo_path, capture_output=True, text=True
        )
    ...
    def generate_report(self) -> str:
        ...
        lines = ["# Migration Validation Report", "", "## Summary", ""]
