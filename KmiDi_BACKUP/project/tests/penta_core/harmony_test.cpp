...
TEST_F(PerformanceBenchmark, DISABLED_ChordAnalysisUnder50Microseconds)
{
...
TEST_F(PerformanceBenchmark, DISABLED_SIMDFasterThanScalar)
{
...
TEST_F(ChordAnalyzerTest, AnalyzesPitchClassSet)
{
...
TEST(ScaleDetectorTest, DetectsCMajorScale)
{
    ...
    // A pitch-class *set* (boolean) does not encode tonic/mode. Many diatonic modes
    // share the same set {C D E F G A B}, so the detector may pick different tonics.
    // Verify that the detected scale matches the provided set and has nonzero confidence.
    EXPECT_GT(scale.confidence, 0.0f);
    EXPECT_TRUE(pitchClasses[scale.tonic]);
    EXPECT_EQ(scale.degrees, pitchClasses);
}
