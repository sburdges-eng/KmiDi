...
public:
    ...
    MockAudioDevice() : MockAudioDevice(Config{}) {}

    explicit MockAudioDevice(const Config &config)
        : config_(config), running_(false), callbackCount_(0)
    {
    }
...
protected:
    ...
    {
        PluginTestHarness::SetUp();
        server_ = std::make_unique<osc::OSCServer>("127.0.0.1", 9001);
