...
def main():
    ...
    print("\n[Phase 2: Technical Constraints]")
    print(
        f"  Key: {kelly_intent.technical_constraints.technical_key} {kelly_intent.technical_constraints.technical_mode}"
    )
    print(
        f"  Tempo: {kelly_intent.technical_constraints.technical_tempo_range[0]}-{kelly_intent.technical_constraints.technical_tempo_range[1]} BPM"
    )
    ...
    print("\n  Dynamics:")
    print(
        f"    Compression: {music.mixer_params.compression_ratio:.1f}:1 @ {music.mixer_params.compression_threshold:.0f}dB"
    )
    print(f"    Attack: {music.mixer_params.compression_attack:.0f}ms")
    print(f"    Release: {music.mixer_params.compression_release:.0f}ms")
    print("\n  Space:")
    print(
        f"    Reverb: {music.mixer_params.reverb_mix:.0%} mix, {music.mixer_params.reverb_decay:.1f}s decay"
    )
    print(f"    Reverb Type: {music.mixer_params.reverb_type}")
    print(
        f"    Delay: {music.mixer_params.delay_mix:.0%} mix, {music.mixer_params.delay_time:.0f}ms"
    )
    print("\n  Character:")
    print(
        f"    Saturation: {music.mixer_params.saturation:.0%} ({music.mixer_params.saturation_type})"
    )
...
""")
    ...
    result = (
        brain.process("grief and loss, slow reveal of pain")
        .map_to_emotion()
        .map_to_music()
        .with_tempo(82)
        .map_to_mixer()
