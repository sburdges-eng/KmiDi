 ...
 * - Type System: kelly::EmotionNode (with ML embeddings from KellyTypes.h)
 *
 ...
 *
 * Features:
 * - Map 64-dim ML embeddings to nearest kelly::EmotionNode by VAD distance
 * - Convert kelly::EmotionNode to ML input format
...
#include "common/Types.h"  // kelly::EmotionNode, EmotionThesaurus
 ...
 * - Mapper finds nearest kelly::EmotionNode by VAD distance
...
public:
    ...
    /**
     * Map ML embedding (64-dim) to nearest kelly::EmotionNode by VAD distance.
     ...
     * @return Nearest kelly::EmotionNode with ML embedding attached, or nullopt if not found
     */
    std::optional<kelly::EmotionNode> embeddingToNode(const std::vector<float>& embedding) const;
     ...
     *
     * @param node kelly::EmotionNode to convert
     * @return ML input vector (size depends on model requirements)
     */
    std::vector<float> nodeToMLInput(const kelly::EmotionNode& node) const;
     ...
     * @return Nearest kelly::EmotionNode
     */
    kelly::EmotionNode findNearestNodeByVAD(float valence, float arousal, float dominance) const;
     ...
     */
    float calculateVADDistance(const kelly::EmotionNode& node1, const kelly::EmotionNode& node2) const;
     ...
     * @return Euclidean distance in VAD space
     */
    float calculateVADDistance(float valence, float arousal, float dominance, const kelly::EmotionNode& node) const;

    /**
     * Enhance node with ML embedding.
     * Attaches ML embedding to an existing kelly::EmotionNode.
     *
     * @param node kelly::EmotionNode to enhance
     * @param embedding 64-dim ML embedding
     * @param confidence Model confidence score (0.0-1.0)
     * @return Enhanced kelly::EmotionNode with ML data
     */
    kelly::EmotionNode enhanceNodeWithML(const kelly::EmotionNode& node,
...
private:
    ...
    std::vector<float> vadToMLInput(float valence, float arousal, float dominance,
                                    const kelly::MusicalAttributes& musicalAttrs) const;
};
