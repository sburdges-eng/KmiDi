...
private:
  ...
  bool idleGenerationPending_ = false;
  ...
  void onEmotionTextCommitted(const juce::String &text);
  void onWoundTextChanged(const juce::String &text);
  void updatePreviewSummary();
  void runGeneration(bool userInitiated);
  void markIdleGenerationPending();
