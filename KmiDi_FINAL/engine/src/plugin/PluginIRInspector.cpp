...
PluginIRInspector::PluginIRInspector() {
    ...
    frameDisplay_.setFont(juce::Font(juce::Font::getDefaultMonospacedFontName(), 11.0f));
    frameDisplay_.setColour(juce::TextEditor::backgroundColourId, juce::Colour(0xff1e1e1e));
    frameDisplay_.setColour(juce::TextEditor::textColourId, juce::Colour(0xffd4d4d4));
...
void PluginIRInspector::updateFrame(const intent_ir::IntentFrame& frame) {
    ...
    // Update display on message thread
...
void PluginIRInspector::updateDisplay() {
    if (!hasFrame_.load()) {
        frameDisplay_.setText("No IntentFrame available", juce::dontSendNotification);
        return;
    }
...
juce::String PluginIRInspector::formatFrame(const intent_ir::IntentFrame& frame) {
    ...
    oss << "Provenance:\n";
    oss << "  Source: " << getProvenanceName(frame.provenance.source).toStdString() << "\n";
    oss << "  User Override Weight: " << frame.provenance.user_override_weight << "\n";
    
    return juce::String(oss.str());
}

juce::Colour PluginIRInspector::getProvenanceColor(intent_ir::IntentSource source) {
    switch (source) {
...
juce::String PluginIRInspector::getProvenanceName(intent_ir::IntentSource source) {
    switch (source) {
        case intent_ir::IntentSource::UiDirect:
            return "UI_DIRECT";
        case intent_ir::IntentSource::UiEdit:
