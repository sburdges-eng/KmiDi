...
VADState NetworkDynamics::calculateAverageVAD(const std::vector<Agent>& agents) const {
    VADState result{0.0f, 0.0f, 0.0f};
    if (agents.empty()) return result;

    VADState sum{0.0f, 0.0f, 0.0f};
    for (const auto& agent : agents) {
    ...
    result.valence = sum.valence / agents.size();
    result.arousal = sum.arousal / agents.size();
    result.dominance = sum.dominance / agents.size();
    return result;
}
