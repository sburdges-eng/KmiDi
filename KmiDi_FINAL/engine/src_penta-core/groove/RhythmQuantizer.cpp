...
namespace penta::groove
{
        ...
        uint64_t barStartPosition) const noexcept
    {
        ...
        if (isTripletGrid)
        {
            // Swing is defined for binary subdivisions; a triplet grid already implies
            // ternary subdivision and should not be "swung" again.
            return samplePosition;
        }
    ...
    {
        // `samplesPerBeat` is in samples per quarter note (the common BPM definition).
        // `GridResolution` values are standard note denominators (e.g., 16 = sixteenth notes).
        // So the grid interval in samples is: samplesPerQuarter * 4 / denominator.
        const uint64_t denom = static_cast<uint64_t>(config_.resolution);
        if (denom == 0)
        {
            return 0;
