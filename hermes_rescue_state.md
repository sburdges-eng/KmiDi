...
60. CI excavation campaign (PR #204, commits 4c26bf1f..8750fea6): opening the rescue PR revealed the repo's CI had never gone green at any layer. Root causes fixed in sequence, each verified locally where the platform allows: unpublishable JUCE submodule gitlink (re-pinned twice, finally to the published patch-compatible base 501c07674e); .gitattributes text=auto stripping CRs from the macos15 JUCE patch, making it unappliable on every runner (third_party/patches/** now -text, blob renormalized); Qt6 required by KellyCore for nothing (decoupled — only KMIDI_BUILD_QT_UI/BUILD_DESKTOP require it; libKellyFFI.dylib no longer loads Qt at runtime); JUCE Linux deps (curl/ALSA into the shared script; gtk avoided via JUCE_WEB_BROWSER=0 with JUCE_USE_CURL=0 on penta's PUBLIC defs); libstdc++ include strata (<optional> x11, <algorithm>/<numeric> x9, <cstddef> x14); CMAKE_POSITION_INDEPENDENT_CODE for the FFI dylib's static archives (moodycamel TLS local-exec relocations); MSVC batches 1-7 (pwsh backslash continuations -> shell:bash; Strawberry MinGW from PATH -> msvc-dev-cmd; _USE_MATH_DEFINES + NOMINMAX global; static constexpr lambda capture; __popcnt/__builtin_popcount split; arc4random -> std::random_device session-id helper in intent_ir_ffi.cpp; localtime_s shim x3; std::min<size_t> LLP64 deduction; cargo's intent_ir.lib vs libintent_ir.a artifact name); product bugs found by the new matrix: watchdog staleness measured with time.monotonic (GetTickCount64 granularity on Windows <=3.13 boundary) -> time.perf_counter, and the flaky fixed-sleep context-manager test -> poll-until-fired; penta_tests suite materialized (7 count-based gtests, lane-filter-matched names, build-root output) closing 8 phantom-target lanes; lcov 2.x strictness (negative/gcov/mismatch/unused) flagged on both coverage twins. Final board: 84 pass / 14 fail / 0 pending, every failure owned by a chip: black+ruff sweep, Sprint 2/3 fix-or-retire, bug-taxonomy re-baseline (chip session repair already on branch), OSCHub implementation belongs in penta_core not KellyCore (bindings silently broken on all platforms; Windows merely reports it), Docker lane builds a Dockerfile never written, and KellyFFI exports zero symbols on Windows (no import library) — an exported-ABI design decision the Pass F rules reserve for human review, which is the campaign's designed stopping point. Loose end: GitNexus index/server version skew (DB v41 vs storage v40) explains the night's silent analyze failures.
