    ...
    for model_type in models_to_run:
        if model_type == "audio_jepa":
            if not os.path.isdir(audio_dir):
                print(
                    f"WARNING: Audio dir not found: {audio_dir}. Create it and add WAV/MP3/FLAC, "
                    f"or set JEPA_AUDIO_DIR.",
                    file=sys.stderr,
                )
            try:
            ...
            if len(dataset) == 0:
                print(
                    f"ERROR: No audio files in {audio_dir}. Add WAV/MP3/FLAC or set "
                    f"JEPA_AUDIO_DIR.",
                    file=sys.stderr,
                )
                if args.model != "both":
