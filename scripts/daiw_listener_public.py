...
class PatternGenerator:
    ...
    def _drum_basic(self, bars: int) -> dict:
        ...
        for bar in range(bars):
            ...
            for i in range(8):
                hat.append(
                    {"pitch": 42, "position": bar_start + i * (tpb // 2), "duration": tpb // 4}
                )
    ...
    def _drum_four_on_floor(self, bars: int) -> dict:
        ...
        for bar in range(bars):
            ...
            for i in range(4):
                hat.append(
                    {"pitch": 42, "position": bar_start + i * tpb + tpb // 2, "duration": tpb // 4}
                )
        ...
        for i, chord_root in enumerate(chord_progression):
            for bar in range(bars_per_chord):
                ...
                if random.random() > 0.3:
                    bass.append(
                        {"pitch": root + 7, "position": bar_start + 2 * tpb, "duration": tpb}
                    )
    ...
    def generate_chords(self, chord_progression: list, bars: int, voicing: str = "basic") -> list:
        ...
        for i, chord_def in enumerate(chord_progression):
            ...
            chords.append({"notes": notes, "position": bar_start, "duration": duration})
...
class DAiWGenerator:
    ...
    def _events_to_track(
        ...
        for event in events:
            ...
            track.append(
                Message("note_off", note=pitch, velocity=0, time=duration, channel=channel)
            )
            current_time = pos + duration

        return track

    def _chords_to_track(
        self, chords: list, name: str, channel: int, pocket: str, humanize: float
    ) -> MidiTrack:
        ...
        for chord_event in chords:
            ...
            for i, pitch in enumerate(notes):
                ...
                track.append(
                    Message(
                        ...
                        note=pitch,
                        velocity=vel,
                        time=strum_delay if i > 0 else delta,
                        channel=channel,
                    )
                )
            ...
            # Note offs
            track.append(
                Message("note_off", note=notes[0], velocity=0, time=duration, channel=channel)
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="DAiW Listener - Generate music from JSON instructions"
    )
    parser.add_argument(
        "--output-dir",
        "-o",
    ...
    parser.add_argument(
        "--once", "-1", type=Path, default=None, help="Process a single JSON file and exit"
    )
