...
def main():
    ...
    if args.backend == "pytorch":
        ...
        infer = lambda mel: encoder(torch.from_numpy(mel)).detach().numpy()  # noqa: E731
    elif args.backend == "coreml":
        model_path = "models/audio_jepa_v01.mlpackage"
        print(f"Backend: Core ML (ANE-preferred) — {model_path}")
        infer = lambda mel: run_coreml_inference(mel, model_path)  # noqa: E731
    else:
        model_path = "models/audio_jepa_v01.onnx"
        print(f"Backend: ONNX Runtime — {model_path}")
        infer = lambda mel: run_onnx_inference(mel, model_path)  # noqa: E731
