...
def find_documentation_files(
    ...
    print(
        f"Scanning for documentation files starting from: {root}"
    ...
    for file_info in doc_files:
        ...
        if file_info.get("value_keywords"):
            categories["with_value_keywords"].append(
                {"path": file_info["path"], "keywords": file_info["value_keywords"]}
            )

        if file_info.get("word_count", 0) > 1000:
            categories["comprehensive"].append(
                {"path": file_info["path"], "word_count": file_info["word_count"]}
            )
