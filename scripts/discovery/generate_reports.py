    ...
    if 'code_files' in data:
        ...
        for file_info in code_files:
            entry = {
                ...
                'integration_opportunity': value_map.get(file_info['path'], {}).get('opportunities', {}),
            }
...
def generate_summary_report(data: Dict, inventory: Dict) -> str:
    ...
    if 'directory_map' in data:
        ...
        report.append(f"- **Total Files in Directories**: {dir_data.get('directories', [{}])[0].get('files', 0) if dir_data.get('directories') else 0:,}\n\n")
    ...
    for i, file_info in enumerate(high_value_files[:30], 1):
        ...
        report.append(f"   - Value: {file_info['value_level']} (Score: {file_info['value_score']})\n")
    ...
    with open(md_path, 'w') as f:
        ...
        for file_type, files in by_type.items():
            ...
            for entry in sorted_files:
                ...
                assessment = entry.get('value_assessment', {})
                if assessment:
                    f.write(f"  - Value: {assessment.get('value_level', 'Unknown')} (Score: {assessment.get('value_score', 0)})\n")
