    ...
    try:
        ...
        for marker, marker_type in PROJECT_MARKERS.items():
            if (root / marker).exists():
                stats['project_markers'].append({
                    'marker': marker,
                    'type': marker_type
                })
        
        # Analyze files
        code_extensions = {'.py', '.pyx', '.pyi', '.cpp', '.hpp', '.h', '.cc', '.cxx', 
                          '.js', '.ts', '.jsx', '.tsx', '.rs', '.go', '.java', 
                          '.swift', '.kt', '.scala', '.rb', '.c', '.hxx'}
        doc_extensions = {'.md', '.markdown', '.txt', '.rst', '.html', '.htm', '.pdf', '.org'}
        model_extensions = {'.pt', '.pth', '.pb', '.h5', '.ckpt', '.onnx', '.tflite', '.mlmodel', '.pkl', '.pickle'}
        ...
        if (stats['files'] > 0 or stats['project_markers'] or 
            stats['code_files'] > 0 or stats['large_files']):
            return stats
...
def scan_user_directories(user_home: str = '/Users/seanburdges') -> Dict:
    ...
    try:
        top_level_dirs = [d for d in user_path.iterdir() 
...
def main():
    ...
    print(f"Summary: {summary['total_directories_scanned']} directories, "
