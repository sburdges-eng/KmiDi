...
def find_model_files(
    ...
    print(
        f"Scanning for model files starting from: {root}"
    ...
    for file_info in model_files:
        ...
        if file_info["size_gb"] > 1:
            categories["very_large_models"].append(
                {"path": file_info["path"], "size_gb": file_info["size_gb"]}
            )
        ...
        if file_info["config_files"]:
            categories["with_configs"].append(
                {"path": file_info["path"], "configs": file_info["config_files"]}
            )
...
def main():
    ...
    print("\n" + "=" * 60)
    print("Model file discovery complete!")
