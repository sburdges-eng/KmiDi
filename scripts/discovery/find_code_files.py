...
ML_KEYWORDS = {'training', 'inference', 'model', 'neural', 'tensor', 'pytorch', 'tensorflow', 'keras'}
...
def find_code_files(root: Path = Path('/Users/seanburdges'), max_files: int | None = None) -> List[Dict]:
    """Find all code files in the directory tree. max_files=None means no limit."""
    code_files = []
    
    print(f"Scanning for code files starting from: {root}" + (f" (max {max_files})" if max_files else " (no limit)"))
...
def categorize_files(code_files: List[Dict]) -> Dict:
    ...
    for file_info in code_files:
        ...
        # KmiDi related
        if (file_info.get('has_kmidi_imports') or 
            file_info.get('has_kmidi_includes') or
            any(kw in file_info['path'].lower() for kw in KMDI_KEYWORDS)):
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"Code file discovery complete!")
    print(f"Found {len(code_files):,} code files")
