...
def test_emotion_thesaurus():
    ...
    try:
        ...
        if hasattr(thesaurus, 'find_emotion'):
            ...
            print(f"  ✅ Emotion lookup method available")
        else:
...
def test_intent_processing():
    ...
    try:
        ...
        print(f"  ✅ Created intent: {intent.song_intent.mood_primary} in {intent.technical_constraints.technical_key}")
    ...
    try:
        ...
        if emotions_dir.exists():
            ...
            if joy_file.exists():
                ...
                print(f"  ✅ Loaded joy.json: {len(joy_data.get('intensity_tiers', []))} intensity tiers")
        else:
...
def test_groove_system():
    ...
    try:
        ...
        extractor_obj = extractor.GrooveExtractor() if hasattr(extractor, 'GrooveExtractor') else None
        applicator_obj = applicator.GrooveApplicator() if hasattr(applicator, 'GrooveApplicator') else None
        ...
        try:
            from music_brain.kelly_companion.groove.groove_engine import GrooveEngine
            from music_brain.kelly_companion.groove.extractor import GrooveExtractor
            from music_brain.kelly_companion.groove.applicator import GrooveApplicator
