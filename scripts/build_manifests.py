...
def match_audio_midi_pairs(
    audio_files: List[Path], midi_files: List[Path]
...
def split_train_val(items: List, val_split: float, seed: int = 42) -> Tuple[List, List]:
...
def save_point_cloud(cloud: np.ndarray, audio_path: Path, clouds_dir: Path) -> Path:
    """
...
def build_spectocloud_manifests(
    ...
    print("Building Spectocloud manifests...")
    ...
    for manifest_path, pairs_list in [(train_manifest, train_pairs), (val_manifest, val_pairs)]:
...
def build_midi_manifests(
    midi_root: Optional[Path], out_dir: Path, val_split: float, seed: int = 42
):
    ...
    print("Building MIDI Generator manifests...")
    print(f"  MIDI root: {midi_root}")
    ...
    for manifest_path, files_list in [(train_manifest, train_files), (val_manifest, val_files)]:
        with open(manifest_path, "w") as f:
...
def main():
    parser = argparse.ArgumentParser(description="Build dataset manifests for v2 training pipeline")
    parser.add_argument("--audio-root", type=str, help="Root directory containing audio files")
    parser.add_argument("--midi-root", type=str, help="Root directory containing MIDI files")
    parser.add_argument(
        "--out-dir",
        type=str,
    ...
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for deterministic splitting (default: 42)"
    )
    parser.add_argument(
        "--generate-clouds",
        action="store_true",
    ...
    parser.add_argument(
        "--clouds-dir", type=str, help="Directory to save generated clouds (default: data/clouds)"
    )
