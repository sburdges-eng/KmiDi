...
# Paths
_storage = get_storage_config()
AUDIO_DATA_ROOT = _storage.audio_data_root
...
class Trainer:
    ...
    def train_epoch(self) -> float:
        ...
        for batch_idx, (inputs, targets) in enumerate(self.train_loader):
            ...
            if (batch_idx + 1) % grad_accum == 0 or (batch_idx + 1) == len(self.train_loader):
            ...
            if batch_idx % self.config.log_interval == 0:
                logger.debug(f"  Batch {batch_idx}/{len(self.train_loader)}, Loss: {loss.item() * grad_accum:.4f}")
