...
class FreesoundAPI:
    ...
    def search(self, query, instrument=None, page_size=10):
        ...
        if not self.api_key:
            print(
                "⚠ Freesound API key required. Set FREESOUND_API_KEY environment variable or run setup."
            )
            return None
    ...
    def build_emotion_tree(self):
        ...
        for base in self.base_emotions:
            ...
            for sub_name, sub_data in sub_emotions.items():
                tree["sub"].append({"base": base_name, "name": sub_name, "data": sub_data})
    ...
    def download_for_combo(self, emotion, instrument, level="base", max_files=5):
        ...
        for sound in results["results"]:
            ...
            if downloaded_size > 0:
                ...
                downloaded_count += 1
                print(
                    f"    ✓ {downloaded_size / 1024 / 1024:.2f}MB (Total: {current_size / 1024 / 1024:.2f}MB)"
                )
    ...
    def auto_fetch_all(self, files_per_combo=5):
        ...
        for base_emotion in self.hierarchy.emotion_tree["base"]:
            for instrument in INSTRUMENTS:
                count = self.download_for_combo(
                    base_emotion, instrument, level="base", max_files=files_per_combo
                )
        ...
        for sub in self.hierarchy.emotion_tree["sub"][:20]:  # First 20 sub-emotions
            ...
            for instrument in INSTRUMENTS:
                count = self.download_for_combo(
                    sub_name, instrument, level="sub", max_files=files_per_combo
                )
                total_downloaded += count
