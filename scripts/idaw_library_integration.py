...
class LibraryScanner:
    ...
    def scan_logic_pro(self) -> Optional[InstrumentLibrary]:
        ...
        if available:
            # Scan Apple Loops
            for loops_path in [
                InstrumentPaths.LOGIC_PRO["apple_loops"],
                InstrumentPaths.LOGIC_PRO["apple_loops_system"],
            ]:
                if loops_path.exists():
                    library.samples.extend(
                        self._scan_directory(loops_path, [".aif", ".caf", ".wav"])
                    )

            # Scan Patches
            for patches_path in [
                InstrumentPaths.LOGIC_PRO["patches"],
    ...
    def scan_melda(self) -> Optional[InstrumentLibrary]:
        ...
        melda_exists = any(
            list(InstrumentPaths.MELDA["plugins_au"].glob("M*.component"))
            if InstrumentPaths.MELDA["plugins_au"].exists()
            else []
        )
        ...
        if melda_exists or InstrumentPaths.MELDA["presets_user"].exists():
            # Scan presets
            for presets_path in [
                InstrumentPaths.MELDA["presets_user"],
    ...
    def scan_vital(self) -> Optional[InstrumentLibrary]:
        ...
        vital_exists = (
            InstrumentPaths.VITAL["presets_user"].exists()
            or InstrumentPaths.VITAL["presets_factory"].exists()
        )
        ...
        if vital_exists:
            # Scan presets
            for presets_path in [
                InstrumentPaths.VITAL["presets_user"],
            ...
            # Scan wavetables
            for wt_path in [
                InstrumentPaths.VITAL["wavetables_user"],
                InstrumentPaths.VITAL["wavetables_factory"],
            ]:
                if wt_path.exists():
                    library.wavetables.extend(
                        self._scan_directory(wt_path, [".wav", ".vitaltable"])
                    )
            ...
            print(
                f"    ✓ Found {len(library.presets)} presets, {len(library.wavetables)} wavetables"
            )
        else:
            print("    ✗ Not installed")
...
class InstrumentSelector:
    ...
    def _select_instrument(self, category: str, emotion: str) -> InstrumentSelection:
        ...
        if (
            "synth" in category
            and self.libraries.get("vital", InstrumentLibrary("", Path(), False)).available
        ):
        ...
        if (
            user_samples
            and self.libraries.get("user_samples", InstrumentLibrary("", Path(), False)).available
        ):
...
class LogicProExporter:
    ...
    def export_for_logic(
        self, midi_path: Path, selections: Dict[str, InstrumentSelection], emotion: str, bpm: int
    ) -> Dict[str, Path]:
        """
