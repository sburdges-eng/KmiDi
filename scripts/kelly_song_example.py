...
def generate_kelly_harmony():
    ...
    print(
        f"✓ Roman numerals: {' - '.join([v.roman_numeral for v in harmony.voicings if hasattr(v, 'roman_numeral') and v.roman_numeral])}"
    )
    print(f"✓ Rule break: {harmony.rule_break_applied}")
    print(f"✓ Why: {harmony.emotional_justification}")
    print("✓ MIDI saved: /mnt/user-data/outputs/kelly_song_harmony.mid")
...
def generate_variations():
    ...
    generate_midi_from_harmony(
        diatonic, "/mnt/user-data/outputs/kelly_diatonic_comparison.mid", tempo_bpm=82
    )
    print(f"\n✓ Diatonic version: {' - '.join(diatonic.chords)}")
    print("  (Standard pop progression, no rule-breaking)")
    ...
    print("  (Modal interchange + avoided resolution)")
    print("  (Even more grief - no return to home)")
