...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--s3-uri",
        default="",
        help="S3 URI to run root (s3://bucket/prefix/run-id) or output root when --run-id is provided",  # noqa: E501
    )
...
def enforce_teacher_fetch_policy(
    ...
    if required_profile and args.profile != required_profile:
        raise ValueError(
            f"Teacher artifact fetch requires profile '{required_profile}' from run contract; got '{args.profile}'"  # noqa: E501
        )
    ...
    if args.break_glass_role_arn != required_role:
        raise ValueError(
            "Teacher artifact fetch role mismatch: provided role does not match security.teacherFetch.requiredRoleArn"  # noqa: E501
        )
