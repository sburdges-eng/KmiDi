...
def _render_intent_frame_rust(schema: Dict[str, Any]) -> str:
    ...
    lines.append(
        '            return Err(format!("unsupported schema_version {}", '
        "self.meta.schema_version));"
    )
    lines.append("        }")
    lines.append("        if self.music.tempo_bias < -1.0 || self.music.tempo_bias > 1.0 {")
    lines.append(
        '            return Err(format!("tempo_bias {} out of range [-1.0, 1.0]", '
        "self.music.tempo_bias));"
    )
    lines.append("        }")
    ...
    for field_name, field_node in musical_intent_props.items():
        ...
        if ft == "number" and mn == 0.0 and mx == 1.0:
            ...
            lines.append(
                f'            return Err(format!("{field_name} {{}} out of range [0.0, 1.0]", '
                f"self.music.{field_name}));"
            )
            lines.append("        }")
    ...
    for field_name, field_node in dsp_targets_props.items():
        ...
        if ft == "number" and mn == 0.0 and mx == 1.0:
            lines.append(
                f"        if self.dsp_targets.{field_name} < 0.0 || self.dsp_targets.{field_name} "
                f"> 1.0 {{"
            )
            lines.append(
                f'            return Err(format!("{field_name} {{}} out of range [0.0, 1.0]", '
                f"self.dsp_targets.{field_name}));"
            )
            lines.append("        }")
    ...
    lines.append("        }")
    lines.append(
        "        if self.time.end_bar != -1 && self.time.start_bar != -1 && self.time.end_bar <= "
        "self.time.start_bar {"
    )
    lines.append(
        '            return Err(format!("end_bar {} must be > start_bar {}", self.time.end_bar, '
        "self.time.start_bar));"
    )
    lines.append("        }")
    lines.append("        Ok(())")
