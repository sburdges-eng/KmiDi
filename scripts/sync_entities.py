...
from music_brain.engine_api.schema import EmotionStateSchema  # noqa: E402
from music_brain.engine_api.schema import IntentFrameSchema  # noqa: E402
...
def _render_emotion_typescript(schema: dict) -> str:
    ...
    for def_name, def_node in defs.items():
        if "enum" in def_node:
            lines.append(
                f"export type {def_name} = {' | '.join(repr(v) for v in def_node['enum'])};"
            )
            lines.append("")
...
def _rust_literal(value: Any, node: Dict[str, Any]) -> str:
    ...
    if node_type == "number":
        ...
        return s
    if node_type == "string":
        return "String::new()" if value == "" else f'"{value}".to_string()'
    # fallback
    return f"{value}"
...
def _render_intent_frame_rust(schema: Dict[str, Any]) -> str:
    ...
    for def_name, def_node in defs.items():
        ...
        has_nonzero = any(p.get("default") not in (None, 0, 0.0, False, "") for p in props.values())
        if has_nonzero:
            lines.append("#[derive(Debug, Clone, Serialize, Deserialize)]")
        else:
    ...
    lines.append(
        '            return Err(format!("unsupported schema_version {}", self.meta.schema_version));'  # noqa: E501
    )
    lines.append("        }")
    lines.append("        if self.music.tempo_bias < -1.0 || self.music.tempo_bias > 1.0 {")
    lines.append(
        '            return Err(format!("tempo_bias {} out of range [-1.0, 1.0]", self.music.tempo_bias));'  # noqa: E501
    )
    lines.append("        }")
    ...
    for field_name, field_node in musical_intent_props.items():
        ...
        if ft == "number" and mn == 0.0 and mx == 1.0:
            lines.append(
                f"        if self.music.{field_name} < 0.0 || self.music.{field_name} > 1.0 {{"
            )
            lines.append(
                f'            return Err(format!("{field_name} {{}} out of range [0.0, 1.0]", self.music.{field_name}));'  # noqa: E501
            )
            lines.append("        }")
    ...
    for field_name, field_node in dsp_targets_props.items():
        ...
        if ft == "number" and mn == 0.0 and mx == 1.0:
            lines.append(
                f"        if self.dsp_targets.{field_name} < 0.0 || self.dsp_targets.{field_name} > 1.0 {{"  # noqa: E501
            )
            lines.append(
                f'            return Err(format!("{field_name} {{}} out of range [0.0, 1.0]", self.dsp_targets.{field_name}));'  # noqa: E501
            )
            lines.append("        }")
    ...
    lines.append("        }")
    lines.append(
        "        if self.time.end_bar != -1 && self.time.start_bar != -1 && self.time.end_bar <= self.time.start_bar {"  # noqa: E501
    )
    lines.append(
        '            return Err(format!("end_bar {} must be > start_bar {}", self.time.end_bar, self.time.start_bar));'  # noqa: E501
    )
    lines.append("        }")
    lines.append("        Ok(())")
    lines.append("    }")
...
def sync_intent_frame() -> None:
    ...
    INTENT_FRAME_TS_OUT.write_text(_render_intent_frame_typescript(schema), encoding="utf-8")
    if INTENT_FRAME_RUST_OUT.parent.exists():
        INTENT_FRAME_RUST_OUT.write_text(_render_intent_frame_rust(schema), encoding="utf-8")
