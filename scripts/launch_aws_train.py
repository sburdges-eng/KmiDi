...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--package-s3-uri",
        default="",
        help="S3 URI to packaged dataset root. "
...
aws s3 cp {q_runner_uri}/scripts/aws_train_entrypoint.py \
/opt/listening-train/scripts/aws_train_entrypoint.py
...
TRAIN_CMD="PYTHONPATH=/opt/listening-train timeout {timeout_sec} \
python3 /opt/listening-train/scripts/aws_train_entrypoint.py \
--run-contract /opt/listening-train/config/run_contract.yaml \
--workdir /opt/listening-train/work --output-s3-uri {q_output_s3_uri} --run-id {q_run_id} \
--checkpoint-cadence-steps {checkpoint_cadence_steps} --max-checkpoints {max_checkpoints} \
--max-total-checkpoints {max_total_checkpoints}{early_stop_args}{teacher_args}"
...
--eval-summary /opt/listening-train/work/upload_bundle/artifacts/student/eval_summary.json \
--metric-path {q_metric_path} --poll-seconds 15 --complete-signal $COMPLETE_SIGNAL \
--patience {int(early_stop.get('patience', 0))} \
--min-delta {float(early_stop.get('minDelta', 0.0))} \
--status-file /opt/listening-train/work/logs/watchdog.status"
COST_CMD="/opt/listening-train/tools/cost_estimate.sh --hourly-usd {hourly_usd} \
--budget-cap-usd {budget_cap_usd} --start-epoch $(date +%s) --poll-seconds 30 \
--complete-signal $COMPLETE_SIGNAL"
...
def validate_s3_prefixes(package_s3_uri: str, output_s3_uri: str, runner_s3_uri: str) -> None:
    ...
    if runner_s3_uri:
        ...
        if not _prefix_is_under(runner_prefix, output_prefix.strip("/")):
            raise ValueError(
                f"--runner-s3-uri prefix {runner_s3_uri} must stay under --output-s3-uri "
                f"{output_s3_uri}"
            )
...
def resolve_max_total_checkpoints(
    ...
    if resolved < max_checkpoints + 1:
        raise ValueError(
            f"max total checkpoints {resolved} is too small for keep-last-N + best policy "
            f"({max_checkpoints + 1})"
        )
    return resolved
...
def run_preflight_gate(preflight_command: str, dry_run: bool) -> None:
    if not preflight_command:
        raise ValueError(
            "Missing preflight command; set training.ops.preflightCommand or pass "
            "--preflight-command"
        )
    if dry_run:
        return
