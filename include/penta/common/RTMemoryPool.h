...
public:
  ...
  void assertSingleProducer() {
    ...
    if (!owner.compare_exchange_strong(expected, current))
      assert(expected == current && "RTMemoryPool: SPSC violation — allocate() "
...
private:
  ...
  bool isBlockAligned(const void *ptr) const noexcept;
 ...
 */
template <typename T> class RTPoolPtr {
public:
  RTPoolPtr(RTMemoryPool &pool)
      : pool_(pool), ptr_(static_cast<T *>(pool.allocate())) {
    if (ptr_) {
  ...
  RTPoolPtr(RTPoolPtr &&other) noexcept : pool_(other.pool_), ptr_(other.ptr_) {
    other.ptr_ = nullptr;
  }
