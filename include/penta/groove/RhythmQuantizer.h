...
namespace penta::groove
{
    ...
    public:
        enum class GridResolution
        {
            ...
            Quarter = 4,
            Eighth = 8,
            // Triplet grids (use standard note denominators so that:
            // gridInterval = samplesPerBeat * 4 / denominator.
            // Example: 8th-note triplet within a quarter note => quarter/3 => whole/12.
            EighthTriplet = 12,
            Sixteenth = 16,
            SixteenthTriplet = 24,
            ThirtySecond = 32,
        ...
        struct Config
        {
            ...
            Config()
                : resolution(GridResolution::Sixteenth), strength(0.8f), enableSwing(false), swingAmount(0.5f), timeSignatureNum(4), timeSignatureDen(4)
            {
            }
        };
