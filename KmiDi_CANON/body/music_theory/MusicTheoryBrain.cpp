    ...
    const std::vector<int>& velocities) const
{
    ...
    for (const auto& conceptItem : analysis.concepts) {
        analysis.learningOpportunities.push_back(
            "Learn about: " + conceptItem.conceptName + " (" + conceptItem.evidence + ")"
        );
    }
    ...
    const UserProfile& userProfile) const
{
    ...
    if (!concepts.empty()) {
        auto conceptItem = concepts[0];
        return knowledge_->explainConcept(
            conceptItem.conceptName,
    ...
    int duration) const
{
    ...
    for (const auto& conceptItem : focusAreas) {
        if (timeRemaining <= 0) break;

        Exercise exercise = knowledge_->generateExercise(conceptItem, level);
    ...
    const std::string& targetEmotion) const
{
    ...
    if (!current.chords.empty()) {
        ...
        prog.chords = current.chords;
        prog.key = current.detectedKey;
        prog.romanNumerals = current.progressionAnalysis.romanNumerals;
        prog.functionalAnalysis = current.progressionAnalysis.functionalAnalysis;
        auto emotionAnalysis = harmony_->analyzeEmotionalContent(prog);
