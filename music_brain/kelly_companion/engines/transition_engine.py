...
SECTION_TRANSITION_PREFERENCES: Dict[Tuple[str, str], List[TransitionType]] = {
    ...
    # Into breakdown
    ("chorus", "breakdown"): [TransitionType.COLLAPSE, TransitionType.SILENCE, TransitionType.STRIP],
    ...
    # Into build
    ("breakdown", "build"): [TransitionType.BUILD, TransitionType.RISER, TransitionType.ANTICIPATION],
...
def generate_transition_curve(
    ...
    for i in range(num_points):
        ...
        if transition_type == TransitionType.HARD_CUT:
            ...
            # Up then down
            if pos < 0.7:
                energy = from_energy + (pos / 0.7) * (max(from_energy, to_energy) * 1.2 - from_energy)
            else:
        ...
        base_tension = 0.3
        if transition_type in [TransitionType.BUILD, TransitionType.RISER, TransitionType.ANTICIPATION]:
            base_tension = 0.5
...
class TransitionEngine:
    ...
    def generate_transition(
        ...
        if trans_type in profile["avoid_types"]:
            alternatives = [t for t in profile["preferred_types"] if t not in profile["avoid_types"]]
            if alternatives:
    ...
    def get_state_at_position(
        ...
        return TransitionPoint(
            position=position,
            energy_level=prev_point.energy_level + t * (next_point.energy_level - prev_point.energy_level),
    ...
    def print_transition(self, spec: TransitionSpec) -> str:
        ...
        lines.append(f"Energy: {spec.from_energy:.2f} → {spec.to_energy:.2f} ({spec.energy_direction.value})")
        ...
        for point in spec.points:
            ...
            active = ""
            if point.drums_active: active += "D"
            if point.bass_active: active += "B"
            if point.chords_active: active += "C"
            if point.melody_active: active += "M"
            if point.effects_active: active += "X"
    ...
    def print_transition_plan(self, plan: TransitionPlan) -> str:
        ...
        for i, trans in enumerate(plan.transitions):
            ...
            energy_arrow = "→" if abs(trans.to_energy - trans.from_energy) < 0.1 else "↑" if trans.to_energy > trans.from_energy else "↓"
            
            elements = []
            if trans.has_fill: elements.append("F")
            if trans.has_riser: elements.append("R")
            if trans.has_impact: elements.append("I")
