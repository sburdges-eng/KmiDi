...
class ModulationMatrix:
    ...
    def remove_route(self, source_name: str, target_effect: str, target_param: str):
        ...
        self.routes = [
            ...
            if not (
                ...
                and r.target_param == target_param
            )
        ]
    ...
    def get_modulation(self, effect_name: str, param_name: str) -> float:
        ...
        for route in self.routes:
            if (
                route.enabled
                and route.target_effect == effect_name
                and route.target_param == param_name
            ):
    ...
    def to_dict(self) -> Dict:
        return {
            "sources": {
                name: {"type": type(s).__name__, "params": vars(s)}
                for name, s in self.sources.items()
            },
            "routes": [
                {
                    "source": r.source_name,
                    "effect": r.target_effect,
