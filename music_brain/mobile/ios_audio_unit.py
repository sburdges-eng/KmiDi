...
public class {config.name}AudioUnit: AUAudioUnit {{
    ...
    {"public override var scheduleMIDIEventBlock: AUScheduleMIDIEventBlock? {" if config.midi_input else ""}  # noqa: E501

    {"    return { [weak self] sampleTime, cable, length, data in" if config.midi_input else ""}
    {"        self?.dspKernel.handleMIDIEvent(data: data, length: Int(length))" if config.midi_input else ""}  # noqa: E501
...
{"void " + config.name + "DSPKernel::handleMIDIEvent(const uint8_t* data, int length) {" if config.midi_input else ""}  # noqa: E501
...
{"        velocityScale_ = noteActive_ ? std::clamp(static_cast<float>(lastVelocity_) / 127.0f, 0.0f, 1.0f) : 0.0f;" if config.midi_input else ""}  # noqa: E501
...
{"            const float mod = std::clamp(static_cast<float>(data[2]) / 127.0f, 0.0f, 1.5f);" if config.midi_input else ""}  # noqa: E501
...
public class AudioUnitViewController: AUViewController {{
    ...
    public override func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {{  # noqa: E501
