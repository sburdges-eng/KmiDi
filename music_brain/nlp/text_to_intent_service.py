...
CLUSTER_TRIGGERS: Dict[str, Dict[str, Any]] = {
    "metal-shred": {
        ...
        "keywords": {"fast", "guitar", "overdrive", "distortion", "distorted",
                     "metal", "shred", "heavy", "aggressive", "blistering"},
    ...
    "blues-slow": {
        "label": "Slow Blues",
        "keywords": {"slow", "blues", "bluesy", "clean", "pentatonic",
                     "laid-back", "laid back", "soulful", "mellow"},
        "modes": {"pentatonic", "minor pentatonic", "blues scale", "minor",
                  "mixolydian", "dorian"},
        "min_match": 2,
        "overrides": {
    ...
    "jazz-modal": {
        "label": "Modal Jazz",
        "keywords": {"jazz", "jazzy", "walking bass", "swing", "swinging",
                     "cool", "modal", "sax", "saxophone", "piano"},
        "modes": {"dorian", "lydian", "mixolydian", "minor"},
        "min_match": 2,
        "overrides": {
            "tempo": ParamDistribution("gaussian", center=130, spread=20),
            "density": ParamDistribution("discrete", weights={"medium": 0.5, "dense": 0.3, "sparse": 0.2}),
            "swing": ParamDistribution("range", min=0.5, max=0.8, bias=0.7),
            "harmonic_rhythm": ParamDistribution("discrete", weights={"fast": 0.5, "medium": 0.4, "slow": 0.1}),
        },
    },
    "ambient-pad": {
        "label": "Ambient Pad",
        "keywords": {"ambient", "pad", "synth pad", "reverb", "atmospheric",
                     "ethereal", "dreamy", "floating", "spacious", "drone"},
    ...
    "funk-groove": {
        "label": "Funk Groove",
        "keywords": {"funk", "funky", "groovy", "groove", "syncopated",
                     "slap", "slap bass", "tight", "bass"},
    ...
    "pop-upbeat": {
        "label": "Upbeat Pop",
        "keywords": {"pop", "upbeat", "catchy", "bright", "happy",
    ...
    "lo-fi-chill": {
        "label": "Lo-fi Chill",
        "keywords": {"lo-fi", "lofi", "chill", "vinyl", "tape", "warm",
                     "cozy", "study", "relax", "mellow"},
        "modes": {"dorian", "mixolydian", "minor", "pentatonic"},
        "min_match": 2,
        "overrides": {
    ...
    "edm-drop": {
        "label": "EDM Drop",
        "keywords": {"edm", "drop", "build", "buildup", "electronic",
                     "synth", "bass drop", "heavy", "massive"},
    ...
    "hip-hop-boom-bap": {
        "label": "Boom Bap",
        "keywords": {"hip-hop", "hip hop", "boom bap", "rap", "oldschool",
                     "drums", "sample", "vinyl"},
    ...
    "cinematic-epic": {
        ...
        "keywords": {"cinematic", "epic", "orchestral", "film", "movie",
                     "dramatic", "sweeping", "massive", "strings", "brass",
                     "choir"},
        ...
        "overrides": {
            ...
            "dynamics": ParamDistribution("discrete", weights={"ff": 0.3, "f": 0.3, "pp": 0.2, "p": 0.2}),
        },
    },
    "punk-rock": {
        "label": "Punk Rock",
        "keywords": {"punk", "fast", "distortion", "distorted", "aggressive",
                     "raw", "loud", "power chords", "angry"},
    ...
    "reggae": {
        "label": "Reggae",
        "keywords": {"reggae", "offbeat", "dub", "ska", "island",
                     "bass", "laid-back"},
    ...
    "post-rock": {
        "label": "Post-Rock",
        "keywords": {"post-rock", "atmospheric", "reverb", "crescendo",
                     "building", "guitar", "delay", "ethereal", "wall of sound"},
        "modes": {"minor", "aeolian", "dorian"},
        "min_match": 2,
        "overrides": {
            "tempo": ParamDistribution("gaussian", center=90, spread=10),
            "dynamics": ParamDistribution("discrete", weights={"pp": 0.3, "p": 0.2, "ff": 0.3, "f": 0.2}),
            "space_probability": ParamDistribution("range", min=0.1, max=0.35, bias=0.2),
        },
    },
    "synthwave-retro": {
        "label": "Synthwave/Retrowave",
        "keywords": {"synthwave", "retrowave", "80s", "neon", "retro",
                     "synth", "arpeggiated", "arpeggio", "driving"},
    ...
    "neo-soul": {
        "label": "Neo-Soul",
        "keywords": {"neo-soul", "neo soul", "soul", "warm", "smooth",
                     "keys", "piano", "bass", "groove", "organic"},
...
class TextToIntentService:
    ...
    def _match_clusters(
        ...
        for cluster_id, cluster_def in CLUSTER_TRIGGERS.items():
            ...
            total_possible = len(trigger_keywords) + len(trigger_modes)
        ...
        for cluster_id, cluster_data in clusters.items():
            ...
            for param, dist in overrides.items():
                if param not in merged:
                    ...
                    merged[param] = self._blend_distributions(
                        existing, dist, score
                    )

        # Default distributions for params that weren't set
        if "tempo" not in merged:
            merged["tempo"] = ParamDistribution(
                "gaussian", center=120, spread=20
            )
        if "mode_weights" not in merged:
            merged["mode_weights"] = ParamDistribution(
                "discrete",
                weights={
                    "major": 0.25, "minor": 0.25, "dorian": 0.15,
                    "mixolydian": 0.15, "lydian": 0.1, "phrygian": 0.1,
                },
            )
    ...
    def _blend_distributions(
        ...
        if a.type == "gaussian" and b.type == "gaussian":
            ...
            if total > 0:
                merged_weights = {
                    k: v / total for k, v in merged_weights.items()
                }
    ...
    def _calculate_confidence(
        ...
        cluster_score = (
            max(c["score"] for c in clusters.values())
            if clusters
            else 0.3
        )
