...
class iDAWWorkflow:
    ...
    def __init__(self, device: str = "auto", tier: int = 1, use_optimization: bool = True):
        """
        Initialize workflow.
    ...
    def emotion_intent_to_embedding(
        self, wound: str, emotion_label: str, intensity: float = 0.7
    ) -> np.ndarray:
        """
        ...
        logger.info(
            f"Created emotion embedding: {emotion_label} "
    ...
    def generate_complete_music(
        ...
        emotion_embedding = self.emotion_intent_to_embedding(wound, emotion_label, intensity)
        ...
        voice = self.voice_gen.speak_emotion(voice_text, emotion=emotion_label.lower())

        # Step 5: Compose to MIDI file
        logger.info("[5/5] Composing MIDI file...")
        midi_bytes = self._notes_to_midi_file(
            midi_result["melody"], midi_result["groove"], tempo_bpm=120
        )
    ...
    def _notes_to_midi_file(
        self, notes: np.ndarray, groove_params: Dict, tempo_bpm: int = 120
    ) -> bytes:
        """Convert notes to MIDI bytes"""
        try:
            from music21 import environment, instrument, note, stream, tempo
    ...
    def save_outputs(self, result: Dict, output_dir: str = "./generated_music"):
        ...
        try:
            ...
            wavfile.write(output_path / "generated_music.wav", 22050, audio_data)

            voice_data = (result["voice"] * 32767).astype(np.int16)
            wavfile.write(output_path / "voice_guidance.wav", 22050, voice_data)
