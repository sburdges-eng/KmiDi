...
class EmotionScaleSampler:
    ...
    def download_for_combination(self, emotion, scale, max_files=10):
        ...
        print(
            f"\n✓ Completed {emotion}/{scale}: {downloaded_count} files, "
    ...
    def show_stats(self):
        ...
        print(
            f"Max Combinations: {len(self.emotions)} × {len(self.base_scales)} = "
            f"{len(self.emotions) * len(self.base_scales)}"
        )  # noqa: E501
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nUSAGE:")
        print(
            "  python -m music_brain.samples.emotion_scale_sampler setup   # Configure Freesound "
            "API key"
        )  # noqa: E501

        print("  python -m music_brain.samples.emotion_scale_sampler fetch <emotion> <scale>")
        print(
            "  python -m music_brain.samples.emotion_scale_sampler batch <count>  # Fetch random "
            "combinations"
        )  # noqa: E501
        ...
        print(
            "  python -m music_brain.samples.emotion_scale_sampler list    # List emotions and "
            "scales"
        )  # noqa: E501
