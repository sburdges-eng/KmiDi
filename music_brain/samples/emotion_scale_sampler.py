...
class EmotionScaleSampler:
        ...
        for query in queries:
            ...
            for sound in results["results"]:
                ...
                if downloaded_size > 0:
                    ...
                    print(
                        f"    Total: {current_size / 1024 / 1024:.2f}MB / {MAX_SIZE_PER_COMBO_MB}MB"
                    )
    ...
    def show_stats(self):
        ...
        print(
            f"Max Combinations: {len(self.emotions)} × {len(self.base_scales)} = {len(self.emotions) * len(self.base_scales)}"
        )
        ...
        if total_combos > 0:
            ...
            for i, (key, data) in enumerate(sorted_combos[:10], 1):
                ...
                print(
                    f"  {i}. {data['emotion']}/{data['scale']}: {file_count} files, {size_mb:.2f}MB"
                )
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nUSAGE:")
        print(
            "  python -m music_brain.samples.emotion_scale_sampler setup   # Configure Freesound API key"
        )
        print("  python -m music_brain.samples.emotion_scale_sampler fetch <emotion> <scale>")
        print(
            "  python -m music_brain.samples.emotion_scale_sampler batch <count>  # Fetch random combinations"
        )
        print(
            "  python -m music_brain.samples.emotion_scale_sampler sync    # Sync to Google Drive"
        )
        print("  python -m music_brain.samples.emotion_scale_sampler stats   # Show statistics")
        print(
            "  python -m music_brain.samples.emotion_scale_sampler list    # List emotions and scales"
        )
        print("\nEXAMPLES:")
