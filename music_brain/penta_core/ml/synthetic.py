...
class SyntheticGenerator:
    ...
    def _generate_notes_from_progression(
        ...
        for bar in range(bars):
            ...
            chord_type = (
                "min" if "i" in chord_symbol.lower() and chord_symbol[0].islower() else "maj"
            )
    ...
    def _save_sample_midi(self, sample: Dict[str, Any], output_dir: Path):
        ...
        for note in notes:
            ...
            track.append(
                mido.Message(
                    "note_on", note=note["pitch"], velocity=note["velocity"], time=max(0, delta)
                )
            )
            track.append(
                mido.Message("note_off", note=note["pitch"], velocity=0, time=duration_ticks)
            )
            ...
            for sample in self.samples:
                ...
                if self.model_name == "emotion_recognizer":
                    ...
                    if len(features) >= 2:
                        ...
                        # arousal in upper freqs
                        mel_spec[:, : mel_height // 2, :] += features[1] * 0.1
                    ...
                    if notes:
                        ...
                        pitches = [
                            int(n.get("pitch", 60)) % self.input_dim for n in notes[:seq_len]
                        ]
                        # Pad if needed
                ...
                if self.model_name == "emotion_recognizer":
                    ...
                    self.targets.append(
                        torch.tensor(target[: self.output_dim], dtype=torch.float32)
                    )
                elif self.model_name == "groove_predictor":
