...
class GeneratedMusic:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.emotional_state and hasattr(self.emotional_state, 'primary_emotion'):
            result["emotional_state"] = {
                "valence": self.emotional_state.valence.value if hasattr(self.emotional_state.valence, 'value') else self.emotional_state.valence,
                "arousal": self.emotional_state.arousal.value if hasattr(self.emotional_state.arousal, 'value') else self.emotional_state.arousal,
        ...
        if self.musical_params and hasattr(self.musical_params, 'tempo_suggested'):
            result["musical_params"] = {
                ...
                "timing_feel": self.musical_params.timing_feel.value if hasattr(self.musical_params.timing_feel, 'value') else str(self.musical_params.timing_feel),
    ...
    def summary(self) -> str:
        ...
        if self.musical_params and hasattr(self.musical_params, 'tempo_suggested'):
            ...
            lines.append(f"  Timing Feel: {self.musical_params.timing_feel.value if hasattr(self.musical_params.timing_feel, 'value') else self.musical_params.timing_feel}")
            lines.append(f"  Dissonance: {self.musical_params.dissonance:.0%}")
            lines.append(f"  Density: {self.musical_params.density_suggested}")
            lines.append("")
        if self.mixer_params and hasattr(self.mixer_params, 'description'):
            ...
            lines.append(f"  Reverb: {self.mixer_params.reverb_mix:.0%} mix, {self.mixer_params.reverb_decay:.1f}s decay")
            lines.append(f"  Compression: {self.mixer_params.compression_ratio:.1f}:1")
            lines.append(f"  Saturation: {self.mixer_params.saturation:.0%} ({self.mixer_params.saturation_type})")
            lines.append("")
...
class MusicBrain:
    ...
    def generate_from_intent(
        ...
        emotional_state = EmotionalState(
            valence=Valence(int(valence * 2)),
            arousal=Arousal(int((arousal - 0.5) * 4)),
        ...
        mixer_params = self.emotion_mapper.map_emotion_to_mixer(
            emotional_state,
            musical_params
        ) if self.emotion_mapper else None
        ...
        ) if self.production_mapper else None

        groove_settings_val = settings_from_intent(
        ...
        ) if settings_from_intent else None
...
class FluentChain:
    ...
    def map_to_emotion(self) -> 'FluentChain':
        ...
        if EmotionalState is None or Valence is None or Arousal is None:
            raise ImportError("Required music_brain modules not available")
    ...
    def map_to_music(self) -> 'FluentChain':
        ...
        if self.emotional_state is None or get_parameters_for_state is None:
            raise RuntimeError("emotional_state not initialized or get_parameters_for_state not available")
    ...
    def get(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "musical_params": {
                ...
                "timing_feel": self.musical_params.timing_feel.value if self.musical_params and hasattr(self.musical_params.timing_feel, 'value') else None,
            } if self.musical_params else None,
            "mixer_params": self.mixer_params.to_dict() if self.mixer_params and hasattr(self.mixer_params, 'to_dict') else None,
            "paths": self._paths,
        }
    ...
    def describe(self) -> str:
        ...
        if self.musical_params:
            lines.extend([
                ...
                f"Timing: {self.musical_params.timing_feel.value if hasattr(self.musical_params.timing_feel, 'value') else self.musical_params.timing_feel}",
