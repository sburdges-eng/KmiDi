...
NOTES = {
    'C2': 36, 'D2': 38, 'E2': 40, 'F2': 41, 'G2': 43, 'A2': 45, 'B2': 47, 'C3': 48, 'C#3': 49, 'D3':
    50, 'D#3': 51, 'E3': 52, 'F3': 53, 'F#3': 54, 'G3': 55, 'G#3': 56, 'A3': 57, 'A#3': 58,
    'B3': 59, 'C4': 60, 'C#4': 61, 'D4': 62, 'D#4': 63, 'E4': 64, 'F4': 65, 'F#4': 66, 'G4': 67,
    'G#4': 68, 'A4': 69, 'A#4': 70, 'B4': 71, 'C5': 72, 'D5': 74, 'E5': 76, 'F5': 77, 'G5': 79,
    'A5': 81, 'B5': 83, }
...
def add_chord_strum(track, chord_name, duration_beats, velocity=70, strum_delay=15):
    ...
    for i, note in enumerate(notes):
        if i == 0:
            track.append(
                Message(
                    'note_off', note=note, velocity=0, time=duration_ticks -
                    (strum_delay * (len(notes) - 1))))
        else:
...
def add_arpeggio(track, chord_name, pattern_beats, velocity=60):
    ...
    notes = CHORDS[chord_name]
    # Each note gets portion of the beat
    note_duration = beats_to_ticks(pattern_beats / len(notes) / 2)
