    ...
    def _detect_rule_breaks(self, analyses: List[ChordAnalysis], chords: List[str]) -> List[str]:
        ...
        if len(analyses) > 0:
            ...
            if last_chord.scale_degree != 0:
                rule_breaks.append(
                    f"HARMONY_AvoidTonicResolution: Ends on "
                    f"{last_chord.roman_numeral} instead of tonic"
                )
...
def print_diagnostic_report(diagnostic: ProgressionDiagnostic):
    ...
    print("=" * 70)
    print("CHORD PROGRESSION DIAGNOSTIC")
    print("=" * 70)
    ...
    for analysis in diagnostic.analyses:
        ...
        print(
            f"{analysis.symbol:<10} {analysis.roman_numeral:<10} "
            f"{diatonic_str:<10} {analysis.emotional_function}"
        )
