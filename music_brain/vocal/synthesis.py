...
def formant_synthesize(
    ...
    for phoneme in phonemes:
        ...
        if phoneme.phoneme_type.value == "vowel":
            ...
            segment = _synthesize_vowel(avg_formants, phoneme, char, sample_rate, emotion_mods)
        else:
            # Synthesize consonant
            segment = _synthesize_consonant(phoneme, char, sample_rate)
...
def _synthesize_vowel(
    formants: FormantData, phoneme: Phoneme, char, sample_rate: int, emotion_mods: dict
) -> np.ndarray:
    ...
    attack_samples = (
        int(char.attack_time * sample_rate) if char.attack_time > 0 else int(0.01 * sample_rate)
    )
    release_samples = (
        int(char.release_time * sample_rate) if char.release_time > 0 else int(0.05 * sample_rate)
    )
...
def _synthesize_consonant(phoneme: Phoneme, char, sample_rate: int) -> np.ndarray:
...
def _generate_glottal_pulse(
    pitch_contour: np.ndarray, t: np.ndarray, sample_rate: int, char
) -> np.ndarray:
...
def _apply_formant_filter(
    audio: np.ndarray, formant_freq: float, bandwidth: float, sample_rate: int
) -> np.ndarray:
...
def _apply_timbre_shaping(audio: np.ndarray, char, sample_rate: int) -> np.ndarray:
...
def _apply_voice_characteristics(
    audio: np.ndarray, char, sample_rate: int, emotion_mods: dict
) -> np.ndarray:
    """Apply global voice characteristics."""
...
def _apply_emotion_to_formants(formants: FormantData, emotion_mods: dict) -> FormantData:
