...
# Fallback: np already imported above; NP_AVAILABLE tracks librosa deps
NP_AVAILABLE = LIBROSA_AVAILABLE
...
class VowelType(Enum):
    ...
    O_VOWEL = "o"  # "oh" as in "go" (E741: avoid ambiguous 'O')
    U = "u"  # "oo" as in "food"
...
class ParrotVocalSynthesizer:
    ...
    def _analyze_pitch(self, audio_data: np.ndarray, sr: int) -> Optional[Dict[str, float]]:
        ...
        if len(pitch_values) > 10:
            ...
            _pitch_diff = np.diff(pitch_array)  # noqa: F841
    ...
    def _analyze_accent(
        ...
        standard_formants = {
            ...
            VowelType.I_VOWEL: FormantData(f1=270, f2=2290, f3=3010),
            VowelType.O_VOWEL: FormantData(f1=570, f2=840, f3=2410),
