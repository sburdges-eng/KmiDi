...
from .base import GenerationResult, GenerativeConfig, GenerativeModel
...
class AudioDiffusion(GenerativeModel):
    ...
    def load(self, path: Optional[str] = None) -> None:
        ...
        """
        model_id = path or self.SUPPORTED_MODELS.get(self.config.model_type, self.config.model_id)
    ...
    def _load_audioldm(self, model_id: str) -> None:
        ...
        if not AUDIOLDM_AVAILABLE:
            warnings.warn("AudioLDM not available. Install with: pip install diffusers torch")
            return
    ...
    def _load_musicgen(self, model_id: str) -> None:
        ...
        if not MUSICGEN_AVAILABLE:
            warnings.warn("MusicGen not available. Install with: pip install transformers torch")
            return
