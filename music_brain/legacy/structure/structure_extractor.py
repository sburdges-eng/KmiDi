...
def analyze_melody_shape(intervals):
    ...
    if same_count / total > 0.6:
        ...
        contour[:midpoint].count("D") > midpoint * 0.5
        and contour[midpoint:].count("U") > (total - midpoint) * 0.5
    ):  # noqa: E501
...
def analyze_structure(filepath, name=None, genre=None):
    ...
    if harmonic_rhythm:
        print(
            f"    Harmonic rhythm: {harmonic_rhythm['pattern']} "
            f"({harmonic_rhythm['changes_per_bar']:.1f} changes/bar)"
        )  # noqa: E501
    ...
    if melody_notes:
        ...
        melody_stats = {
            ...
            "average_interval": (
                round(statistics.mean([abs(i) for i in intervals]), 2) if intervals else 0
            ),  # noqa: E501
            "interval_distribution": interval_dist,
            "predominant_shape": shape,
            "phrase_count": len(phrases),
            "avg_phrase_length_beats": (
                round(statistics.mean([p["length_beats"] for p in phrase_analysis]), 2)
                if phrase_analysis
                else 0
            ),  # noqa: E501
        }
        ...
        print(
            f"    Range: {range_semitones} semitones ({melody_stats['lowest_note']} to "
            f"{melody_stats['highest_note']})"
        )  # noqa: E501
    ...
    if melody:
        ...
        """,
            (
                ...
                melody["lowest_note"],
                melody["highest_note"],
                melody["average_interval"],
                json.dumps(melody["interval_distribution"]),
                melody["predominant_shape"],
                melody["phrase_count"],
                melody["avg_phrase_length_beats"],
            ),
        )
