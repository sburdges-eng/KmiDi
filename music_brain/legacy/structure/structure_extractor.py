...
def init_database():
    ...
    cursor.execute(
        'CREATE INDEX IF NOT EXISTS idx_progressions_analysis ON chord_progressions(analysis_id)')
...
def extract_chords_by_beat(notes, ppq, beats_per_bar=4):
    ...
    """
    ticks_per_beat = ppq
    _ticks_per_bar = ppq * beats_per_bar  # noqa: F841
...
def analyze_melody_shape(intervals):
    ...
    if same_count / total > 0.6:
        ...
        return 'arch', contour
    elif contour[:midpoint].count('D') > midpoint * 0.5 and contour[midpoint:].count('U') > (total - midpoint) * 0.5:  # noqa: E501
...
def analyze_structure(filepath, name=None, genre=None):
    ...
    if harmonic_rhythm:
        print(
            f"    Harmonic rhythm: {harmonic_rhythm['pattern']} ({harmonic_rhythm['changes_per_bar']:.1f} changes/bar)")  # noqa: E501
    ...
    if melody_notes:
        ...
        melody_stats = {
            ...
            'average_interval': round(statistics.mean([abs(i) for i in intervals]), 2) if intervals else 0,  # noqa: E501

            'interval_distribution': interval_dist,
            'predominant_shape': shape,
            'phrase_count': len(phrases),
            'avg_phrase_length_beats': round(statistics.mean([p['length_beats'] for p in phrase_analysis]), 2) if phrase_analysis else 0  # noqa: E501

        }

        print(
            f"    Range: {range_semitones} semitones ({melody_stats['lowest_note']} to {melody_stats['highest_note']})")  # noqa: E501
...
def save_to_database(analysis):
    ...
    cursor.execute('''
        ...
        (name, source_file, genre, key_detected, mode_detected, time_signature, bpm, total_bars, date_analyzed)  # noqa: E501
    ...
    for phrase in analysis.get('phrases', []):
        cursor.execute('''
            INSERT INTO phrases
            (analysis_id, phrase_number, start_bar, end_bar, length_beats, note_count, shape, contour)  # noqa: E501
    ...
    if hr:
        cursor.execute('''
            ...
            (analysis_id, avg_chord_duration_beats, changes_per_bar, rhythm_pattern, pattern_variance)  # noqa: E501

            VALUES (?, ?, ?, ?, ?)
        ''', (analysis_id, hr['avg_duration_beats'], hr['changes_per_bar'], hr['pattern'], hr['variance']))  # noqa: E501
...
def find_similar(analysis_id, limit=10):
    ...
    cursor.execute(
        'SELECT key_detected, mode_detected, bpm FROM structure_analysis WHERE id = ?',
        (analysis_id,))
    ref = cursor.fetchone()
