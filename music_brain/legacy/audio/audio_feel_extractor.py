...
def analyze_rms_swing_pattern(y, sr, bpm=None):
    ...
    subdivisions = 16
    _sub_interval = beat_interval / (subdivisions / 4)  # noqa: F841
...
def analyze_spectrum(y, sr):
    ...
    return {
        ...
        'spectral_movement': float(np.std(centroid) / np.mean(centroid)) if np.mean(centroid) > 0 else 0  # noqa: E501
...
def save_to_database(analysis):
    ...
    cursor.execute('''
        ...
        (name, source_file, genre, duration_seconds, sample_rate, channels, estimated_bpm, estimated_key, date_analyzed)  # noqa: E501
    ...
    cursor.execute('''
        ...
        (analysis_id, onset_count, avg_onset_interval_ms, onset_interval_std_ms, transient_drift_ms, attack_sharpness)  # noqa: E501
    ...
    cursor.execute('''
        ...
        (analysis_id, rms_mean, rms_std, rms_swing_ratio, dynamic_range_db, crest_factor_db, loudness_lufs)  # noqa: E501
    ...
    cursor.execute('''
        ...
        (analysis_id, spectral_centroid_mean, spectral_centroid_std, spectral_bandwidth_mean, spectral_rolloff_mean, spectral_flux_mean, brightness)  # noqa: E501
...
def list_analyses(genre=None, limit=50):
    ...
    for row in results:
        ...
        print(f"{id_:<6} {name_display:<30} {genre or '':<10} {bpm or 0:.0f}  {rms_swing or 0:.3f}     {drift or 0:.1f}")  # noqa: E501
...
def get_analysis_detail(analysis_id):
    ...
    # Genre matches
    cursor.execute(
        'SELECT genre, match_score FROM genre_matches WHERE analysis_id = ? ORDER BY match_score DESC LIMIT 5',  # noqa: E501
...
def compare_analyses(id1, id2):
    ...
    print(
        f"{'Dynamic Range':<20} {a1[2]:.1f}dB           {a2[2]:.1f}dB           {a2[2]-a1[2]:+.1f}dB")  # noqa: E501

    print(
        f"{'Transient Drift':<20} {a1[3]:.1f}ms           {a2[3]:.1f}ms           {a2[3]-a1[3]:+.1f}ms")  # noqa: E501
