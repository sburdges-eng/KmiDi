...
def apply_genre_pocket(midi_path, genre, output_path=None, intensity=1.0):
    ...
    if genre not in pocket_maps:
        print(
            f"Genre '{genre}' not found. Available: {[k for k in pocket_maps if not k.startswith('_')]}")  # noqa: E501
        ...
        for msg in track:
            ...
            if msg.type in ['note_on', 'note_off']:
                ...
                if inst_type in push_pull:
                    ...
                    closest_pos = min(
                        pp_data.keys(),
                        key=lambda x: abs(float(x) - beat_pos_16th),
                        default=None)
                    if closest_pos:
                ...
                if msg.type == 'note_on' and msg.velocity > 0:
                    ...
                    if inst_type in vel_curves:
                        ...
                        closest_pos = min(
                            vel_data.keys(),
                            key=lambda x: abs(int(x) - beat_pos_16th),
                            default=None)
                        if closest_pos:
...
Examples:
  ...
  %(prog)s list-templates
    ...
    genre_parser.add_argument('--intensity', '-i', type=float,
                              default=1.0, help='Effect intensity 0.0-1.0')
    ...
    template_parser.add_argument('--intensity', '-i', type=float,
                                 default=1.0, help='Effect intensity 0.0-1.0')
    ...
    humanize_parser.add_argument('--timing', '-t', type=int, default=10,
                                 help='Timing variation in ticks')
    humanize_parser.add_argument('--velocity', '-v', type=int,
                                 default=15, help='Velocity variation')
    humanize_parser.add_argument('--intensity', '-i', type=float,
                                 default=1.0, help='Effect intensity 0.0-1.0')
