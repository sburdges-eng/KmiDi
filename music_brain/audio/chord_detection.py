...
class ChordDetector:
    ...
    def __init__(
        ...
        if not LIBROSA_AVAILABLE:
            raise ImportError(
                "librosa required for ChordDetector. Install with: pip install librosa numpy"
            )
    ...
    def _merge_consecutive_chords(self, chords: List[ChordDetection]) -> List[ChordDetection]:
        """Merge consecutive identical chords."""
        if not chords:
            return []
    ...
    def _estimate_key_from_chords(self, chords: List[ChordDetection]) -> Optional[str]:
