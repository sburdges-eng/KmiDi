...
class AnalysisConfig:
    """Configurable timing thresholds (milliseconds at target BPM)."""

    flam_threshold_ms: float = 30.0
    buzz_threshold_ms: float = 50.0
    drag_threshold_ms: float = 80.0
    alternation_window_ms: float = 200.0

    @classmethod
    def from_dict(cls, data: dict) -> "AnalysisConfig":
        return cls(
            flam_threshold_ms=data.get("flam_threshold_ms", 30.0),
            buzz_threshold_ms=data.get("buzz_threshold_ms", 50.0),
...
class DrumAnalyzer:
    ...
    def __init__(
        ...
        self.flam_ticks = self._ms_to_ticks(self.config.flam_threshold_ms)
        self.buzz_ticks = self._ms_to_ticks(self.config.buzz_threshold_ms)
        self.drag_ticks = self._ms_to_ticks(self.config.drag_threshold_ms)
        self.alternation_ticks = self._ms_to_ticks(self.config.alternation_window_ms)
    ...
    def _analyze_snare_bounces(self, snare_notes: List) -> SnareBounceSignature:
        ...
        while i < len(snare_notes) - 1:
            ...
            if len(cluster) >= 2:
                ...
                if len(cluster) == 2 and gap_ms <= self.config.flam_threshold_ms:
                    ...
                    # Buzz roll: relatively even velocity over 3+ notes
