...
# ---------------------------------------------------------------------------
try:
    from music_brain.voice.phoneme_processor import PhonemeProcessor, Phoneme
...
class PRROTBridge:
    ...
    def __init__(self) -> None:
        ...
        if self._lib is not None:
            try:
                ...
                logger.warning(
                    "PRROTBridge: exception creating native engine: %s. "
                    "Falling back to Python.",
                    exc,
                )
                self._lib = None
    ...
    def initialize(self) -> bool:
        ...
        """
        if self.is_native:
            ok = bool(self._lib.kelly_prrot_initialize(self._native_handle))  # type: ignore[union-attr]
