...
class InstrumentSynthesizer:
    ...
    def _generate_adsr(self, num_samples: int) -> np.ndarray:
        ...
        # Clamp segments so notes shorter than the preset envelope times
        # degrade gracefully instead of overflowing the buffer.
        attack_samples = min(attack_samples, num_samples)
        release_samples = min(release_samples, max(0, num_samples - attack_samples))
        ...
        # Sustain
        sustain_start = max(decay_end, decay_start)
        ...
        if release_samples > 0:
            envelope[num_samples - release_samples :] = np.linspace(
    ...
    def save_audio(self, audio: np.ndarray, output_path: str) -> None:
        ...
        try:
            if sf is not None:
                ...
                print(f"soundfile not available, cannot save to {output_path}")
