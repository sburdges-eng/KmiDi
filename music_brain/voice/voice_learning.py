...
def _sanitize_id(identifier: str) -> str:
    """Sanitize a sample/profile identifier for safe use as a filename stem.

    Strips path separators, parent references, drive/stream separators, and
    NUL bytes so a crafted identifier cannot escape the storage directory on
    any platform (POSIX or Windows).
    """
    for token in ("/", "\\", "..", ":", "\x00"):
        identifier = identifier.replace(token, "_")
    return identifier
...
class VoiceSampleStore:
    ...
    def save_sample(
        ...
        sample_id = _sanitize_id(sample_id)
        ...
        if sf is not None:
            ...
            print(f"Warning: soundfile not available, cannot save {audio_path}")
    ...
    def load_sample(self, sample_id: str) -> Optional[VoiceSample]:
        ...
        try:
            if sf is None:
                print("soundfile not available for loading")
                return None
...
class VoiceLearningManager:
    ...
    def save_profile(self, profile: LearnedVoiceProfile) -> Path:
        ...
        safe_name = _sanitize_id(profile.name)
        profile_path = self.store.profiles_dir / f"{safe_name}.json"
    ...
    def load_profile(self, profile_name: str) -> Optional[LearnedVoiceProfile]:
        ...
        profile_name = _sanitize_id(profile_name)
