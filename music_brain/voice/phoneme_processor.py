...
class PhonemeProcessor:
    ...
    def estimate_durations(
        ...
        for i, (phoneme, rel_dur) in enumerate(zip(phonemes, relative_durations)):
            ...
            phoneme_obj = Phoneme(
                symbol=phoneme, duration_ms=duration_ms, start_time_ms=current_time
            )
            phoneme_objects.append(phoneme_obj)
            current_time += duration_ms

        return PhonemeSequence(phonemes=phoneme_objects, total_duration_ms=current_time)

    def align_to_melody(
        self, phonemes: List[str], melody_notes: List[int], tempo_bpm: float = 120.0
    ) -> PhonemeSequence:
        """
        ...
        for note_idx in range(total_notes):
            ...
            if not note_phonemes:
                ...
                phoneme_obj = Phoneme(
                    symbol="SIL", duration_ms=note_duration_ms, start_time_ms=current_time
                )
                phoneme_objects.append(phoneme_obj)
            else:
                ...
                for phoneme in note_phonemes:
                    phoneme_obj = Phoneme(
                        symbol=phoneme, duration_ms=phoneme_duration, start_time_ms=current_time
                    )
                    phoneme_objects.append(phoneme_obj)
    ...
    def process_lyrics(
        self, lyrics: str, melody_notes: Optional[List[int]] = None, tempo_bpm: float = 120.0
    ) -> PhonemeSequence:
        """
...
def process_lyrics(
    lyrics: str, melody_notes: Optional[List[int]] = None, tempo_bpm: float = 120.0
) -> PhonemeSequence:
    """
