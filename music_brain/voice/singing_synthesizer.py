...
CONSONANT_PARAMS = {
    ...
    "DH": {"type": "fricative", "freq": 1500, "noise": False},
    "JH": {"type": "fricative", "freq": 2000, "noise": True},
...
class SingingSynthesizer:
    ...
    def _synthesize_vowel(
        ...
        if frequency_curve is not None and len(frequency_curve) == len(t):
            # Use frequency curve (vectorized)
            valid = frequency_curve > 0
            periods = np.where(valid, 1.0 / np.maximum(frequency_curve, 1e-6), 1.0)
            phase = (t % periods) / periods
            waveform = np.where(valid, np.sin(np.pi * phase) ** 2 * (1 - phase), 0.0)
        else:
            # Constant frequency
    ...
    def _synthesize_consonant(
        ...
        if signal is None:
            # scipy not available, return simple noise or silence
            if params["noise"]:
