...
def _load_audio_mel(audio_path: str, duration: float = 3.0, sr: int = 22050, n_mels: int = 128) -> np.ndarray:
    try:
...
class VoiceClassifier:
    ...
    def classify(self, audio_path: str, top_k: int = 3) -> Dict[str, object]:
        ...
        ranked = [
            {"label": self.class_names[i] if i < len(self.class_names) else str(i), "score": float(probs[i])}
            for i in idx
        ]
