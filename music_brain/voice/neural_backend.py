...
try:
    ...
    if find_spec("diffsinger") or find_spec("DiffSinger"):
        DIFFSINGER_AVAILABLE = True
except ImportError:
    pass

# Default model paths — honor KELLY_MODEL_ROOT (data governance), fall back to legacy ~/.idaw
_MODEL_ROOT = os.environ.get("KELLY_MODEL_ROOT")
DEFAULT_MODEL_DIR = (
    Path(_MODEL_ROOT) / "voice" if _MODEL_ROOT else Path.home() / ".idaw" / "models" / "voice"
)
...
class NeuralBackend:
    ...
    def _griffin_lim(
        ...
        try:
            ...
            # Convert mel spectrogram to linear STFT magnitude, then Griffin-Lim
            mel_power = librosa.db_to_power(mel.squeeze())
            stft_mag = librosa.feature.inverse.mel_to_stft(
                mel_power,
                sr=self.config.sample_rate,
                n_fft=self.config.win_length,
            )
            return librosa.griffinlim(stft_mag, hop_length=self.config.hop_length, n_iter=n_iter)
        except (ImportError, AttributeError, Exception):
            # Return silence if librosa not available or conversion fails
