...
class PitchController:
    ...
    def add_pitch_bend(
        ...
        # Sort bend points by time and apply per-segment to avoid compounding
        sorted_points = sorted(bend_points, key=lambda x: x[0])
        for i, (time_sec, bend_semitones) in enumerate(sorted_points):
            start_idx = int(time_sec * self.sample_rate)
            if i + 1 < len(sorted_points):
                end_idx = int(sorted_points[i + 1][0] * self.sample_rate)
            else:
                end_idx = len(frequencies)
            if 0 <= start_idx < len(frequencies):
                # Apply bend only to this segment
                frequencies[start_idx:end_idx] *= (2 ** (bend_semitones / 12.0))
    ...
    def audio_to_midi_notes(
        ...
        if len(frequencies) == 0:
            return [60]  # Default to middle C for empty/too-short audio

        # Convert to MIDI notes
        midi_notes = []
        samples_per_note = max(1, int(note_duration * len(frequencies)))
