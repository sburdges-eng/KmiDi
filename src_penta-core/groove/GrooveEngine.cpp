...
void GrooveEngine::processAudio(const float *buffer, size_t frames) noexcept {
  ...
  if (configUpdating_.load(std::memory_order_acquire))
    return;
  ...
  if (onsetDetector_) {
    ...
    if (onsetDetector_->hasOnset()) {
      ...
      // audio thread.
      // static so the captureless lambda may use it: MSVC, unlike
      // gcc/clang, rejects uncaptured non-static constexpr locals
      // (C3493).
      static constexpr size_t kMaxOnsetHistory = 128;
      ...
      if (quantizer_) {
        ...
        qConfig.swingAmount =
            std::clamp(0.5f + (analysis_.swing * 0.25f), 0.0f, 1.0f);
...
uint64_t GrooveEngine::quantizeToGrid(uint64_t timestamp) const noexcept {
  if (!quantizer_ || !config_.enableQuantization ||
      analysis_.currentTempo <= 0.0f) {
    return timestamp;
  }

  // Calculate samples per beat based on current tempo
  uint64_t samplesPerBeat = static_cast<uint64_t>((60.0 * config_.sampleRate) /
                                                  analysis_.currentTempo);
...
void GrooveEngine::detectTimeSignature() {
  ...
  for (size_t i = 0; i < analysis_.onsetPositions.size(); ++i) {
    ...
    float strength = (i < analysis_.onsetStrengths.size())
                         ? analysis_.onsetStrengths[i]
                         : 1.0f;

    auto beatIndex = static_cast<size_t>(
        std::llround(static_cast<double>(onset - firstOnset) / samplesPerBeat));
...
void GrooveEngine::analyzeSwing() {
  ...
  const double beatIntervalSamples =
      (60.0 / analysis_.currentTempo) * config_.sampleRate;
  ...
  for (uint64_t onset : analysis_.onsetPositions) {
    double beatPos =
        std::fmod(static_cast<double>(onset), beatIntervalSamples) /
        beatIntervalSamples;
  ...
  float avgOffBeat =
      static_cast<float>(sumOffBeat / static_cast<double>(countOffBeat));
